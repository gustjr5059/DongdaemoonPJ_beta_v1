...
function generateOrderEmailBody(ordererInfo, recipientInfo, amountInfo, productInfo, orderNumber) {
  ...
  body += `<tr><td><b>상품 할인금액</b></td><td>${formatNumber(amountInfo.product_discount_price)}원</td></tr>`;
  body += `<tr><td><b>선택된 배송비</b></td><td>${formatNumber(amountInfo.delivery_fee)}원</td></tr>`;
  body += `<tr><td><b>총 결제금액</b></td><td>${formatNumber(amountInfo.total_payment_price_included_delivery_fee)}원</td></tr>`;
  body += `</table><br>`;
...
exports.sendMessageWithNotification_V2 = functions.region('asia-northeast3').firestore
  .document('wearcano_message_list/{recipientId}/message/{messageId}')
  .onCreate(async (snap, context) => {
    const messageData = snap.data();
    const recipientId = context.params.recipientId;
    ...
    const message = {
      ...
      apns: {
        headers: {
          ...
          'apns-priority': '10',
          // 'apns-topic': '여기에_실제_BundleID를_정확히_기재',
          // (예: 'com.yourcompany.myapp') - 실제 값으로 수정 필요
          'apns-topic': 'com.gshe.wearcano',
        },
        payload: {
          aps: {
            sound: 'default',
