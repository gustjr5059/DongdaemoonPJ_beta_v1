...
// AaiAutumnSubMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AaiAutumnSubMainScreen extends ConsumerStatefulWidget {
  const AaiAutumnSubMainScreen({Key? key}) : super(key: key);

  @override
  _AaiAutumnSubMainScreenState createState() => _AaiAutumnSubMainScreenState();
}

// _AaiAutumnSubMainScreenState 클래스 시작
// _AaiAutumnSubMainScreenState 클래스는 AaiAutumnSubMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AaiAutumnSubMainScreenState extends ConsumerState<AaiAutumnSubMainScreen>
  ...
  // aaiAutumnSubMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aaiAutumnSubMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aaiAutumnSubMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void _updateScrollPosition() {
    ...
    double currentScrollPosition =
        autumnSubMainScreenPointScrollController.offset;

    // 'ref'를 사용하여 'aaiAutumnSubMainScrollPositionProvider'의 notifier를 읽어옴.
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (autumnSubMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition =
            ref.read(aaiAutumnSubMainScrollPositionProvider);
      ...
      // -> 가을 섹션 더보기 화면 초기화 시, 하단 탭 바 내 모든 버튼 비활성화
  ...
  void dispose() {
    ...
    autumnSubMainScreenPointScrollController
  ...
  Widget build(BuildContext context) {
    ...
    //
    // // 대배너 부분 관련 수치
    // final double autumnSubMainScreenLargeBannerWidth =
    //     screenSize.width * (393 / referenceWidth); // 대배너 이미지 너비
    // final double autumnSubMainScreenLargeBannerHeight =
    ...
    //
    // // 소배너 부분 관련 수치
    // final double autumnSubMainScreenSmallBannerWidth =
    //     screenSize.width * (361 / referenceWidth); // 소배너 이미지 너비
    // final double autumnSubMainScreenSmallBannerHeight =
    ...
    final double autumnSubMainScreenLargeBannerHeight = screenSize.width * (378 / referenceWidth); // 대배너 이미지 높이
    final double autumnSubMainLargeBannerViewHeight = screenSize.width * (378 / referenceWidth); // 대배너 화면 세로 비율

    // 소배너 부분 관련 수치
    final double autumnSubMainScreenSmallBannerWidth = screenSize.width * (361 / referenceWidth); // 소배너 이미지 너비
    final double autumnSubMainScreenSmallBannerHeight = screenSize.width * (90 / referenceWidth); // 소배너 이미지 높이
    final double autumnSubMainScreenSmallBannerViewHeight = screenSize.width * (90 / referenceWidth); // 소배너 화면 세로 비율
                          ...
                          children: [
                            ...
                            CommonCardView(
                              content: Container(
                                ...
                                child: SizedBox(
                                  height: autumnSubMainLargeBannerViewHeight,
                                  // 큰 배너 섹션의 높이를 설정함
                                  child: buildCommonBannerPageViewSection<
                                      AllLargeBannerImage>(
                                    context: context,
                                    // 위젯 트리를 위한 빌드 컨텍스트를 전달함
                                    ref: ref,
                                    // 상태 관리를 위한 참조를 전달함
                                    currentPageProvider:
                                        aaiAutumnSubMainLargeBannerPageProvider,
                                    // 현재 배너 페이지의 상태 제공자를 전달함
                                    pageController: _largeBannerPageController,
                                    ...
                                    width: autumnSubMainScreenLargeBannerWidth,
                                    // 배너 섹션의 너비를 설정함
                                    height:
                                        autumnSubMainScreenLargeBannerHeight,
                                    // 배너 섹션의 높이를 설정함
                            ...
                            CommonCardView(
                              content: Container(
                                ...
                                child: SizedBox(
                                  height:
                                      autumnSubMainScreenSmallBannerViewHeight,
                                  // 작은 배너 섹션의 높이를 설정함
                                  child: buildCommonBannerPageViewSection<
                                      AllSmallBannerImage>(
                                    context: context,
                                    ...
                                    // 상태 관리를 위한 참조를 전달함
                                    currentPageProvider:
                                        aaiAutumnSubMainSmall1BannerPageProvider,
                                    // 작은 배너 페이지의 상태 제공자를 전달함
                                    pageController: _small1BannerPageController,
                                    // 작은 배너 페이지의 스크롤을 제어할 컨트롤러를 전달함
                                    bannerAutoScroll: _small1BannerAutoScroll,
                                    // 작은 배너 자동 스크롤 설정을 전달함
                                    bannerImagesProvider:
                                        aaiAutumnSubMainSmall1BannerImagesProvider,
                                    ...
                                    onPageTap: (context, index) =>
                                        ...
                                        aaiOnSmallBannerTap(
                                            context, // 현재 화면의 컨텍스트를 전달함
                                            index, // 클릭된 배너의 인덱스를 전달함
                                            // aaiAutumnSubMainSmall1BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                            ref
                                                    .watch(
                                                        aaiAutumnSubMainSmall1BannerImagesProvider)
                                                    .value ??
                                    ...
                                    width: autumnSubMainScreenSmallBannerWidth,
                                    // 작은 배너 섹션의 너비를 설정함
                                    height:
                                        autumnSubMainScreenSmallBannerHeight,
                                    // 작은 배너 섹션의 높이를 설정함
                            ...
                            AaiPriceAndDiscountPercentSortButtons<
                              ...
                              productListProvider:
                                  aaiAutumnSubMainProductListProvider,
                              // 가을 제품 리스트 프로바이더 전달
                              sortButtonProvider:
                                  aaiAutumnSubMainSortButtonProvider, // 가을 정렬 버튼 프로바이더 전달
                            ),
                            // 가격 및 할인 정렬 버튼 추가
                            SizedBox(height: interval1Y),
