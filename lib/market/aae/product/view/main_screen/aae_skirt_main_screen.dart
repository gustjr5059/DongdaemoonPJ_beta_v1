...
// AaeSkirtMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AaeSkirtMainScreen extends ConsumerStatefulWidget {
  const AaeSkirtMainScreen({Key? key}) : super(key: key);

  @override
  _AaeSkirtMainScreenState createState() => _AaeSkirtMainScreenState();
}

// _AaeSkirtMainScreenState 클래스 시작
// _AaeSkirtMainScreenState 클래스는 AaeSkirtMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AaeSkirtMainScreenState extends ConsumerState<AaeSkirtMainScreen>
  ...
  // aaeSkirtMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aaeSkirtMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aaeSkirtMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void _updateScrollPosition() {
    ...
    // 'ref'를 사용하여 'aaeSkirtMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (skirtMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(aaeSkirtMainScrollPositionProvider);
      ...
      // aaeSkirtCurrentTabProvider를 통해 저장된 탭 인덱스를 읽어옴.
      int savedTabIndex = ref.read(aaeSkirtCurrentTabProvider);
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(aaeSkirtMainLargeBannerPageProvider.notifier).state = 0;
        ref.read(aaeSkirtMainSmall1BannerPageProvider.notifier).state = 0;
        ref.read(aaeSkirtMainScrollPositionProvider.notifier).state =
            0.0; // 로그아웃 시 aaeSkirtMainScrollPositionProvider가 초기화되므로, 재로그인 시 초기 스크롤 위치에서 시작됨. 하지만 상품 데이터는 유지됨.
  ...
  // aaeSkirtMainProductListProvider 관련 카테고리 인덱스를 받아 해당 카테고리 이름을 반환하는 함수
  ...
  Widget build(BuildContext context) {
    ...
    void onTopBarTap(int index) {
      ...
      Future.delayed(Duration.zero, () {
        ref.read(aaeSkirtMainProductListProvider.notifier).reset(); // 상태 초기화
        ref.read(aaeSkirtMainProductListProvider.notifier).fetchInitialProducts(
    ...
    Widget topBarList = buildTopBarList(context, onTopBarTap,
        aaeSkirtCurrentTabProvider, skirtMainTopBarPointAutoScrollController);
                      ...
                      child: Column(
                        children: [
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  currentPageProvider:
                                  aaeSkirtMainLargeBannerPageProvider, // 큰 배너 페이지의 상태 제공자를 전달함
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  ref: ref, // 상태 관리를 위한 참조 전달함
                                  currentPageProvider:
                                  aaeSkirtMainSmall1BannerPageProvider, // 작은 배너 페이지의 상태 제공자를 전달함
                                  ...
                                  bannerImagesProvider:
                                  aaeSkirtMainSmall1BannerImagesProvider, // 작은 배너 이미지의 상태 제공자를 전달함
                                  // 배너를 탭했을 때 실행할 함수를 전달
                                  onPageTap: (context, index) =>
                                  ...
                                  aaeOnSmallBannerTap(
                                      context, // 현재 화면의 컨텍스트를 전달함
                                      index, // 클릭된 배너의 인덱스를 전달함
                                      // aaeSkirtMainSmall1BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                      ref.watch(aaeSkirtMainSmall1BannerImagesProvider).value ?? [],
                                      ref // Provider의 참조를 전달함
                                  ),
                          ...
                          AaePriceAndDiscountPercentSortButtons<
                            ...
                            productListProvider: aaeSkirtMainProductListProvider,
                            sortButtonProvider: aaeSkirtMainSortButtonProvider,
                          ), // 가격 및 할인 정렬 버튼 추가
                          ...
                          Consumer(
                            ...
                            builder: (context, ref, child) {
                              ...
                              final currentTab =
                              ref.watch(aaeSkirtCurrentTabProvider);
                              // 현재 탭: aaeSkirtCurrentTabProvider를 구독하고 현재 선택된 탭 정보를 가져옴.
                              final productListProvider =
                                  aaeSkirtMainProductListProvider;
                              // 제품 리스트 제공자: aaeSkirtMainProductListProvider를 productListProvider 변수에 할당.
