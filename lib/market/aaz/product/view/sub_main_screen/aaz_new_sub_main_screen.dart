...
// AazNewSubMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AazNewSubMainScreen extends ConsumerStatefulWidget {
  const AazNewSubMainScreen({Key? key}) : super(key: key);

  @override
  _AazNewSubMainScreenState createState() => _AazNewSubMainScreenState();
}

// _AazNewSubMainScreenState 클래스 시작
// _AazNewSubMainScreenState 클래스는 AazNewSubMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AazNewSubMainScreenState extends ConsumerState<AazNewSubMainScreen>
  ...
  // aazNewSubMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aazNewSubMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aazNewSubMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  late ScrollController newSubMainScreenPointScrollController; // 스크롤 컨트롤러 선언
  ...
  void _updateScrollPosition() {
    // 'newSubMainScreenPointScrollController'에서 현재의 스크롤 위치(offset)를 가져와서 'currentScrollPosition' 변수에 저장함.
    double currentScrollPosition = newSubMainScreenPointScrollController.offset;

    // 'ref'를 사용하여 'aazNewSubMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    newSubMainScreenPointScrollController = ScrollController();
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (newSubMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(aazNewSubMainScrollPositionProvider);
        // newSubMainScreenPointScrollController.jumpTo 메서드를 사용하여 스크롤 위치를 savedScrollPosition으로 즉시 이동함.
        // 이는 스크롤 애니메이션이나 다른 복잡한 동작 없이 바로 지정된 위치로 점프함.
        newSubMainScreenPointScrollController.jumpTo(savedScrollPosition);
      }

      // tabIndexProvider의 상태를 하단 탭 바 내 버튼과 매칭이 되면 안되므로 0~3이 아닌 -1로 매핑
      // -> 신상 섹션 더보기 화면 초기화 시, 하단 탭 바 내 모든 버튼 비활성화
    ...
    // 사용자가 스크롤할 때마다 현재의 스크롤 위치를 newSubMainScreenPointScrollController에 저장하는 코드
    // 상단 탭바 버튼 클릭 시, 해당 섹션으로 화면 이동하는 위치를 저장하는거에 해당 부분도 추가하여
    // 사용자가 앱을 종료하거나 다른 화면으로 이동한 후 돌아왔을 때 마지막으로 본 위치로 자동으로 스크롤되도록 함.
    newSubMainScreenPointScrollController.addListener(_updateScrollPosition);

    // 큰 배너에 대한 PageController 및 AutoScroll 초기화
    // 'aazNewSubMainLargeBannerPageProvider'에서 초기 페이지 인덱스를 읽어옴
    _largeBannerPageController = PageController(
        initialPage: ref.read(aazNewSubMainLargeBannerPageProvider));

    // 큰 배너를 자동으로 스크롤하는 기능 초기화
    _largeBannerAutoScroll = BannerAutoScrollClass(
      pageController: _largeBannerPageController,
      currentPageProvider: aazNewSubMainLargeBannerPageProvider,
      itemCount: bannerImageCount1, // 총 배너 이미지 개수 전달
    );

    // 작은 배너1에 대한 PageController 및 AutoScroll 초기화
    // 'aazNewSubMainSmall1BannerPageProvider'에서 초기 페이지 인덱스를 읽어옴
    _small1BannerPageController = PageController(
        initialPage: ref.read(aazNewSubMainSmall1BannerPageProvider));

    // 작은 배너1을 자동으로 스크롤하는 기능 초기화
    _small1BannerAutoScroll = BannerAutoScrollClass(
      pageController: _small1BannerPageController,
      currentPageProvider: aazNewSubMainSmall1BannerPageProvider,
  ...
  void dispose() {
    ...
    // 'newSubMainScreenPointScrollController'의 리스너 목록에서 '_updateScrollPosition' 함수를 제거함.
    // 이는 '_updateScrollPosition' 함수가 더 이상 스크롤 이벤트에 반응하지 않도록 설정함.
    newSubMainScreenPointScrollController.removeListener(_updateScrollPosition);

    newSubMainScreenPointScrollController.dispose(); // ScrollController 해제
  ...
  Widget build(BuildContext context) {
    ...
    //
    // // 대배너 부분 관련 수치
    // final double newSubMainScreenLargeBannerWidth =
    //     screenSize.width * (393 / referenceWidth); // 대배너 이미지 너비
    // final double newSubMainScreenLargeBannerHeight =
    ...
    //
    // // 소배너 부분 관련 수치
    // final double newSubMainScreenSmallBannerWidth =
    //     screenSize.width * (361 / referenceWidth); // 소배너 이미지 너비
    // final double newSubMainScreenSmallBannerHeight =
    ...
    final double newSubMainScreenLargeBannerHeight = screenSize.width * (378 / referenceWidth); // 대배너 이미지 높이
    final double newSubMainLargeBannerViewHeight = screenSize.width * (378 / referenceWidth); // 대배너 화면 세로 비율

    // 소배너 부분 관련 수치
    final double newSubMainScreenSmallBannerWidth = screenSize.width * (361 / referenceWidth); // 소배너 이미지 너비
    final double newSubMainScreenSmallBannerHeight = screenSize.width * (90 / referenceWidth); // 소배너 이미지 높이
    final double newSubMainScreenSmallBannerViewHeight = screenSize.width * (90 / referenceWidth); // 소배너 화면 세로 비율
    ...
    final double sectionPlusChevronIconHeight = 24;
    final double sectionPlusChevronIconX = screenSize.width * (10 / referenceWidth);
    final double sectionPlusChevronIconY = 9;

    // 찜 목록 버튼 수치 (Case 2)
    final double sectionPlusWishlistBtnWidth = 40;
    final double sectionPlusWishlistBtnHeight = 40;
    final double sectionPlusWishlistBtnX = 10;
    final double sectionPlusWishlistBtnY = 7;
    ...
    return Scaffold(
      body: Stack(
        children: [
          CustomScrollView(
            controller: newSubMainScreenPointScrollController, // 스크롤 컨트롤러 연결
            slivers: <Widget>[
                        ...
                        child: Column(
                          children: [
                            ...
                            CommonCardView(
                              content: Container(
                                ...
                                child: SizedBox(
                                  ...
                                  height: newSubMainLargeBannerViewHeight,
                                  ...
                                  child: buildCommonBannerPageViewSection<
                                    ...
                                    // 상태 관리를 위한 참조 전달함
                                    currentPageProvider:
                                        aazNewSubMainLargeBannerPageProvider,
                                    ...
                                    aazAllLargeBannerImagesProvider,
                                    // 배너 이미지의 상태 제공자를 전달함
                                    ...
                                    width: newSubMainScreenLargeBannerWidth,
                                    // 배너 섹션의 너비를 설정함
                                    height: newSubMainScreenLargeBannerHeight,
                                    // 배너 섹션의 높이를 설정함
                            ...
                            CommonCardView(
                              content: Container(
                                ...
                                child: SizedBox(
                                  ...
                                  height: newSubMainScreenSmallBannerViewHeight,
                                  ...
                                  child: buildCommonBannerPageViewSection<
                                    ...
                                    // 상태 관리를 위한 참조 전달함
                                    currentPageProvider:
                                        aazNewSubMainSmall1BannerPageProvider,
                                    ...
                                    bannerImagesProvider:
                                        aazNewSubMainSmall1BannerImagesProvider,
                                    ...
                                    onPageTap: (context, index) =>
                                        ...
                                        aazOnSmallBannerTap(
                                            context, // 현재 화면의 컨텍스트를 전달함
                                            index, // 클릭된 배너의 인덱스를 전달함
                                            // aazNewSubMainSmall1BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                            ref
                                                    .watch(
                                                        aazNewSubMainSmall1BannerImagesProvider)
                                                    .value ??
                                    ...
                                    width: newSubMainScreenSmallBannerWidth,
                                    // 작은 배너 섹션의 너비를 설정함
                                    height: newSubMainScreenSmallBannerHeight,
                                    // 작은 배너 섹션의 높이를 설정함
                            ...
                            AazPriceAndDiscountPercentSortButtons<
                                AazSectionMoreProductListNotifier>(
                              productListProvider:
                                  aazNewSubMainProductListProvider,
                              // 신상 제품 리스트 프로바이더 전달
                              sortButtonProvider:
                                  aazNewSubMainSortButtonProvider, // 신상 정렬 버튼 프로바이더 전달
                            ),
                            // 가격 및 할인 정렬 버튼 추가
                            SizedBox(height: interval1Y),
                            // AazGeneralProductList 위젯을 생성, AazSectionMoreProductListNotifier를 사용
                            AazGeneralProductList<AazSectionMoreProductListNotifier>(
                              // 스크롤 컨트롤러를 설정 (신상 섹션의 스크롤 컨트롤러)
                              scrollController:
                                  newSubMainScreenPointScrollController,
                              // 상품 리스트 프로바이더를 설정 (신상 섹션의 상품 리스트 프로바이더)
                              productListProvider:
                                  aazNewSubMainProductListProvider,
                              // 카테고리를 '신상'으로 설정
                              category: '신상',
          ...
          // buildTopButton 함수는 주어진 context와 newSubMainScreenPointScrollController를 사용하여
          // 화면 상단으로 스크롤하기 위한 버튼 생성 위젯이며, common_body_parts_layout.dart 내에 있는 곳에서 재사용하여 구현한 부분
          buildTopButton(context, newSubMainScreenPointScrollController),
        ],
      ),
      // 하단 탭 바 - 1번 케이스인 '홈','장바구니', '발주내역', '마이페이지' 버튼이 UI로 구현됨.
      bottomNavigationBar: buildCommonBottomNavigationBar(
          ref.watch(tabIndexProvider), ref, context, 5, 1,
          scrollController: newSubMainScreenPointScrollController),
