...
// AazCardiganDetailProductScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AazCardiganDetailProductScreen extends ConsumerStatefulWidget {
  final String fullPath; // 전체 경로를 나타내는 문자열 변수
  final String title; // 타이틀을 나타내는 문자열 변수

  const AazCardiganDetailProductScreen({
  ...
  @override
  _AazCardiganDetailProductScreenState createState() =>
      _AazCardiganDetailProductScreenState();
}

// _AazCardiganDetailProductScreenState 클래스 시작
// _AazCardiganDetailProductScreenState 클래스는 AazCardiganDetailProductScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AazCardiganDetailProductScreenState
    extends ConsumerState<AazCardiganDetailProductScreen>
    with WidgetsBindingObserver, RouteAware {
  // 사용자 인증 상태 변경을 감지하는 스트림 구독 객체임.
  // 이를 통해 사용자 로그인 또는 로그아웃 상태 변경을 실시간으로 감지하고 처리할 수 있음.
  StreamSubscription<User?>? authStateChangesSubscription;

  // aazCardiganDetailScrollPositionProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aazCardiganDetailScrollPositionProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aazCardiganDetailScrollPositionProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  late ScrollController
      cardiganDetailProductScreenPointScrollController; // 스크롤 컨트롤러 선언
  ...
  void initState() {
    ...
    print(
        "AazCardiganDetailProductScreen: 제품 경로에 대한 화면 로드 중: ${widget.fullPath}"); // 디버깅 메시지 추가
    ...
    cardiganDetailProductScreenPointScrollController = ScrollController();

    // 스크롤 리스너 추가: 리뷰 탭일 때만 다음 페이지 로드
    cardiganDetailProductScreenPointScrollController.addListener(() {
      // 현재 어떤 탭인지 확인
      ...
      if (currentTabSection == ProdDetailScreenTabSection.reviews) {
        if (cardiganDetailProductScreenPointScrollController.position.pixels ==
            cardiganDetailProductScreenPointScrollController.position.maxScrollExtent) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (cardiganDetailProductScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition =
            ref.read(aazCardiganDetailScrollPositionProvider);
        // aazCardiganDetailProductScreenPointScrollController.jumpTo 메서드를 사용하여 스크롤 위치를 savedScrollPosition으로 즉시 이동함.
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(aazCardiganDetailScrollPositionProvider.notifier).state =
            0; // cardiganDetailScrollPositionProvider의 상태를 0으로 설정
        ref.read(getImagePageProvider(widget.fullPath).notifier).state =
    ...
    // 스크롤 리스너 추가
    cardiganDetailProductScreenPointScrollController.addListener(_onScroll);
  }
  ...
  void dispose() {
    ...
    // 스크롤 리스너 제거
    cardiganDetailProductScreenPointScrollController.removeListener(_onScroll);
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<AsyncValue<ProductContent>>(
      aazCardiganDetailProdFirestoreDataProvider(widget.fullPath),
          (previous, next) {
        // 데이터 상태가 로딩 중 -> 데이터 로드 완료로 변경되는 순간에만 로그를 출력합니다.
        if (next is AsyncData && (previous is AsyncLoading || previous == null)) {
          print("CardiganDetailProductScreen: 제품 데이터 로드 완료 (ref.listen)");
        }
      },
    );
    ...
    final productContent =
        ref.watch(aazCardiganDetailProdFirestoreDataProvider(widget.fullPath));
    ...
    return Scaffold(
      body: Stack(
        children: [
          CustomScrollView(
            controller: cardiganDetailProductScreenPointScrollController,
            // 스크롤 컨트롤러 연결
            slivers: <Widget>[
                      ...
                      return Column(
                        children: [
                          productContent.when(
                            data: (product) {
                              print(
                                  "CardiganDetailProductScreen: 제품 데이터 로드 완료"); // 디버깅 메시지 추가
                              return Column(
                                children: [
          ...
          // buildTopButton 함수는 주어진 context와 cardiganDetailProductScreenPointScrollController를 사용하여
          // 화면 상단으로 스크롤하기 위한 버튼 생성 위젯이며, common_body_parts_layout.dart 내에 있는 곳에서 재사용하여 구현한 부분
          buildTopButton(
              context, cardiganDetailProductScreenPointScrollController),
        ],
      ),
...
// _AazCardiganDetailProductScreenState 클래스 끝
