...
// AajCardiganMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AajCardiganMainScreen extends ConsumerStatefulWidget {
  const AajCardiganMainScreen({Key? key}) : super(key: key);

  @override
  _AajCardiganMainScreenState createState() => _AajCardiganMainScreenState();
}

// _AajCardiganMainScreenState 클래스 시작
// _AajCardiganMainScreenState 클래스는 AajCardiganMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AajCardiganMainScreenState extends ConsumerState<AajCardiganMainScreen>
  ...
  // aajCardiganMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aajCardiganMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aajCardiganMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void _updateScrollPosition() {
    ...
    // 'ref'를 사용하여 'aajCardiganMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (cardiganMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition =
            ref.read(aajCardiganMainScrollPositionProvider);
      ...
      // aajCardiganCurrentTabProvider를 통해 저장된 탭 인덱스를 읽어옴.
      int savedTabIndex = ref.read(aajCardiganCurrentTabProvider);
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(aajCardiganMainLargeBannerPageProvider.notifier).state = 0;
        ref.read(aajCardiganMainSmall1BannerPageProvider.notifier).state = 0;
        ref.read(aajCardiganMainScrollPositionProvider.notifier).state =
            0.0; // 로그아웃 시 aajCardiganMainScrollPositionProvider가 초기화되므로, 재로그인 시 초기 스크롤 위치에서 시작됨. 하지만 상품 데이터는 유지됨.
  ...
  // aajCardiganMainProductListProvider 관련 카테고리 인덱스를 받아 해당 카테고리 이름을 반환하는 함수
  ...
  Widget build(BuildContext context) {
    ...
    void onTopBarTap(int index) {
      ...
      Future.delayed(Duration.zero, () {
        ref.read(aajCardiganMainProductListProvider.notifier).reset(); // 상태 초기화
        ref.read(aajCardiganMainProductListProvider.notifier).fetchInitialProducts(
                      ...
                      child: Column(
                        children: [
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  currentPageProvider:
                                  aajCardiganMainLargeBannerPageProvider, // 큰 배너 페이지의 상태 제공자를 전달함
                                  ...
                                  aajAllLargeBannerImagesProvider, // 큰 배너 이미지의 상태 제공자를 전달함
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  ref: ref, // 상태 관리를 위한 참조 전달함
                                  currentPageProvider:
                                  aajCardiganMainSmall1BannerPageProvider, // 작은 배너 페이지의 상태 제공자를 전달함
                                  ...
                                  bannerImagesProvider:
                                  aajCardiganMainSmall1BannerImagesProvider, // 작은 배너 이미지의 상태 제공자를 전달함
                                  // 배너를 탭했을 때 실행할 함수를 전달
                                  onPageTap: (context, index) =>
                                  ...
                                  aajOnSmallBannerTap(
                                      context, // 현재 화면의 컨텍스트를 전달함
                                      index, // 클릭된 배너의 인덱스를 전달함
                                      // aajCardiganMainSmall1BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                      ref.watch(aajCardiganMainSmall1BannerImagesProvider).value ?? [],
                                      ref // Provider의 참조를 전달함
                                  ),
                          ...
                          AajPriceAndDiscountPercentSortButtons<
                            ...
                            productListProvider:
                            aajCardiganMainProductListProvider,
                            // 가디건 제품 리스트 정렬 프로바이더 전달
                            sortButtonProvider:
                            aajCardiganMainSortButtonProvider, // 가디건 정렬 버튼 프로바이더 전달
                          ), // 가격 및 할인 정렬 버튼 추가
                          ...
                          Consumer(
                            ...
                            builder: (context, ref, child) {
                              ...
                              final currentTab =
                              ref.watch(aajCardiganCurrentTabProvider);
