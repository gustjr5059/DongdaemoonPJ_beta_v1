...
// AamPolaMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AamPolaMainScreen extends ConsumerStatefulWidget {
  const AamPolaMainScreen({Key? key}) : super(key: key);

  @override
  _AamPolaMainScreenState createState() => _AamPolaMainScreenState();
}

// _AamPolaMainScreenState 클래스 시작
// _AamPolaMainScreenState 클래스는 AamPolaMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AamPolaMainScreenState extends ConsumerState<AamPolaMainScreen>
  ...
  // aamPolaMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aamPolaMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aamPolaMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  // => polaMainTopBarPointAutoScrollController는 전체 화면의 UI를 담당하는게 아니므로 scaffold의 body 내 컨트롤러에 연결이 안되어도 addListener()에 _onScroll()로 연결해놓은거라 해당 기능 사용이 가능!!
  ...
  void _updateScrollPosition() {
    // 'polaMainScreenPointScrollController'에서 현재의 스크롤 위치(offset)를 가져와서 'currentScrollPosition' 변수에 저장함.
    double currentScrollPosition = polaMainScreenPointScrollController.offset;

    // 'ref'를 사용하여 'aamPolaMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (polaMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(aamPolaMainScrollPositionProvider);
        // polaMainScreenPointScrollController.jumpTo 메서드를 사용하여 스크롤 위치를 savedScrollPosition으로 즉시 이동함.
        // 이는 스크롤 애니메이션이나 다른 복잡한 동작 없이 바로 지정된 위치로 점프함.
        polaMainScreenPointScrollController.jumpTo(savedScrollPosition);
      }

      // 저장된 탭 인덱스를 불러와 상단 탭 바의 위치를 복원.
      // aamPolaCurrentTabProvider를 통해 저장된 탭 인덱스를 읽어옴.
      int savedTabIndex = ref.read(aamPolaCurrentTabProvider);

      // 저장된 탭 인덱스가 6 이상인 경우 (탭이 끝부분에 위치한 경우),
      // 상단 탭 바를 스크롤 가능한 최대 범위까지 이동시킴.
      if (savedTabIndex >= 6) {
        double offset =
            polaMainTopBarPointAutoScrollController.position.maxScrollExtent;
        polaMainTopBarPointAutoScrollController.jumpTo(offset);
      }
      // 저장된 탭 인덱스가 1 이하인 경우 (탭이 처음 부분에 위치한 경우),
      // 상단 탭 바를 맨 앞으로 이동시킴.
      else if (savedTabIndex <= 1) {
        polaMainTopBarPointAutoScrollController.jumpTo(0.0);
      }

      // tabIndexProvider의 상태를 하단 탭 바 내 버튼과 매칭이 되면 안되므로 0~3이 아닌 -1로 매핑
      // -> 폴라티 메인 화면 초기화 시, 하단 탭 바 내 모든 버튼 비활성화
    ...
    // 사용자가 스크롤할 때마다 현재의 스크롤 위치를 polaMainScreenPointScrollController에 저장하는 코드
    // 상단 탭바 버튼 클릭 시, 해당 섹션으로 화면 이동하는 위치를 저장하는거에 해당 부분도 추가하여
    // 사용자가 앱을 종료하거나 다른 화면으로 이동한 후 돌아왔을 때 마지막으로 본 위치로 자동으로 스크롤되도록 함.
    polaMainScreenPointScrollController.addListener(_updateScrollPosition);

    // 큰 배너에 대한 PageController 및 AutoScroll 초기화
    // 'aamPolaMainLargeBannerPageProvider'에서 초기 페이지 인덱스를 읽어옴
    _largeBannerPageController =
        PageController(initialPage: ref.read(aamPolaMainLargeBannerPageProvider));

    // 큰 배너를 자동으로 스크롤하는 기능 초기화
    _largeBannerAutoScroll = BannerAutoScrollClass(
      pageController: _largeBannerPageController,
      currentPageProvider: aamPolaMainLargeBannerPageProvider,
      itemCount: bannerImageCount1, // 총 배너 이미지 개수 전달
    );

    // 작은 배너1에 대한 PageController 및 AutoScroll 초기화
    // 'aamPolaMainSmall1BannerPageProvider'에서 초기 페이지 인덱스를 읽어옴
    _small1BannerPageController =
        PageController(initialPage: ref.read(aamPolaMainSmall1BannerPageProvider));

    // 작은 배너1을 자동으로 스크롤하는 기능 초기화
    _small1BannerAutoScroll = BannerAutoScrollClass(
      pageController: _small1BannerPageController,
      currentPageProvider: aamPolaMainSmall1BannerPageProvider,
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(aamPolaMainLargeBannerPageProvider.notifier).state = 0;
        ref.read(aamPolaMainSmall1BannerPageProvider.notifier).state = 0;
        ref.read(aamPolaMainScrollPositionProvider.notifier).state =
            0.0; // 로그아웃 시 aamPolaMainScrollPositionProvider가 초기화되므로, 재로그인 시 초기 스크롤 위치에서 시작됨. 하지만 상품 데이터는 유지됨.
  ...
  // aamPolaMainProductListProvider 관련 카테고리 인덱스를 받아 해당 카테고리 이름을 반환하는 함수
  ...
  Widget build(BuildContext context) {
    ...
    void onTopBarTap(int index) {
      ref.read(aamPolaCurrentTabProvider.notifier).state = index; // 현재 탭 인덱스를 업데이트
      ...
      Future.delayed(Duration.zero, () {
        ref.read(aamPolaMainProductListProvider.notifier).reset(); // 상태 초기화
        ref.read(aamPolaMainProductListProvider.notifier).fetchInitialProducts(
    ...
    Widget topBarList = buildTopBarList(context, onTopBarTap,
        aamPolaCurrentTabProvider, polaMainTopBarPointAutoScrollController);
    ...
    // 대배너 부분 관련 수치
    final double polaMainScreenLargeBannerWidth = screenSize.width * (393 / referenceWidth); // 대배너 이미지 너비
    final double polaMainScreenLargeBannerHeight = screenSize.width * (378 / referenceWidth); // 대배너 이미지 높이
    final double polaMainLargeBannerViewHeight = screenSize.width * (378 / referenceWidth); // 대배너 화면 세로 비율

    // 소배너 부분 관련 수치
    final double polaMainScreenSmallBannerWidth = screenSize.width * (361 / referenceWidth); // 소배너 이미지 너비
    final double polaMainScreenSmallBannerHeight = screenSize.width * (90 / referenceWidth); // 소배너 이미지 높이
    final double polaMainScreenSmallBannerViewHeight = screenSize.width * (90 / referenceWidth); // 소배너 화면 세로 비율
    ...
    return Scaffold(
      body: Stack(
        children: [
          CustomScrollView(
            controller: polaMainScreenPointScrollController, // 스크롤 컨트롤러 연결
            slivers: <Widget>[
                      ...
                      child: Column(
                        children: [
                          ...
                          CommonCardView(
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                height: polaMainLargeBannerViewHeight,
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  currentPageProvider:
                                  aamPolaMainLargeBannerPageProvider, // 큰 배너 페이지의 상태 제공자를 전달함
                                  ...
                                  width: polaMainScreenLargeBannerWidth, // 배너 섹션의 너비를 설정함
                                  height: polaMainScreenLargeBannerHeight, // 배너 섹션의 높이를 설정함
                          ...
                          CommonCardView(
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                height: polaMainScreenSmallBannerViewHeight,
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  ref: ref, // 상태 관리를 위한 참조 전달함
                                  currentPageProvider:
                                  aamPolaMainSmall1BannerPageProvider, // 작은 배너 페이지의 상태 제공자를 전달함
                                  ...
                                  bannerImagesProvider:
                                  aamPolaMainSmall1BannerImagesProvider, // 작은 배너 이미지의 상태 제공자를 전달함
                                  // 배너를 탭했을 때 실행할 함수를 전달
                                  onPageTap: (context, index) =>
                                  ...
                                  aamOnSmallBannerTap(
                                      context, // 현재 화면의 컨텍스트를 전달함
                                      index, // 클릭된 배너의 인덱스를 전달함
                                      // aamPolaMainSmall1BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                      ref.watch(aamPolaMainSmall1BannerImagesProvider).value ?? [],
                                      ref // Provider의 참조를 전달함
                                  ),
                                  width: polaMainScreenSmallBannerWidth, // 작은 배너 섹션의 너비를 설정함
                                  height: polaMainScreenSmallBannerHeight, // 작은 배너 섹션의 높이를 설정함
                          ...
                          AamPriceAndDiscountPercentSortButtons<
                            ...
                            productListProvider: aamPolaMainProductListProvider,
                            // 폴라티 제품 리스트 프로바이더 전달
                            sortButtonProvider:
                            aamPolaMainSortButtonProvider, // 폴라티 정렬 버튼 프로바이더 전달
                          ), // 가격 및 할인 정렬 버튼 추가
                          ...
                          Consumer(
                            ...
                            builder: (context, ref, child) {
                              ...
                              final currentTab =
                              ref.watch(aamPolaCurrentTabProvider);
                              // 현재 탭: aamPolaCurrentTabProvider를 구독하고 현재 선택된 탭 정보를 가져옴.
                              final productListProvider =
                                  aamPolaMainProductListProvider;
                              // 제품 리스트 제공자: aamPolaMainProductListProvider를 productListProvider 변수에 할당.
                              return AamGeneralProductList<
                                  AamProductMainListNotifier>(
                                // AamGeneralProductList 반환: AamGeneralProductList 위젯을 반환하여 화면에 제품 목록을 표시.
                                scrollController:
                                polaMainScreenPointScrollController,
                                // 스크롤 컨트롤러: polaMainScreenPointScrollController를 GeneralProductList의 scrollController로 전달.
          ...
          // buildTopButton 함수는 주어진 context와 polaMainScreenPointScrollController를 사용하여
          // 화면 상단으로 스크롤하기 위한 버튼 생성 위젯이며, common_body_parts_layout.dart 내에 있는 곳에서 재사용하여 구현한 부분
          buildTopButton(context, polaMainScreenPointScrollController),
        ],
      ),
      // 하단 탭 바 - 1번 케이스인 '홈','장바구니', '발주내역', '마이페이지' 버튼이 UI로 구현됨.
      bottomNavigationBar: buildCommonBottomNavigationBar(
          ref.watch(tabIndexProvider), ref, context, 5, 1, scrollController: polaMainScreenPointScrollController),
