...
// AamCoatMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AamCoatMainScreen extends ConsumerStatefulWidget {
  const AamCoatMainScreen({Key? key}) : super(key: key);

  @override
  _AamCoatMainScreenState createState() => _AamCoatMainScreenState();
}

// _AamCoatMainScreenState 클래스 시작
// _AamCoatMainScreenState 클래스는 AamCoatMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AamCoatMainScreenState extends ConsumerState<AamCoatMainScreen>
  ...
  // aamCoatMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aamCoatMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aamCoatMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void _updateScrollPosition() {
    ...
    // 'ref'를 사용하여 'aamCoatMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (coatMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(aamCoatMainScrollPositionProvider);
      ...
      // aamCoatCurrentTabProvider를 통해 저장된 탭 인덱스를 읽어옴.
      int savedTabIndex = ref.read(aamCoatCurrentTabProvider);
    ...
    // 'aamCoatMainLargeBannerPageProvider'에서 초기 페이지 인덱스를 읽어옴
    _largeBannerPageController =
        PageController(initialPage: ref.read(aamCoatMainLargeBannerPageProvider));

    // 큰 배너를 자동으로 스크롤하는 기능 초기화
    _largeBannerAutoScroll = BannerAutoScrollClass(
      pageController: _largeBannerPageController,
      currentPageProvider: aamCoatMainLargeBannerPageProvider,
      itemCount: bannerImageCount1, // 총 배너 이미지 개수 전달
    );

    // 작은 배너1에 대한 PageController 및 AutoScroll 초기화
    // 'aamCoatMainSmall1BannerPageProvider'에서 초기 페이지 인덱스를 읽어옴
    _small1BannerPageController =
        PageController(initialPage: ref.read(aamCoatMainSmall1BannerPageProvider));

    // 작은 배너1을 자동으로 스크롤하는 기능 초기화
    _small1BannerAutoScroll = BannerAutoScrollClass(
      pageController: _small1BannerPageController,
      currentPageProvider: aamCoatMainSmall1BannerPageProvider,
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(aamCoatMainLargeBannerPageProvider.notifier).state = 0;
        ref.read(aamCoatMainSmall1BannerPageProvider.notifier).state = 0;
        ref.read(aamCoatMainScrollPositionProvider.notifier).state =
            0.0; // 로그아웃 시 aamCoatMainScrollPositionProvider가 초기화되므로, 재로그인 시 초기 스크롤 위치에서 시작됨. 하지만 상품 데이터는 유지됨.
  ...
  // aamCoatMainProductListProvider 관련 카테고리 인덱스를 받아 해당 카테고리 이름을 반환하는 함수
  ...
  Widget build(BuildContext context) {
    ...
    void onTopBarTap(int index) {
      ref.read(aamCoatCurrentTabProvider.notifier).state = index; // 현재 탭 인덱스를 업데이트
      ...
      Future.delayed(Duration.zero, () {
        ref.read(aamCoatMainProductListProvider.notifier).reset(); // 상태 초기화
        ref.read(aamCoatMainProductListProvider.notifier).fetchInitialProducts(
    ...
    Widget topBarList = buildTopBarList(context, onTopBarTap,
        aamCoatCurrentTabProvider, coatMainTopBarPointAutoScrollController);
                      ...
                      child: Column(
                        children: [
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  currentPageProvider:
                                  aamCoatMainLargeBannerPageProvider, // 큰 배너 페이지의 상태 제공자를 전달함
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  ref: ref, // 상태 관리를 위한 참조를 전달함
                                  currentPageProvider:
                                  aamCoatMainSmall1BannerPageProvider, // 작은 배너 페이지의 상태 제공자를 전달함
                                  ...
                                  bannerImagesProvider:
                                  aamCoatMainSmall1BannerImagesProvider, // 작은 배너 이미지의 상태 제공자를 전달함
                                  // 배너를 탭했을 때 실행할 함수를 전달
                                  onPageTap: (context, index) =>
                                  ...
                                  aamOnSmallBannerTap(
                                      context, // 현재 화면의 컨텍스트를 전달함
                                      index, // 클릭된 배너의 인덱스를 전달함
                                      // aamCoatMainSmall1BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                      ref.watch(aamCoatMainSmall1BannerImagesProvider).value ?? [],
                                      ref // Provider의 참조를 전달함
                                  ),
                          ...
                          AamPriceAndDiscountPercentSortButtons<
                            ...
                            productListProvider: aamCoatMainProductListProvider,
                            // 코트 제품 리스트 정렬 프로바이더 전달
                            sortButtonProvider:
                            aamCoatMainSortButtonProvider, // 코트 정렬 버튼 프로바이더 전달
                          ), // 가격 및 할인 정렬 버튼 추가
                          ...
                          Consumer(
                            ...
                            builder: (context, ref, child) {
                              ...
                              final currentTab =
                              ref.watch(aamCoatCurrentTabProvider);
                              // 현재 탭: aamCoatCurrentTabProvider를 구독하고 현재 선택된 탭 정보를 가져옴.
                              final productListProvider =
                                  aamCoatMainProductListProvider;
                              // 제품 리스트 제공자: aamCoatMainProductListProvider를 productListProvider 변수에 할당.
