...
// AamSkirtDetailProductScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AamSkirtDetailProductScreen extends ConsumerStatefulWidget {
  final String fullPath; // 전체 경로를 나타내는 문자열 변수
  final String title; // 타이틀을 나타내는 문자열 변수

  const AamSkirtDetailProductScreen({
  ...
  @override
  _AamSkirtDetailProductScreenState createState() =>
      _AamSkirtDetailProductScreenState();
}

// _AamSkirtDetailProductScreenState 클래스 시작
// _AamSkirtDetailProductScreenState 클래스는 AamSkirtDetailProductScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AamSkirtDetailProductScreenState
    extends ConsumerState<AamSkirtDetailProductScreen>
    with WidgetsBindingObserver, RouteAware {
  // 사용자 인증 상태 변경을 감지하는 스트림 구독 객체임.
  // 이를 통해 사용자 로그인 또는 로그아웃 상태 변경을 실시간으로 감지하고 처리할 수 있음.
  StreamSubscription<User?>? authStateChangesSubscription;

  // aamSkirtDetailScrollPositionProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aamSkirtDetailScrollPositionProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aamSkirtDetailScrollPositionProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void initState() {
    ...
    print(
        "AamSkirtDetailProductScreen: 제품 경로에 대한 화면 로드 중: ${widget.fullPath}"); // 디버깅 메시지 추가
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (skirtDetailProductScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition =
            ref.read(aamSkirtDetailScrollPositionProvider);
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(aamSkirtDetailScrollPositionProvider.notifier).state =
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<AsyncValue<ProductContent>>(
      aamSkirtDetailProdFirestoreDataProvider(widget.fullPath),
          (previous, next) {
    ...
    final productContent =
        ref.watch(aamSkirtDetailProdFirestoreDataProvider(widget.fullPath));
...
// _AamSkirtDetailProductScreenState 클래스 끝
