...
class _AadCardiganMainScreenState extends ConsumerState<AadCardiganMainScreen>
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.resumed) {
      ...
      if (savedTabIndex >= 6) {
        double offset =
            cardiganMainTopBarPointAutoScrollController.position.maxScrollExtent;
        cardiganMainTopBarPointAutoScrollController.jumpTo(offset);
      }
      // 저장된 탭 인덱스가 1 이하인 경우 (탭이 처음 부분에 위치한 경우),
      // 상단 탭 바를 맨 앞으로 이동시킴.
  ...
  Widget build(BuildContext context) {
    ...
    final largeBannerWidget = buildCommonBannerPageViewSection<AllLargeBannerImage>(
      ...
      width: cardiganMainScreenLargeBannerWidth,
      height: cardiganMainScreenLargeBannerHeight,
      borderRadius: 0,
    );
    ...
    final small1BannerWidget = buildCommonBannerPageViewSection<AllSmallBannerImage>(
      ...
      bannerImagesProvider: aadCardiganMainSmall1BannerImagesProvider,
      onPageTap: (context, index) =>
          onSmallBannerTap(context, index, ref.watch(aadCardiganMainSmall1BannerImagesProvider).value ?? [], ref),
      width: cardiganMainScreenSmallBannerWidth,
      height: cardiganMainScreenSmallBannerHeight,
      borderRadius: 5,
    );
