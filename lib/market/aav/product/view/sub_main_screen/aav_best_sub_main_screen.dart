...
// AavBestSubMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AavBestSubMainScreen extends ConsumerStatefulWidget {
  const AavBestSubMainScreen({Key? key}) : super(key: key);

  @override
  _AavBestSubMainScreenState createState() => _AavBestSubMainScreenState();
}

// _AavBestSubMainScreenState 클래스 시작
// _AavBestSubMainScreenState 클래스는 AavBestSubMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AavBestSubMainScreenState extends ConsumerState<AavBestSubMainScreen>
  ...
  // aavBestSubMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aavBestSubMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aavBestSubMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void _updateScrollPosition() {
    ...
    // 'ref'를 사용하여 'aavBestSubMainScrollPositionProvider'의 notifier를 읽어옴.
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (bestSubMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition =
            ref.read(aavBestSubMainScrollPositionProvider);
  ...
  Widget build(BuildContext context) {
    ...
    final small1BannerWidget = buildCommonBannerPageViewSection<AllSmallBannerImage>(
      ...
      bannerImagesProvider: aavBestSubMainSmall1BannerImagesProvider,
      onPageTap: (context, index) =>
          onSmallBannerTap(context, index, ref.watch(aavBestSubMainSmall1BannerImagesProvider).value ?? [], ref),
                        ...
                        child: Column(
                          children: [
                            ...
                            AavPriceAndDiscountPercentSortButtons<
                              ...
                              productListProvider:
                              aavBestSubMainProductListProvider,
                              // 스테디 셀러 제품 리스트 프로바이더 전달
                              sortButtonProvider:
                              aavBestSubMainSortButtonProvider, // 스테디 셀러 정렬 버튼 프로바이더 전달
                            ),
                            // 가격 및 할인 정렬 버튼 추가
                            SizedBox(height: interval1Y),
                            Container(
