...
// AavCardiganMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AavCardiganMainScreen extends ConsumerStatefulWidget {
  const AavCardiganMainScreen({Key? key}) : super(key: key);

  @override
  _AavCardiganMainScreenState createState() => _AavCardiganMainScreenState();
}

// _AavCardiganMainScreenState 클래스 시작
// _AavCardiganMainScreenState 클래스는 AavCardiganMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AavCardiganMainScreenState extends ConsumerState<AavCardiganMainScreen>
  ...
  // aavCardiganMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aavCardiganMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aavCardiganMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void _updateScrollPosition() {
    ...
    // 'ref'를 사용하여 'aavCardiganMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (cardiganMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition =
            ref.read(aavCardiganMainScrollPositionProvider);
      ...
      // aavCardiganCurrentTabProvider를 통해 저장된 탭 인덱스를 읽어옴.
      int savedTabIndex = ref.read(aavCardiganCurrentTabProvider);
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(aavCardiganMainLargeBannerPageProvider.notifier).state = 0;
        ref.read(aavCardiganMainSmall1BannerPageProvider.notifier).state = 0;
        ref.read(aavCardiganMainScrollPositionProvider.notifier).state =
            0.0; // 로그아웃 시 aavCardiganMainScrollPositionProvider가 초기화되므로, 재로그인 시 초기 스크롤 위치에서 시작됨. 하지만 상품 데이터는 유지됨.
  ...
  // aavCardiganMainProductListProvider 관련 카테고리 인덱스를 받아 해당 카테고리 이름을 반환하는 함수
  ...
  Widget build(BuildContext context) {
    ...
    void onTopBarTap(int index) {
      ...
      Future.delayed(Duration.zero, () {
        ref.read(aavCardiganMainProductListProvider.notifier).reset(); // 상태 초기화
        ref.read(aavCardiganMainProductListProvider.notifier).fetchInitialProducts(
    ...
    final small1BannerWidget = buildCommonBannerPageViewSection<AllSmallBannerImage>(
      ...
      bannerImagesProvider: aavCardiganMainSmall1BannerImagesProvider,
      onPageTap: (context, index) =>
          onSmallBannerTap(context, index, ref.watch(aavCardiganMainSmall1BannerImagesProvider).value ?? [], ref),
                      ...
                      child: Column(
                        children: [
                          ...
                          AavPriceAndDiscountPercentSortButtons<
                            ...
                            productListProvider:
                            aavCardiganMainProductListProvider,
                            // 가디건 제품 리스트 정렬 프로바이더 전달
                            sortButtonProvider:
                            aavCardiganMainSortButtonProvider, // 가디건 정렬 버튼 프로바이더 전달
                          ), // 가격 및 할인 정렬 버튼 추가
                          ...
                          Consumer(
                            ...
                            builder: (context, ref, child) {
                              ...
                              final currentTab =
                              ref.watch(aavCardiganCurrentTabProvider);
