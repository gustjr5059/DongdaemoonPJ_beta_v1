...
// AaqBlouseMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AaqBlouseMainScreen extends ConsumerStatefulWidget {
  const AaqBlouseMainScreen({Key? key}) : super(key: key);

  @override
  _AaqBlouseMainScreenState createState() => _AaqBlouseMainScreenState();
}

// _AaqBlouseMainScreenState 클래스 시작
// _AaqBlouseMainScreenState 클래스는 AaqBlouseMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AaqBlouseMainScreenState extends ConsumerState<AaqBlouseMainScreen>
  ...
  // aaqBlouseMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aaqBlouseMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aaqBlouseMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void _updateScrollPosition() {
    ...
    // 'ref'를 사용하여 'aaqBlouseMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (blouseMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(aaqBlouseMainScrollPositionProvider);
        // aaqBlouseMainScrollPositionProvider.jumpTo 메서드를 사용하여 스크롤 위치를 savedScrollPosition으로 즉시 이동함.
      ...
      // aaqBlouseCurrentTabProvider를 통해 저장된 탭 인덱스를 읽어옴.
      int savedTabIndex = ref.read(aaqBlouseCurrentTabProvider);
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(aaqBlouseMainLargeBannerPageProvider.notifier).state = 0;
        ref.read(aaqBlouseMainSmall1BannerPageProvider.notifier).state = 0;
        ref.read(aaqBlouseMainScrollPositionProvider.notifier).state =
  ...
  // aaqBlouseMainProductListProvider 관련 카테고리 인덱스를 받아 해당 카테고리 이름을 반환하는 함수
  ...
  Widget build(BuildContext context) {
    ...
    void onTopBarTap(int index) {
      ...
      Future.delayed(Duration.zero, () {
        ref.read(aaqBlouseMainProductListProvider.notifier).reset(); // 상태 초기화
        ref.read(aaqBlouseMainProductListProvider.notifier).fetchInitialProducts(
    ...
    Widget topBarList = buildTopBarList(context, onTopBarTap,
        aaqBlouseCurrentTabProvider, blouseMainTopBarPointAutoScrollController);
                      ...
                      child: Column(
                        children: [
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  currentPageProvider:
                                  aaqBlouseMainLargeBannerPageProvider, // 큰 배너 페이지의 상태 제공자를 전달함
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  ref: ref, // 상태 관리를 위한 참조를 전달함
                                  currentPageProvider:
                                  aaqBlouseMainSmall1BannerPageProvider, // 작은 배너 페이지의 상태 제공자를 전달함
                                  ...
                                  bannerImagesProvider:
                                  aaqBlouseMainSmall1BannerImagesProvider, // 작은 배너 이미지의 상태 제공자를 전달함
                                  // 배너를 탭했을 때 실행할 함수를 전달
                                  onPageTap: (context, index) =>
                                  ...
                                  aaqOnSmallBannerTap(
                                      context, // 현재 화면의 컨텍스트를 전달함
                                      index, // 클릭된 배너의 인덱스를 전달함
                                      // aaqBlouseMainSmall1BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                      ref.watch(aaqBlouseMainSmall1BannerImagesProvider).value ?? [],
                                      ref // Provider의 참조를 전달함
                                  ),
                          ...
                          AaqPriceAndDiscountPercentSortButtons<
                            ...
                            productListProvider:
                            aaqBlouseMainProductListProvider,
                            // 블라우스 제품 리스트 프로바이더 전달
                            sortButtonProvider:
                            aaqBlouseMainSortButtonProvider, // 블라우스 정렬 버튼 프로바이더 전달
                          ), // 가격 및 할인 정렬 버튼 추가
                          ...
                          Consumer(
                            ...
                            builder: (context, ref, child) {
                              ...
                              final currentTab =
                              ref.watch(aaqBlouseCurrentTabProvider);
                              // 현재 탭: aaqBlouseCurrentTabProvider를 구독하고 현재 선택된 탭 정보를 가져옴.
...
// _AaqBlouseMainScreenState 클래스 끝
