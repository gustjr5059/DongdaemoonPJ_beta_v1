...
import '../../common/layout/aag_common_body_parts_layout.dart';
...
import '../layout/aag_home_body_parts_layout.dart';
...
import '../provider/aag_home_all_providers.dart';

// 홈 화면의 상태를 관리하기 위한 Provider 파일을 임포트합니다.
import '../provider/aag_home_state_provider.dart';
...
// AagHomeMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AagHomeMainScreen extends ConsumerStatefulWidget {
  const AagHomeMainScreen({Key? key}) : super(key: key);

  @override
  _AagHomeMainScreenState createState() => _AagHomeMainScreenState();
}

// _AagHomeMainScreenState 클래스 시작
// _AagHomeMainScreenState 클래스는 AagHomeMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AagHomeMainScreenState extends ConsumerState<AagHomeMainScreen>
  ...
  void _onScroll() {
    ...
    if (newCurrentIndex != currentIndex) {
      ...
      ref.read(aagHomeCurrentTabProvider.notifier).state = currentIndex; // 상태 업데이트
    }
  ...
  void _updateScrollPosition() {
    ...
    // 'ref'를 사용하여 'aagHomeScrollPositionProvider'의 notifier를 읽어옴.
    // 읽어온 notifier의 'state' 값을 'currentScrollPosition'으로 설정함.
    // 이렇게 하면 앱의 다른 부분에서 해당 스크롤 위치 정보를 참조할 수 있게 됨.
    ref.read(aagHomeScrollPositionProvider.notifier).state = currentScrollPosition;
  }
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (homeScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = FirebaseAuth.instance.currentUser != null
            ? ref.read(aagHomeScrollPositionProvider)
            : ref.read(aagHomeLoginAndLogoutScrollPositionProvider);
        // 저장된 위치로 스크롤 이동
      ...
      currentIndex = ref.read(aagHomeCurrentTabProvider);
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(aagHomeLargeBannerPageProvider.notifier).state = 0;
        ref.read(aagHomeSmall1BannerPageProvider.notifier).state = 0;
        ref.read(aagHomeSmall2BannerPageProvider.notifier).state = 0;
        ref.read(aagHomeSmall3BannerPageProvider.notifier).state = 0;
  ...
  Widget build(BuildContext context) {
    ...
    Widget topBarList = buildTopBarList(context, onTopBarTap,
        aagHomeCurrentTabProvider, homeTopBarPointAutoScrollController);
                        ...
                        children: [
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  currentPageProvider:
                                  aagHomeLargeBannerPageProvider,
                          ...
                          CommonCardView(
                            ...
                            content: Container(
                              child: MidCategoryButtonList(
                                onCategoryTap: aagOnMidCategoryTap,
                              ),
                              decoration: BoxDecoration(
                                border: Border(
                                  bottom: BorderSide(color: BLACK_COLOR, width: 1.0), // 하단 테두리 색상을 설정함
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  // 현재 페이지를 관리하는 Provider를 전달
                                  currentPageProvider:
                                  aagHomeSmall1BannerPageProvider,
                                  ...
                                  // 배너 이미지들을 관리하는 Provider를 전달
                                  bannerImagesProvider:
                                  aagHomeSmall1BannerImagesProvider,
                                  // 배너를 탭했을 때 실행할 함수를 전달
                                  onPageTap: (context, index) =>
                                  ...
                                  aagOnSmallBannerTap(
                                      context, // 현재 화면의 컨텍스트를 전달함
                                      index, // 클릭된 배너의 인덱스를 전달함
                                      // aagHomeSmall1BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                      ref.watch(aagHomeSmall1BannerImagesProvider).value ?? [],
                                      ref // Provider의 참조를 전달함
                                  ),
                          ...
                          Container(
                            ...
                            child: buildSectionCard(
                                context, ref, "신상", aagBuildNewProductsSection,
                          ...
                          Container(
                            key: sectionSaleKey,
                            child: buildSectionCard(
                                context, ref, "특가 상품", aagBuildSaleProductsSection,
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  // 현재 페이지를 관리하는 Provider를 전달
                                  currentPageProvider:
                                  aagHomeSmall2BannerPageProvider,
                                  // 페이지 전환을 관리하는 페이지 컨트롤러를 전달
                                  pageController: _small2BannerPageController,
                                  // 배너 자동 스크롤 기능을 전달
                                  ...
                                  // 배너 이미지들을 관리하는 Provider를 전달
                                  bannerImagesProvider:
                                  aagHomeSmall2BannerImagesProvider,
                                  // 배너를 탭했을 때 실행할 함수를 전달
                                  onPageTap: (context, index) =>
                                  // 소배너 클릭 시 호출할 함수 aagOnSmallBannerTap 실행
                                  aagOnSmallBannerTap(
                                      context, // 현재 화면의 컨텍스트를 전달함
                                      index, // 클릭된 배너의 인덱스를 전달함
                                      // aagHomeSmall2BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                      ref.watch(aagHomeSmall2BannerImagesProvider).value ?? [],
                                      ref // Provider의 참조를 전달함
                                  ),
                                  width: homeScreenSmallBannerWidth, // 원하는 너비
                                  height: homeScreenSmallBannerHeight, // 원하는 높이
                          ...
                          Container(
                            ...
                            child: buildSectionCard(
                                context, ref, "봄", aagBuildSpringProductsSection,
                          ...
                          Container(
                            ...
                            child: buildSectionCard(
                                context, ref, "여름", aagBuildSummerProductsSection,
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  // 현재 페이지를 관리하는 Provider를 전달
                                  currentPageProvider:
                                  aagHomeSmall3BannerPageProvider,
                                  // 페이지 전환을 관리하는 페이지 컨트롤러를 전달
                                  pageController: _small3BannerPageController,
                                  // 배너 자동 스크롤 기능을 전달
                                  ...
                                  // 배너 이미지들을 관리하는 Provider를 전달
                                  bannerImagesProvider:
                                  aagHomeSmall3BannerImagesProvider,
                                  // 배너를 탭했을 때 실행할 함수를 전달
                                  onPageTap: (context, index) =>
                                  // 소배너 클릭 시 호출할 함수 aagOnSmallBannerTap 실행
                                  aagOnSmallBannerTap(
                                      context, // 현재 화면의 컨텍스트를 전달함
                                      index, // 클릭된 배너의 인덱스를 전달함
                                      // aagHomeSmall3BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                      ref.watch(aagHomeSmall3BannerImagesProvider).value ?? [],
                                      ref // Provider의 참조를 전달함
                                  ),
                                  width: homeScreenSmallBannerWidth, // 원하는 너비
                                  height: homeScreenSmallBannerHeight, // 원하는 높이
                          ...
                          Container(
                            ...
                            child: buildSectionCard(
                                context, ref, "가을", aagBuildAutumnProductsSection,
                          ...
                          Container(
                            ...
                            child: buildSectionCard(
                                context, ref, "겨울", aagBuildWinterProductsSection,
                          ...
                          Container(
                            ...
                            child: buildSectionCard(context, ref, "이벤트",
                                aagBuildEventPosterImgProductsSection,
