...
import '../../../common/layout/aak_common_body_parts_layout.dart'; // 공통 UI 컴포넌트 파일

// 제품 상태 관리를 위해 사용되는 상태 제공자 파일을 임포트합니다.
// 이 파일은 제품 관련 데이터의 상태를 관리하고, 필요에 따라 상태를 업데이트하는 로직을 포함합니다.
import '../../../common/provider/aak_common_all_provider.dart';
import '../../../home/provider/aak_home_all_providers.dart';
import '../../layout/aak_product_body_parts_layout.dart';
import '../../provider/aak_product_state_provider.dart';
...
import '../../provider/aak_product_all_providers.dart';
...
// AakNeatMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AakNeatMainScreen extends ConsumerStatefulWidget {
  const AakNeatMainScreen({Key? key}) : super(key: key);

  @override
  _AakNeatMainScreenState createState() => _AakNeatMainScreenState();
}

// _AakNeatMainScreenState 클래스 시작
// _AakNeatMainScreenState 클래스는 AakNeatMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AakNeatMainScreenState extends ConsumerState<AakNeatMainScreen>
  ...
  // aakNeatMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aakNeatMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aakNeatMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void _updateScrollPosition() {
    ...
    // 'ref'를 사용하여 'aakNeatMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (neatMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(aakNeatMainScrollPositionProvider);
      ...
      // aakNeatCurrentTabProvider를 통해 저장된 탭 인덱스를 읽어옴.
      int savedTabIndex = ref.read(aakNeatCurrentTabProvider);
    ...
    // 'aakNeatMainLargeBannerPageProvider'에서 초기 페이지 인덱스를 읽어옴
    _largeBannerPageController =
        PageController(initialPage: ref.read(aakNeatMainLargeBannerPageProvider));

    // 큰 배너를 자동으로 스크롤하는 기능 초기화
    _largeBannerAutoScroll = BannerAutoScrollClass(
      pageController: _largeBannerPageController,
      currentPageProvider: aakNeatMainLargeBannerPageProvider,
      itemCount: bannerImageCount1, // 총 배너 이미지 개수 전달
    );

    // 작은 배너1에 대한 PageController 및 AutoScroll 초기화
    // 'aakNeatMainSmall1BannerPageProvider'에서 초기 페이지 인덱스를 읽어옴
    _small1BannerPageController =
        PageController(initialPage: ref.read(aakNeatMainSmall1BannerPageProvider));

    // 작은 배너1을 자동으로 스크롤하는 기능 초기화
    _small1BannerAutoScroll = BannerAutoScrollClass(
      pageController: _small1BannerPageController,
      currentPageProvider: aakNeatMainSmall1BannerPageProvider,
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(aakNeatMainLargeBannerPageProvider.notifier).state = 0;
        ref.read(aakNeatMainSmall1BannerPageProvider.notifier).state = 0;
        ref.read(aakNeatMainScrollPositionProvider.notifier).state =
            0.0; // 로그아웃 시 aakNeatMainScrollPositionProvider가 초기화되므로, 재로그인 시 초기 스크롤 위치에서 시작됨. 하지만 상품 데이터는 유지됨.
  ...
  // aakNeatMainProductListProvider 관련 카테고리 인덱스를 받아 해당 카테고리 이름을 반환하는 함수
  ...
  Widget build(BuildContext context) {
    ...
    void onTopBarTap(int index) {
      ref.read(aakNeatCurrentTabProvider.notifier).state = index; // 현재 탭 인덱스를 업데이트
      ...
      Future.delayed(Duration.zero, () {
        ref.read(aakNeatMainProductListProvider.notifier).reset(); // 상태 초기화
        ref.read(aakNeatMainProductListProvider.notifier).fetchInitialProducts(
    ...
    Widget topBarList = buildTopBarList(context, onTopBarTap,
        aakNeatCurrentTabProvider, neatMainTopBarPointAutoScrollController);
                    ...
                    return Padding(
                      ...
                      child: Column(
                        children: [
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  currentPageProvider:
                                  aakNeatMainLargeBannerPageProvider, // 큰 배너 페이지의 상태 제공자를 전달함
                                  ...
                                  aakAllLargeBannerImagesProvider, // 배너 이미지의 상태 제공자를 전달함
                                  // 배너를 탭했을 때 실행할 함수를 전달
                                  onPageTap: (context, index) =>
                                  // 대배너 클릭 시 호출할 함수 aakOnLargeBannerTap 실행
                                  aakOnLargeBannerTap(
                                      context, // 현재 화면의 컨텍스트를 전달함
                                      index, // 클릭된 배너의 인덱스를 전달함
                                      // aakAllLargeBannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                      ref.watch(aakAllLargeBannerImagesProvider).value ?? [],
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  ref: ref, // 상태 관리를 위한 참조 전달함
                                  currentPageProvider:
                                  aakNeatMainSmall1BannerPageProvider, // 작은 배너 페이지의 상태 제공자를 전달함
                                  ...
                                  bannerImagesProvider:
                                  aakNeatMainSmall1BannerImagesProvider, // 작은 배너 이미지의 상태 제공자를 전달함
                                  // 배너를 탭했을 때 실행할 함수를 전달
                                  onPageTap: (context, index) =>
                                  // 소배너 클릭 시 호출할 함수 aakOnSmallBannerTap 실행
                                  aakOnSmallBannerTap(
                                      context, // 현재 화면의 컨텍스트를 전달함
                                      index, // 클릭된 배너의 인덱스를 전달함
                                      // aakNeatMainSmall1BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                      ref.watch(aakNeatMainSmall1BannerImagesProvider).value ?? [],
                                      ref // Provider의 참조를 전달함
                                  ),
                          ...
                          AakPriceAndDiscountPercentSortButtons<
                              AakProductMainListNotifier>(
                            productListProvider: aakNeatMainProductListProvider,
                            // 니트 제품 리스트 정렬 프로바이더 전달
                            sortButtonProvider:
                            aakNeatMainSortButtonProvider, // 니트 정렬 버튼 프로바이더 전달
                          ), // 가격 및 할인 정렬 버튼 추가
                          ...
                          Consumer(
                            ...
                            builder: (context, ref, child) {
                              ...
                              final currentTab =
                              ref.watch(aakNeatCurrentTabProvider);
                              // 현재 탭: aakNeatMainProductListProvider를 구독하고 현재 선택된 탭 정보를 가져옴.
                              final productListProvider =
                                  aakNeatMainProductListProvider;
                              // 제품 리스트 제공자: aakNeatMainProductListProvider를 productListProvider 변수에 할당.
                              return AakGeneralProductList<
                                  AakProductMainListNotifier>(
                                // AakGeneralProductList 반환: AakGeneralProductList 위젯을 반환하여 화면에 제품 목록을 표시.
                                scrollController:
...
// _AakCardiganMainScreenState 클래스 끝
