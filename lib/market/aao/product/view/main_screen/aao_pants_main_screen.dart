...
// AaoPantsMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AaoPantsMainScreen extends ConsumerStatefulWidget {
  const AaoPantsMainScreen({Key? key}) : super(key: key);

  @override
  _AaoPantsMainScreenState createState() => _AaoPantsMainScreenState();
}

// _AaoPantsMainScreenState 클래스 시작
// _AaoPantsMainScreenState 클래스는 AaoPantsMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AaoPantsMainScreenState extends ConsumerState<AaoPantsMainScreen>
  ...
  // aaoPantsMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aaoPantsMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aaoPantsMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void _updateScrollPosition() {
    ...
    // 'ref'를 사용하여 'aaoPantsMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (pantsMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(aaoPantsMainScrollPositionProvider);
      ...
      // aaoPantsCurrentTabProvider를 통해 저장된 탭 인덱스를 읽어옴.
      int savedTabIndex = ref.read(aaoPantsCurrentTabProvider);
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(aaoPantsMainLargeBannerPageProvider.notifier).state = 0;
        ref.read(aaoPantsMainSmall1BannerPageProvider.notifier).state = 0;
        ref.read(aaoPantsMainScrollPositionProvider.notifier).state =
            0.0; // 로그아웃 시 aaoPantsMainScrollPositionProvider가 초기화되므로, 재로그인 시 초기 스크롤 위치에서 시작됨. 하지만 상품 데이터는 유지됨.
  ...
  // aaoPantsMainProductListProvider 관련 카테고리 인덱스를 받아 해당 카테고리 이름을 반환하는 함수
  ...
  Widget build(BuildContext context) {
    ...
    void onTopBarTap(int index) {
      ...
      Future.delayed(Duration.zero, () {
        ref.read(aaoPantsMainProductListProvider.notifier).reset(); // 상태 초기화
        ref.read(aaoPantsMainProductListProvider.notifier).fetchInitialProducts(
    ...
    Widget topBarList = buildTopBarList(context, onTopBarTap,
        aaoPantsCurrentTabProvider, pantsMainTopBarPointAutoScrollController);
                      ...
                      child: Column(
                        children: [
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  currentPageProvider:
                                  aaoPantsMainLargeBannerPageProvider, // 큰 배너 페이지의 상태 제공자를 전달함
                                  ...
                                  aaoOnLargeBannerTap(
                                      ...
                                      // aaoAllLargeBannerImagesProvider 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  ref: ref, // 상태 관리를 위한 참조 전달함
                                  currentPageProvider:
                                  aaoPantsMainSmall1BannerPageProvider, // 작은 배너 페이지의 상태 제공자를 전달함
                                  ...
                                  bannerImagesProvider:
                                  aaoPantsMainSmall1BannerImagesProvider, // 작은 배너 이미지의 상태 제공자를 전달함
                                  // 배너를 탭했을 때 실행할 함수를 전달
                                  onPageTap: (context, index) =>
                                  ...
                                  aaoOnSmallBannerTap(
                                      context, // 현재 화면의 컨텍스트를 전달함
                                      index, // 클릭된 배너의 인덱스를 전달함
                                      // aaoPantsMainSmall1BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                      ref.watch(aaoPantsMainSmall1BannerImagesProvider).value ?? [],
                                      ref // Provider의 참조를 전달함
                                  ),
                          ...
                          AaoPriceAndDiscountPercentSortButtons<
                            ...
                            productListProvider: aaoPantsMainProductListProvider,
                            // 팬츠 제품 리스트 정렬 프로바이더 전달
                            sortButtonProvider:
                            aaoPantsMainSortButtonProvider, // 팬츠 정렬 버튼 프로바이더 전달
                          ), // 가격 및 할인 정렬 버튼 추가
                          ...
                          Consumer(
                            ...
                            builder: (context, ref, child) {
                              ...
                              final currentTab =
                              ref.watch(aaoPantsCurrentTabProvider);
                              // 현재 탭: aaoPantsCurrentTabProvider를 구독하고 현재 선택된 탭 정보를 가져옴.
                              final productListProvider =
                                  aaoPantsMainProductListProvider;
                              // 제품 리스트 제공자: aaoPantsMainProductListProvider를 productListProvider 변수에 할당.
