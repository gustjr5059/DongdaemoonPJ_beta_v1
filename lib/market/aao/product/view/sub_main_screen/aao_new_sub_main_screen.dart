...
// AaoNewSubMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AaoNewSubMainScreen extends ConsumerStatefulWidget {
  const AaoNewSubMainScreen({Key? key}) : super(key: key);

  @override
  _AaoNewSubMainScreenState createState() => _AaoNewSubMainScreenState();
}

// _AaoNewSubMainScreenState 클래스 시작
// _AaoNewSubMainScreenState 클래스는 AaoNewSubMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AaoNewSubMainScreenState extends ConsumerState<AaoNewSubMainScreen>
  ...
  // aaoNewSubMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aaoNewSubMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aaoNewSubMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void _updateScrollPosition() {
    ...
    // 'ref'를 사용하여 'aaoNewSubMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (newSubMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(aaoNewSubMainScrollPositionProvider);
                        ...
                        child: Column(
                          children: [
                              ...
                              content: Container(
                                ...
                                child: SizedBox(
                                  ...
                                  child: buildCommonBannerPageViewSection<
                                    ...
                                    // 상태 관리를 위한 참조 전달함
                                    currentPageProvider:
                                        aaoNewSubMainLargeBannerPageProvider,
                                    ...
                                    aaoAllLargeBannerImagesProvider,
                                    // 배너 이미지의 상태 제공자를 전달함
                              ...
                              content: Container(
                                ...
                                child: SizedBox(
                                  ...
                                  child: buildCommonBannerPageViewSection<
                                    ...
                                    // 상태 관리를 위한 참조 전달함
                                    currentPageProvider:
                                        aaoNewSubMainSmall1BannerPageProvider,
                                    ...
                                    bannerImagesProvider:
                                        aaoNewSubMainSmall1BannerImagesProvider,
                                    ...
                                    onPageTap: (context, index) =>
                                        ...
                                        aaoOnSmallBannerTap(
                                            context, // 현재 화면의 컨텍스트를 전달함
                                            index, // 클릭된 배너의 인덱스를 전달함
                                            // aaoNewSubMainSmall1BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                            ref
                                                    .watch(
                                                        aaoNewSubMainSmall1BannerImagesProvider)
                                                    .value ??
                            ...
                            AaoPriceAndDiscountPercentSortButtons<
                                AaoSectionMoreProductListNotifier>(
                              productListProvider:
                                  aaoNewSubMainProductListProvider,
                              // 신상 제품 리스트 프로바이더 전달
                              sortButtonProvider:
                                  aaoNewSubMainSortButtonProvider, // 신상 정렬 버튼 프로바이더 전달
                            ),
                            // 가격 및 할인 정렬 버튼 추가
                            SizedBox(height: interval1Y),
                            // AaoGeneralProductList 위젯을 생성, AaoSectionMoreProductListNotifier를 사용
                            AaoGeneralProductList<AaoSectionMoreProductListNotifier>(
