...
// AahSummerSubMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AahSummerSubMainScreen extends ConsumerStatefulWidget {
  const AahSummerSubMainScreen({Key? key}) : super(key: key);

  @override
  _AahSummerSubMainScreenState createState() => _AahSummerSubMainScreenState();
}

// _AahSummerSubMainScreenState 클래스 시작
// _AahSummerSubMainScreenState 클래스는 AahSummerSubMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AahSummerSubMainScreenState extends ConsumerState<AahSummerSubMainScreen>
  ...
  // aahSummerSubMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aahSummerSubMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aahSummerSubMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  late ScrollController summerSubMainScreenPointScrollController; // 스크롤 컨트롤러 선언
  ...
  void _updateScrollPosition() {
    // 'summerSubMainScreenPointScrollController'에서 현재의 스크롤 위치(offset)를 가져와서 'currentScrollPosition' 변수에 저장함.
    double currentScrollPosition =
        summerSubMainScreenPointScrollController.offset;

    // 'ref'를 사용하여 'aahSummerSubMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    summerSubMainScreenPointScrollController = ScrollController();
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (summerSubMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition =
            ref.read(aahSummerSubMainScrollPositionProvider);
        // summerSubMainScreenPointScrollController.jumpTo 메서드를 사용하여 스크롤 위치를 savedScrollPosition으로 즉시 이동함.
        // 이는 스크롤 애니메이션이나 다른 복잡한 동작 없이 바로 지정된 위치로 점프함.
        summerSubMainScreenPointScrollController.jumpTo(savedScrollPosition);
      }

      // tabIndexProvider의 상태를 하단 탭 바 내 버튼과 매칭이 되면 안되므로 0~3이 아닌 -1로 매핑
      // -> 여름 섹션 더보기 화면 초기화 시, 하단 탭 바 내 모든 버튼 비활성화
    ...
    // 사용자가 스크롤할 때마다 현재의 스크롤 위치를 summerSubMainScreenPointScrollController에 저장하는 코드
    // 상단 탭바 버튼 클릭 시, 해당 섹션으로 화면 이동하는 위치를 저장하는거에 해당 부분도 추가하여
    // 사용자가 앱을 종료하거나 다른 화면으로 이동한 후 돌아왔을 때 마지막으로 본 위치로 자동으로 스크롤되도록 함.
    summerSubMainScreenPointScrollController.addListener(_updateScrollPosition);

    // 큰 배너에 대한 PageController 및 AutoScroll 초기화
    // 'aahSummerSubMainLargeBannerPageProvider'에서 초기 페이지 인덱스를 읽어옴
    _largeBannerPageController = PageController(
        initialPage: ref.read(aahSummerSubMainLargeBannerPageProvider));

    // 큰 배너를 자동으로 스크롤하는 기능 초기화
    _largeBannerAutoScroll = BannerAutoScrollClass(
      pageController: _largeBannerPageController,
      currentPageProvider: aahSummerSubMainLargeBannerPageProvider,
      itemCount: bannerImageCount1, // 총 배너 이미지 개수 전달
    );

    // 작은 배너1에 대한 PageController 및 AutoScroll 초기화
    // 'aahSummerSubMainSmall1BannerPageProvider'에서 초기 페이지 인덱스를 읽어옴
    _small1BannerPageController = PageController(
        initialPage: ref.read(aahSummerSubMainSmall1BannerPageProvider));

    // 작은 배너1을 자동으로 스크롤하는 기능 초기화
    _small1BannerAutoScroll = BannerAutoScrollClass(
      pageController: _small1BannerPageController,
      currentPageProvider: aahSummerSubMainSmall1BannerPageProvider,
  ...
  void dispose() {
    ...
    // 'summerSubMainScreenPointScrollController'의 리스너 목록에서 '_updateScrollPosition' 함수를 제거함.
    // 이는 '_updateScrollPosition' 함수가 더 이상 스크롤 이벤트에 반응하지 않도록 설정함.
    summerSubMainScreenPointScrollController
        .removeListener(_updateScrollPosition);

    summerSubMainScreenPointScrollController.dispose(); // ScrollController 해제
  ...
  Widget build(BuildContext context) {
    ...
    //
    // // 대배너 부분 관련 수치
    // final double summerSubMainScreenLargeBannerWidth =
    //     screenSize.width * (393 / referenceWidth); // 대배너 이미지 너비
    // final double summerSubMainScreenLargeBannerHeight =
    ...
    //
    // // 소배너 부분 관련 수치
    // final double summerSubMainScreenSmallBannerWidth =
    //     screenSize.width * (361 / referenceWidth); // 소배너 이미지 너비
    // final double summerSubMainScreenSmallBannerHeight =
    //     screenSize.height * (90 / referenceHeight); // 소배너 이미지 높이
    // final double summerSubMainScreenSmallBannerViewHeight =
    ...
    return Scaffold(
      body: Stack(
        children: [
          CustomScrollView(
            controller: summerSubMainScreenPointScrollController, // 스크롤 컨트롤러 연결
            slivers: <Widget>[
                        ...
                        child: Column(
                          children: [
                            ...
                            CommonCardView(
                              content: Container(
                                ...
                                child: SizedBox(
                                  ...
                                  height: summerSubMainLargeBannerViewHeight,
                                  ...
                                  child: buildCommonBannerPageViewSection<
                                    ...
                                    // 상태 관리를 위한 참조 전달함
                                    currentPageProvider:
                                        aahSummerSubMainLargeBannerPageProvider,
                                    ...
                                    width: summerSubMainScreenLargeBannerWidth,
                                    // 배너 섹션의 너비를 설정함
                                    height:
                                        summerSubMainScreenLargeBannerHeight,
                                    // 배너 섹션의 높이를 설정함
                            ...
                            CommonCardView(
                              content: Container(
                                ...
                                child: SizedBox(
                                  ...
                                  height:
                                      summerSubMainScreenSmallBannerViewHeight,
                                  ...
                                  child: buildCommonBannerPageViewSection<
                                    ...
                                    // 상태 관리를 위한 참조 전달함
                                    currentPageProvider:
                                        aahSummerSubMainSmall1BannerPageProvider,
                                    ...
                                    bannerImagesProvider:
                                        aahSummerSubMainSmall1BannerImagesProvider,
                                    ...
                                    onPageTap: (context, index) =>
                                        ...
                                        aahOnSmallBannerTap(
                                            context, // 현재 화면의 컨텍스트를 전달함
                                            index, // 클릭된 배너의 인덱스를 전달함
                                            // aahSummerSubMainSmall1BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                            ref
                                                    .watch(
                                                        aahSummerSubMainSmall1BannerImagesProvider)
                                                    .value ??
                                    ...
                                    width: summerSubMainScreenSmallBannerWidth,
                                    // 작은 배너 섹션의 너비를 설정함
                                    height:
                                        summerSubMainScreenSmallBannerHeight,
                                    // 작은 배너 섹션의 높이를 설정함
                            ...
                            AahPriceAndDiscountPercentSortButtons<
                              ...
                              productListProvider:
                                  aahSummerSubMainProductListProvider,
                              // 여름 제품 리스트 프로바이더 전달
                              sortButtonProvider:
                                  aahSummerSubMainSortButtonProvider, // 여름 정렬 버튼 프로바이더 전달
                            ),
                            // 가격 및 할인 정렬 버튼 추가
                            SizedBox(height: interval1Y),
                            ...
                            AahGeneralProductList<AahSectionMoreProductListNotifier>(
                              // 스크롤 컨트롤러를 설정 (여름 섹션의 스크롤 컨트롤러)
                              scrollController:
                                  summerSubMainScreenPointScrollController,
                              // 상품 리스트 프로바이더를 설정 (여름 섹션의 상품 리스트 프로바이더)
                              productListProvider:
                                  aahSummerSubMainProductListProvider,
                              // 카테고리를 '여름'으로 설정
                              category: '여름',
          ...
          // buildTopButton 함수는 주어진 context와 summerSubMainScreenPointScrollController를 사용하여
          // 화면 상단으로 스크롤하기 위한 버튼 생성 위젯이며, common_body_parts_layout.dart 내에 있는 곳에서 재사용하여 구현한 부분
          buildTopButton(context, summerSubMainScreenPointScrollController),
        ],
      ),
      ...
      bottomNavigationBar: buildCommonBottomNavigationBar(
          ...
          scrollController:
              summerSubMainScreenPointScrollController), // 공통으로 사용되는 하단 네비게이션 바를 가져옴.
    );
    // ------ 화면구성 끝
  }
...
// _AahSummerSubMainScreenState 클래스 끝
