...
// AahCardiganMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AahCardiganMainScreen extends ConsumerStatefulWidget {
  const AahCardiganMainScreen({Key? key}) : super(key: key);

  @override
  _AahCardiganMainScreenState createState() => _AahCardiganMainScreenState();
}

// _AahCardiganMainScreenState 클래스 시작
// _AahCardiganMainScreenState 클래스는 AahCardiganMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AahCardiganMainScreenState extends ConsumerState<AahCardiganMainScreen>
  ...
  // aahCardiganMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aahCardiganMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aahCardiganMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  late ScrollController cardiganMainScreenPointScrollController; // 스크롤 컨트롤러 선언
  ...
  // ScrollController가 여러 ScrollView에 attach 되어서 ScrollController가 동시에 여러 ScrollView에서 사용될 때 발생한 문제를 해결한 방법
  // => late로 변수 선언 / 해당 변수를 초기화(initState()) / 해당 변수를 해제 (dispose())
  late ScrollController cardiganMainTopBarPointAutoScrollController;

  NetworkChecker? _networkChecker; // NetworkChecker 인스턴스 저장

  // => cardiganMainScreenPointScrollController는 전체 화면의 스크롤을 제어함 vs cardiganMainTopBarPointAutoScrollController는 상단 탭 바의 스크롤을 제어함.
  // => 그러므로, 서로 다른 UI 요소 제어, 다른 동작 방식, _onScroll 함수 내 다르게 사용하므로 두 컨트롤러를 병합하면 복잡성 증가하고, 동작이 충돌할 수 있어 독립적으로 제작!!
  // => cardiganMainTopBarPointAutoScrollController는 전체 화면의 UI를 담당하는게 아니므로 scaffold의 body 내 컨트롤러에 연결이 안되어도 addListener()에 _onScroll()로 연결해놓은거라 해당 기능 사용이 가능!!
  ...
  void _updateScrollPosition() {
    // 'cardiganMainScreenPointScrollController'에서 현재의 스크롤 위치(offset)를 가져와서 'currentScrollPosition' 변수에 저장함.
    double currentScrollPosition =
        cardiganMainScreenPointScrollController.offset;

    // 'ref'를 사용하여 'aahCardiganMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    cardiganMainScreenPointScrollController = ScrollController();
    cardiganMainTopBarPointAutoScrollController = ScrollController();
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (cardiganMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition =
            ref.read(aahCardiganMainScrollPositionProvider);
        // cardiganMainScreenPointScrollController.jumpTo 메서드를 사용하여 스크롤 위치를 savedScrollPosition으로 즉시 이동함.
      ...
      // aahCardiganCurrentTabProvider를 통해 저장된 탭 인덱스를 읽어옴.
      int savedTabIndex = ref.read(aahCardiganCurrentTabProvider);
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        // 가디건 메인 화면에서 로그아웃 이벤트를 실시간으로 감지하고 처리하는 로직 (여기에도 가디건 메인 화면 내 프로바이더 중 초기화해야하는 것을 로직 구현)
        ref.read(aahCardiganMainLargeBannerPageProvider.notifier).state = 0;
        ref.read(aahCardiganMainSmall1BannerPageProvider.notifier).state = 0;
        ref.read(aahCardiganMainScrollPositionProvider.notifier).state =
            0.0; // 로그아웃 시 aahCardiganMainScrollPositionProvider가 초기화되므로, 재로그인 시 초기 스크롤 위치에서 시작됨. 하지만 상품 데이터는 유지됨.
  ...
  void dispose() {
    ...
    // 'cardiganMainScreenPointScrollController'의 리스너 목록에서 '_updateScrollPosition' 함수를 제거함.
    // 이는 '_updateScrollPosition' 함수가 더 이상 스크롤 이벤트에 반응하지 않도록 설정함.
    cardiganMainScreenPointScrollController
        .removeListener(_updateScrollPosition);

    cardiganMainScreenPointScrollController.dispose(); // ScrollController 해제

    cardiganMainTopBarPointAutoScrollController
  ...
  // aahCardiganMainProductListProvider 관련 카테고리 인덱스를 받아 해당 카테고리 이름을 반환하는 함수
  ...
  Widget build(BuildContext context) {
    ...
    void onTopBarTap(int index) {
      ...
      Future.delayed(Duration.zero, () {
        ref.read(aahCardiganMainProductListProvider.notifier).reset(); // 상태 초기화
        ref.read(aahCardiganMainProductListProvider.notifier).fetchInitialProducts(
    ...
    final double cardiganMainScreenLargeBannerHeight = screenSize.width * (378 / referenceWidth); // 대배너 이미지 높이
    final double cardiganMainLargeBannerViewHeight = screenSize.width * (378 / referenceWidth); // 대배너 화면 세로 비율

    // 소배너 부분 관련 수치
    final double cardiganMainScreenSmallBannerWidth = screenSize.width * (361 / referenceWidth); // 소배너 이미지 너비
    final double cardiganMainScreenSmallBannerHeight = screenSize.width * (90 / referenceWidth); // 소배너 이미지 높이
    final double cardiganMainScreenSmallBannerViewHeight = screenSize.width * (90 / referenceWidth); // 소배너 화면 세로 비율
    ...
    return Scaffold(
      body: Stack(
        children: [
          CustomScrollView(
            controller: cardiganMainScreenPointScrollController, // 스크롤 컨트롤러 연결
            slivers: <Widget>[
                    ...
                    return Padding(
                      ...
                      child: Column(
                        children: [
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  currentPageProvider:
                                  aahCardiganMainLargeBannerPageProvider, // 큰 배너 페이지의 상태 제공자를 전달함
                                  ...
                                  aahAllLargeBannerImagesProvider, // 큰 배너 이미지의 상태 제공자를 전달함
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  ref: ref, // 상태 관리를 위한 참조 전달함
                                  currentPageProvider:
                                  aahCardiganMainSmall1BannerPageProvider, // 작은 배너 페이지의 상태 제공자를 전달함
                                  ...
                                  bannerImagesProvider:
                                  aahCardiganMainSmall1BannerImagesProvider, // 작은 배너 이미지의 상태 제공자를 전달함
                                  // 배너를 탭했을 때 실행할 함수를 전달
                                  onPageTap: (context, index) =>
                                  ...
                                  aahOnSmallBannerTap(
                                      context, // 현재 화면의 컨텍스트를 전달함
                                      index, // 클릭된 배너의 인덱스를 전달함
                                      // aahCardiganMainSmall1BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                      ref.watch(aahCardiganMainSmall1BannerImagesProvider).value ?? [],
                                      ref // Provider의 참조를 전달함
                                  ),
                          ...
                          SizedBox(height: interval2Y), // interval2Y의 높이를 가진 간격 추가
                          Container(
                            decoration: BoxDecoration(
                              border: Border(
                                bottom: BorderSide(color: BLACK_COLOR, width: 1.0), // 하단 테두리 색상을 설정함
                          ...
                          AahPriceAndDiscountPercentSortButtons<
                            ...
                            productListProvider:
                            aahCardiganMainProductListProvider,
                            // 가디건 제품 리스트 정렬 프로바이더 전달
                            sortButtonProvider:
                            aahCardiganMainSortButtonProvider, // 가디건 정렬 버튼 프로바이더 전달
                          ), // 가격 및 할인 정렬 버튼 추가
                          ...
                          Consumer(
                            ...
                            builder: (context, ref, child) {
                              ...
                              final currentTab =
                              ref.watch(aahCardiganCurrentTabProvider);
                              // 현재 탭: cardiganCurrentTabProvider를 구독하고 현재 선택된 탭 정보를 가져옴.
                              final productListProvider =
                                  aahCardiganMainProductListProvider;
                              // 제품 리스트 제공자: cardiganMainProductListProvider를 productListProvider 변수에 할당.
                              return AahGeneralProductList<
                                  AahProductMainListNotifier>(
                                // AahGeneralProductList 반환: AahGeneralProductList 위젯을 반환하여 화면에 제품 목록을 표시.
                                scrollController:
                                cardiganMainScreenPointScrollController,
                                // 스크롤 컨트롤러: cardiganMainScreenPointScrollController를 GeneralProductList의 scrollController로 전달.
          ...
          // buildTopButton 함수는 주어진 context와 cardiganMainScreenPointScrollController를 사용하여
      ...
      bottomNavigationBar: buildCommonBottomNavigationBar(
          ref.watch(tabIndexProvider), ref, context, 5, 1, scrollController: cardiganMainScreenPointScrollController),
