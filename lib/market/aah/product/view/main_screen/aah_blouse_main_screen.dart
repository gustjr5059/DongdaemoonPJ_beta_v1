...
// AahBlouseMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AahBlouseMainScreen extends ConsumerStatefulWidget {
  const AahBlouseMainScreen({Key? key}) : super(key: key);

  @override
  _AahBlouseMainScreenState createState() => _AahBlouseMainScreenState();
}

// _AahBlouseMainScreenState 클래스 시작
// _AahBlouseMainScreenState 클래스는 AahBlouseMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AahBlouseMainScreenState extends ConsumerState<AahBlouseMainScreen>
  ...
  // aahBlouseMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aahBlouseMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aahBlouseMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void _updateScrollPosition() {
    ...
    // 'ref'를 사용하여 'aahBlouseMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (blouseMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(aahBlouseMainScrollPositionProvider);
        // aahBlouseMainScrollPositionProvider.jumpTo 메서드를 사용하여 스크롤 위치를 savedScrollPosition으로 즉시 이동함.
      ...
      // aahBlouseCurrentTabProvider를 통해 저장된 탭 인덱스를 읽어옴.
      int savedTabIndex = ref.read(aahBlouseCurrentTabProvider);
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(aahBlouseMainLargeBannerPageProvider.notifier).state = 0;
        ref.read(aahBlouseMainSmall1BannerPageProvider.notifier).state = 0;
        ref.read(aahBlouseMainScrollPositionProvider.notifier).state =
  ...
  // aahBlouseMainProductListProvider 관련 카테고리 인덱스를 받아 해당 카테고리 이름을 반환하는 함수
  ...
  Widget build(BuildContext context) {
    ...
    void onTopBarTap(int index) {
      ...
      Future.delayed(Duration.zero, () {
        ref.read(aahBlouseMainProductListProvider.notifier).reset(); // 상태 초기화
        ref.read(aahBlouseMainProductListProvider.notifier).fetchInitialProducts(
    ...
    Widget topBarList = buildTopBarList(context, onTopBarTap,
        aahBlouseCurrentTabProvider, blouseMainTopBarPointAutoScrollController);
                      ...
                      child: Column(
                        children: [
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  currentPageProvider:
                                  aahBlouseMainLargeBannerPageProvider, // 큰 배너 페이지의 상태 제공자를 전달함
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  ref: ref, // 상태 관리를 위한 참조를 전달함
                                  currentPageProvider:
                                  aahBlouseMainSmall1BannerPageProvider, // 작은 배너 페이지의 상태 제공자를 전달함
                                  ...
                                  bannerImagesProvider:
                                  aahBlouseMainSmall1BannerImagesProvider, // 작은 배너 이미지의 상태 제공자를 전달함
                                  // 배너를 탭했을 때 실행할 함수를 전달
                                  onPageTap: (context, index) =>
                                  ...
                                  aahOnSmallBannerTap(
                                      context, // 현재 화면의 컨텍스트를 전달함
                                      index, // 클릭된 배너의 인덱스를 전달함
                                      // aahBlouseMainSmall1BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                      ref.watch(aahBlouseMainSmall1BannerImagesProvider).value ?? [],
                                      ref // Provider의 참조를 전달함
                                  ),
                          ...
                          AahPriceAndDiscountPercentSortButtons<
                            ...
                            productListProvider:
                            aahBlouseMainProductListProvider,
                            // 블라우스 제품 리스트 프로바이더 전달
                            sortButtonProvider:
                            aahBlouseMainSortButtonProvider, // 블라우스 정렬 버튼 프로바이더 전달
                          ), // 가격 및 할인 정렬 버튼 추가
                          ...
                          Consumer(
                            ...
                            builder: (context, ref, child) {
                              ...
                              final currentTab =
                              ref.watch(aahBlouseCurrentTabProvider);
                              // 현재 탭: aahBlouseCurrentTabProvider를 구독하고 현재 선택된 탭 정보를 가져옴.
...
// _AahBlouseMainScreenState 클래스 끝
