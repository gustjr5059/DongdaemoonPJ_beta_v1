...
// ------- AacProductsSectionList 클래스 내용 구현 시작
// 주로, 홈 화면 내 2차 카테고리별 섹션 내 데이터를 4개 단위로 스크롤뷰로 UI 구현하는 부분 관련 로직
class AacProductsSectionList extends ConsumerStatefulWidget {
  ...
  // 생성자
  AacProductsSectionList({required this.category, required this.fetchProducts});

  @override
  _AacProductsSectionListState createState() =>
      _AacProductsSectionListState(); // 상태 객체 생성
}

class _AacProductsSectionListState extends ConsumerState<AacProductsSectionList> {
  ...
  void initState() {
    ...
    final savedProducts = ref
        .read(aacHomeSectionDataStateProvider.notifier)
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (_scrollController.hasClients) {
        ...
        final savedScrollPosition =
            ref.read(aacHomeSectionScrollPositionsProvider)[widget.category] ?? 0;
  ...
  void _scrollListener() {
    ...
    ref.read(aacHomeSectionScrollPositionsProvider.notifier).state = {
      ...ref.read(aacHomeSectionScrollPositionsProvider),
  ...
  Future<void> _fetchMoreProducts() async {
    ...
    try {
      ...
      setState(() {
        ...
        ref.read(aacHomeSectionDataStateProvider.notifier).updateSection(
  ...
  Widget build(BuildContext context) {
    final products = ref.watch(aacHomeSectionDataStateProvider)[widget.category] ??
...
// ------- AacProductsSectionList 클래스 내용 구현 끝

// ------ 가격 순, 할인율 순 관련 분류가능하도록 하는 버튼인 AacPriceAndDiscountPercentSortButtons 클래스 내용 구현 시작
class AacPriceAndDiscountPercentSortButtons<T extends AacBaseProductListNotifier>
...
// ------ 가격 순, 할인율 순 관련 분류가능하도록 하는 버튼인 AacPriceAndDiscountPercentSortButtons 클래스 내용 구현 끝

// ------- provider로부터 데이터 받아와서 UI에 구현하는 3개씩 열로 데이터를 보여주는 UI 구현 관련
// AacGeneralProductList 클래스 내용 구현 시작
// 1차 카테고리 관련 메인 화면과 섹션 더보기 화면에서 데이터를 불러올 때 사용하는 UI 구현 부분
class AacGeneralProductList<T extends AacBaseProductListNotifier>
...
class _ProductListState extends ConsumerState<AacGeneralProductList> {
  @override
  void initState() {
