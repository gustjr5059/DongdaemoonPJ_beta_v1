...
// AbaSaleSubMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AbaSaleSubMainScreen extends ConsumerStatefulWidget {
  const AbaSaleSubMainScreen({Key? key}) : super(key: key);

  @override
  _AbaSaleSubMainScreenState createState() => _AbaSaleSubMainScreenState();
}

// _AbaSaleSubMainScreenState 클래스 시작
// _AbaSaleSubMainScreenState 클래스는 AbaSaleSubMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AbaSaleSubMainScreenState extends ConsumerState<AbaSaleSubMainScreen>
  ...
  // abaSaleSubMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(abaSaleSubMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // abaSaleSubMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void _updateScrollPosition() {
    ...
    // 'ref'를 사용하여 'abaSaleSubMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (saleSubMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition =
            ref.read(abaSaleSubMainScrollPositionProvider);
    ...
    // 'abaSaleSubMainLargeBannerPageProvider'에서 초기 페이지 인덱스를 읽어옴
    _largeBannerPageController = PageController(
        initialPage: ref.read(abaSaleSubMainLargeBannerPageProvider));
    ...
    // 'abaSaleSubMainSmall1BannerPageProvider'에서 초기 페이지 인덱스를 읽어옴
    _small1BannerPageController = PageController(
        initialPage: ref.read(abaSaleSubMainSmall1BannerPageProvider));
