...
// AaaMtmMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AaaMtmMainScreen extends ConsumerStatefulWidget {
  const AaaMtmMainScreen({Key? key}) : super(key: key);

  @override
  _AaaMtmMainScreenState createState() => _AaaMtmMainScreenState();
}

// _AaaMtmMainScreenState 클래스 시작
// _AaaMtmMainScreenState 클래스는 AaaMtmMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AaaMtmMainScreenState extends ConsumerState<AaaMtmMainScreen>
  ...
  // aaaMtmMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aaaMtmMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aaaMtmMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void _updateScrollPosition() {
    ...
    // 'ref'를 사용하여 'aaaMtmMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (mtmMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(aaaMtmMainScrollPositionProvider);
      ...
      // aaaMtmCurrentTabProvider를 통해 저장된 탭 인덱스를 읽어옴.
      int savedTabIndex = ref.read(aaaMtmCurrentTabProvider);
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(aaaMtmMainLargeBannerPageProvider.notifier).state = 0;
        ref.read(aaaMtmMainSmall1BannerPageProvider.notifier).state = 0;
        ref.read(aaaMtmMainScrollPositionProvider.notifier).state =
            0.0; // 로그아웃 시 aaaMtmMainScrollPositionProvider가 초기화되므로, 재로그인 시 초기 스크롤 위치에서 시작됨. 하지만 상품 데이터는 유지됨.
  ...
  // aaaMtmMainProductListProvider 관련 카테고리 인덱스를 받아 해당 카테고리 이름을 반환하는 함수
  ...
  Widget build(BuildContext context) {
    ...
    void onTopBarTap(int index) {
      ref.read(aaaMtmCurrentTabProvider.notifier).state = index; // 현재 탭 인덱스를 업데이트
      ...
      Future.delayed(Duration.zero, () {
        ref.read(aaaMtmMainProductListProvider.notifier).reset(); // 상태 초기화
        ref.read(aaaMtmMainProductListProvider.notifier).fetchInitialProducts(
    ...
    Widget topBarList = buildTopBarList(context, onTopBarTap,
        aaaMtmCurrentTabProvider, mtmMainTopBarPointAutoScrollController);
    ...
    final small1BannerWidget = buildCommonBannerPageViewSection<AllSmallBannerImage>(
      ...
      bannerImagesProvider: aaaMtmMainSmall1BannerImagesProvider,
      onPageTap: (context, index) =>
          onSmallBannerTap(context, index, ref.watch(aaaMtmMainSmall1BannerImagesProvider).value ?? [], ref),
                      ...
                      child: Column(
                        children: [
                          ...
                          AaaPriceAndDiscountPercentSortButtons<
                            ...
                            productListProvider: aaaMtmMainProductListProvider,
                            // 맨투맨 제품 리스트 정렬 프로바이더 전달
                            sortButtonProvider:
                            aaaMtmMainSortButtonProvider, // 맨투맨 정렬 버튼 프로바이더 전달
                          ), // 가격 및 할인 정렬 버튼 추가
                          ...
                          Consumer(
                            ...
                            builder: (context, ref, child) {
                              ...
                              final currentTab =
                              ref.watch(aaaMtmCurrentTabProvider);
                              // 현재 탭: aaaMtmCurrentTabProvider를 구독하고 현재 선택된 탭 정보를 가져옴.
                              final productListProvider =
                                  aaaMtmMainProductListProvider;
                              // 제품 리스트 제공자: aaaMtmMainProductListProvider를 productListProvider 변수에 할당.
