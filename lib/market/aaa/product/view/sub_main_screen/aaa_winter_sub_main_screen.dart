...
// AaaWinterSubMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AaaWinterSubMainScreen extends ConsumerStatefulWidget {
  const AaaWinterSubMainScreen({Key? key}) : super(key: key);

  @override
  _AaaWinterSubMainScreenState createState() => _AaaWinterSubMainScreenState();
}

// _AaaWinterSubMainScreenState 클래스 시작
// _AaaWinterSubMainScreenState 클래스는 AaaWinterSubMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AaaWinterSubMainScreenState extends ConsumerState<AaaWinterSubMainScreen>
  ...
  // aaaWinterSubMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aaaWinterSubMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aaaWinterSubMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void _updateScrollPosition() {
    ...
    // 'ref'를 사용하여 'aaaWinterSubMainScrollPositionProvider'의 notifier를 읽어옴.
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (winterSubMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition =
            ref.read(aaaWinterSubMainScrollPositionProvider);
  ...
  Widget build(BuildContext context) {
    ...
    final small1BannerWidget = buildCommonBannerPageViewSection<AllSmallBannerImage>(
      ...
      bannerImagesProvider: aaaWinterSubMainSmall1BannerImagesProvider,
      onPageTap: (context, index) =>
          onSmallBannerTap(context, index, ref.watch(aaaWinterSubMainSmall1BannerImagesProvider).value ?? [], ref),
                        ...
                        child: Column(
                          children: [
                            ...
                            AaaPriceAndDiscountPercentSortButtons<
                              ...
                              productListProvider:
                                  aaaWinterSubMainProductListProvider,
                              // 겨울 제품 리스트 프로바이더 전달
                              sortButtonProvider:
                                  aaaWinterSubMainSortButtonProvider, // 겨울 정렬 버튼 프로바이더 전달
                            ),
                            // 가격 및 할인 정렬 버튼 추가
                            SizedBox(height: interval1Y),
                            Container(
...
// _AaaWinterSubMainScreenState 클래스 끝
