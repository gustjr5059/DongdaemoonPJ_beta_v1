...
final aayHomeLargeBannerPageProvider = StateProvider<int>((ref) => 0);
// 홈 화면의 작은 배너1 페이지 인덱스를 관리하기 위한 StateProvider
final aayHomeSmall1BannerPageProvider = StateProvider<int>((ref) => 0);
// 홈 화면의 작은 배너2 페이지 인덱스를 관리하기 위한 StateProvider
final aayHomeSmall2BannerPageProvider = StateProvider<int>((ref) => 0);
// 홈 화면의 작은 배너3 페이지 인덱스를 관리하기 위한 StateProvider
final aayHomeSmall3BannerPageProvider = StateProvider<int>((ref) => 0);
// 다른 화면으로 이동했다가 다시 돌아오는 경우의
// 홈 화면에서 각 상단 탭 바 관련 섹션의 스크롤 위치와 단순 화면 스크롤로 이동환 위치를 저장하는 StateProvider
final aayHomeScrollPositionProvider = StateProvider<double>((ref) => 0);
// 로그아웃했다가 다시 재로그인하는 경우의
// 홈 화면에서 단순 화면 스크롤로 이동환 위치를 저장하는 StateProvider
final aayHomeLoginAndLogoutScrollPositionProvider =
    StateProvider<double>((ref) => 0);
// 현재 선택된 상단 탭 바 관련 탭의 인덱스 상태 관리를 위한 StateProvider
final aayHomeCurrentTabProvider = StateProvider<int>((ref) => 0);
// 이벤트 섹션 내 가로 스크롤 상태 관리를 위한 StateProvider
final aayEventPosterScrollPositionProvider = StateProvider<double>((ref) => 0.0);

// ------ AaySectionDataStateNotifier 클래스 내용 구현 시작
// AaySectionDataStateNotifier 클래스는 홈 화면 내 섹션의 불러온 데이터 상태를 관리하는 기능을 함
class AaySectionDataStateNotifier
    extends StateNotifier<Map<String, List<ProductContent>>> {
  AaySectionDataStateNotifier() : super({});
...
final aayHomeSectionDataStateProvider = StateNotifierProvider<
    AaySectionDataStateNotifier, Map<String, List<ProductContent>>>((ref) {
  return AaySectionDataStateNotifier();
});

// 홈 화면 내 섹션의 가로 스크롤 위치 저장을 위한 StateProvider
final aayHomeSectionScrollPositionsProvider =
StateProvider<Map<String, double>>((ref) => {});

// ScrollController를 프로바이더로 추가하는 코드
// 이 코드는 aayHomeScrollControllerProvider라는 이름의 Provider를 정의함.
// 하단 탭 바의 홈 버튼 관련 상태 관리 provider
final aayHomeScrollControllerProvider = Provider<ScrollController>((ref) {
