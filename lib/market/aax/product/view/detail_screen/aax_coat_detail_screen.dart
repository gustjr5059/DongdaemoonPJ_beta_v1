...
// AaxCoatDetailProductScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AaxCoatDetailProductScreen extends ConsumerStatefulWidget {
  final String fullPath; // 전체 경로를 나타내는 문자열 변수
  final String title; // 타이틀을 나타내는 문자열 변수

  const AaxCoatDetailProductScreen({
  ...
  @override
  _AaxCoatDetailProductScreenState createState() =>
      _AaxCoatDetailProductScreenState();
}

// _AaxCoatDetailProductScreenState 클래스 시작
// _AaxCoatDetailProductScreenState 클래스는 AaxCoatDetailProductScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AaxCoatDetailProductScreenState
    extends ConsumerState<AaxCoatDetailProductScreen> with WidgetsBindingObserver, RouteAware {
  // 사용자 인증 상태 변경을 감지하는 스트림 구독 객체임.
  // 이를 통해 사용자 로그인 또는 로그아웃 상태 변경을 실시간으로 감지하고 처리할 수 있음.
  StreamSubscription<User?>? authStateChangesSubscription;

  // aaxCoatDetailScrollPositionProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aaxCoatDetailScrollPositionProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aaxCoatDetailScrollPositionProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void initState() {
    ...
    print(
        "AaxCoatDetailProductScreen: 제품 경로에 대한 화면 로드 중: ${widget.fullPath}"); // 디버깅 메시지 추가
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (coatDetailProductScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(aaxCoatDetailScrollPositionProvider);
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(aaxCoatDetailScrollPositionProvider.notifier).state =
            0; // coatDetailScrollPositionProvider의 상태를 0으로 설정
        ref.read(getImagePageProvider(widget.fullPath).notifier).state =
    ...
    // 스크롤 리스너 추가
    coatDetailProductScreenPointScrollController.addListener(_onScroll);
  }
  ...
  void dispose() {
    ...
    // 스크롤 리스너 제거
    coatDetailProductScreenPointScrollController.removeListener(_onScroll);
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<AsyncValue<ProductContent>>(
      aaxCoatDetailProdFirestoreDataProvider(widget.fullPath),
          (previous, next) {
        // 데이터 상태가 로딩 중 -> 데이터 로드 완료로 변경되는 순간에만 로그를 출력합니다.
        if (next is AsyncData && (previous is AsyncLoading || previous == null)) {
          print("CoatDetailProductScreen: 제품 데이터 로드 완료 (ref.listen)");
        }
      },
    );
    ...
    final productContent =
        ref.watch(aaxCoatDetailProdFirestoreDataProvider(widget.fullPath));
    ...
    return Scaffold(
      body: Stack(
        children: [
          CustomScrollView(
            controller: coatDetailProductScreenPointScrollController,
            // 스크롤 컨트롤러 연결
            slivers: <Widget>[
                      ...
                      return Column(
                        children: [
                          productContent.when(
                            data: (product) {
                              print(
                                  "CoatDetailProductScreen: 제품 데이터 로드 완료"); // 디버깅 메시지 추가
                              return Column(
                                children: [
      ...
      bottomNavigationBar: productContent.when(
        data: (product) {
          return buildCommonBottomNavigationBar(
              ...
              product: product,
              scrollController: coatDetailProductScreenPointScrollController);
        },
...
// _AaxCoatDetailProductScreenState 클래스 끝
