...
// AaxShirtDetailProductScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AaxShirtDetailProductScreen extends ConsumerStatefulWidget {
  final String fullPath; // 전체 경로를 나타내는 문자열 변수
  final String title; // 타이틀을 나타내는 문자열 변수

  const AaxShirtDetailProductScreen({
  ...
  @override
  _AaxShirtDetailProductScreenState createState() =>
      _AaxShirtDetailProductScreenState();
}

// _AaxShirtDetailProductScreenState 클래스 시작
// _AaxShirtDetailProductScreenState 클래스는 AaxShirtDetailProductScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AaxShirtDetailProductScreenState
    extends ConsumerState<AaxShirtDetailProductScreen>
    with WidgetsBindingObserver, RouteAware {
  // 사용자 인증 상태 변경을 감지하는 스트림 구독 객체임.
  // 이를 통해 사용자 로그인 또는 로그아웃 상태 변경을 실시간으로 감지하고 처리할 수 있음.
  StreamSubscription<User?>? authStateChangesSubscription;

  // aaxShirtDetailScrollPositionProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aaxShirtDetailScrollPositionProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aaxShirtDetailScrollPositionProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  late ScrollController
      shirtDetailProductScreenPointScrollController; // 스크롤 컨트롤러 선언
  ...
  void initState() {
    ...
    print(
        "AaxShirtDetailProductScreen: 제품 경로에 대한 화면 로드 중: ${widget.fullPath}"); // 디버깅 메시지 추가
    ...
    shirtDetailProductScreenPointScrollController.addListener(() {
      // 현재 어떤 탭인지 확인
      ...
      if (currentTabSection == ProdDetailScreenTabSection.reviews) {
        if (shirtDetailProductScreenPointScrollController.position.pixels ==
            shirtDetailProductScreenPointScrollController.position.maxScrollExtent) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (shirtDetailProductScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition =
            ref.read(aaxShirtDetailScrollPositionProvider);
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(aaxShirtDetailScrollPositionProvider.notifier).state =
            0; // shirtDetailScrollPositionProvider의 상태를 0으로 설정
        ref.read(getImagePageProvider(widget.fullPath).notifier).state =
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<AsyncValue<ProductContent>>(
      aaxShirtDetailProdFirestoreDataProvider(widget.fullPath),
          (previous, next) {
        // 데이터 상태가 로딩 중 -> 데이터 로드 완료로 변경되는 순간에만 로그를 출력합니다.
        if (next is AsyncData && (previous is AsyncLoading || previous == null)) {
          print("ShirtDetailProductScreen: 제품 데이터 로드 완료 (ref.listen)");
        }
      },
    );
    ...
    final productContent =
        ref.watch(aaxShirtDetailProdFirestoreDataProvider(widget.fullPath));
    ...
    return Scaffold(
      body: Stack(
        children: [
          CustomScrollView(
            controller: shirtDetailProductScreenPointScrollController,
            // 스크롤 컨트롤러 연결
            slivers: <Widget>[
                      ...
                      return Column(
                        children: [
                          productContent.when(
                            data: (product) {
                              print(
                                  "ShirtDetailProductScreen: 제품 데이터 로드 완료"); // 디버깅 메시지 추가
                              return Column(
                                children: [
          ...
          buildTopButton(
              context, shirtDetailProductScreenPointScrollController),
        ],
      ),
      ...
      bottomNavigationBar: productContent.when(
        data: (product) {
          return buildCommonBottomNavigationBar(
              ...
              product: product,
              scrollController: shirtDetailProductScreenPointScrollController);
        },
...
// _AaxShirtDetailProductScreenState 클래스 끝
