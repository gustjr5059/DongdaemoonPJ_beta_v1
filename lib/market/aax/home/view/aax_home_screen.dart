...
import '../../common/layout/aax_common_body_parts_layout.dart';
...
import '../layout/aax_home_body_parts_layout.dart';
...
import '../provider/aax_home_all_providers.dart';

// 홈 화면의 상태를 관리하기 위한 Provider 파일을 임포트합니다.
import '../provider/aax_home_state_provider.dart';
...
// AaxHomeMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AaxHomeMainScreen extends ConsumerStatefulWidget {
  const AaxHomeMainScreen({Key? key}) : super(key: key);

  @override
  _AaxHomeMainScreenState createState() => _AaxHomeMainScreenState();
}

// _AaxHomeMainScreenState 클래스 시작
// _AaxHomeMainScreenState 클래스는 AaxHomeMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AaxHomeMainScreenState extends ConsumerState<AaxHomeMainScreen>
  ...
  void _onScroll() {
    ...
    if (newCurrentIndex != currentIndex) {
      ...
      ref.read(aaxHomeCurrentTabProvider.notifier).state = currentIndex; // 상태 업데이트
    }
  ...
  void _updateScrollPosition() {
    ...
    // 'ref'를 사용하여 'aaxHomeScrollPositionProvider'의 notifier를 읽어옴.
    // 읽어온 notifier의 'state' 값을 'currentScrollPosition'으로 설정함.
    // 이렇게 하면 앱의 다른 부분에서 해당 스크롤 위치 정보를 참조할 수 있게 됨.
    ref.read(aaxHomeScrollPositionProvider.notifier).state = currentScrollPosition;
  }
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (homeScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = FirebaseAuth.instance.currentUser != null
            ? ref.read(aaxHomeScrollPositionProvider)
            : ref.read(aaxHomeLoginAndLogoutScrollPositionProvider);
        // 저장된 위치로 스크롤 이동
      ...
      currentIndex = ref.read(aaxHomeCurrentTabProvider);
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(aaxHomeLargeBannerPageProvider.notifier).state = 0;
        ref.read(aaxHomeSmall1BannerPageProvider.notifier).state = 0;
        ref.read(aaxHomeSmall2BannerPageProvider.notifier).state = 0;
        ref.read(aaxHomeSmall3BannerPageProvider.notifier).state = 0;
  ...
  Widget build(BuildContext context) {
    ...
    Widget topBarList = buildTopBarList(context, onTopBarTap,
        aaxHomeCurrentTabProvider, homeTopBarPointAutoScrollController);
                        ...
                        children: [
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  currentPageProvider:
                                  aaxHomeLargeBannerPageProvider,
                          ...
                          CommonCardView(
                            ...
                            content: Container(
                              child: MidCategoryButtonList(
                                onCategoryTap: aaxOnMidCategoryTap,
                              ),
                              decoration: BoxDecoration(
                                border: Border(
                                  bottom: BorderSide(color: BLACK_COLOR, width: 1.0), // 하단 테두리 색상을 설정함
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  // 현재 페이지를 관리하는 Provider를 전달
                                  currentPageProvider:
                                  aaxHomeSmall1BannerPageProvider,
                                  ...
                                  // 배너 이미지들을 관리하는 Provider를 전달
                                  bannerImagesProvider:
                                  aaxHomeSmall1BannerImagesProvider,
                                  // 배너를 탭했을 때 실행할 함수를 전달
                                  onPageTap: (context, index) =>
                                  ...
                                  aaxOnSmallBannerTap(
                                      context, // 현재 화면의 컨텍스트를 전달함
                                      index, // 클릭된 배너의 인덱스를 전달함
                                      // aaxHomeSmall1BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                      ref.watch(aaxHomeSmall1BannerImagesProvider).value ?? [],
                                      ref // Provider의 참조를 전달함
                                  ),
                          ...
                          Container(
                            ...
                            child: buildSectionCard(
                                context, ref, "신상", aaxBuildNewProductsSection,
                          ...
                          Container(
                            key: sectionSaleKey,
                            child: buildSectionCard(
                                context, ref, "특가 상품", aaxBuildSaleProductsSection,
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  // 현재 페이지를 관리하는 Provider를 전달
                                  currentPageProvider:
                                  aaxHomeSmall2BannerPageProvider,
                                  // 페이지 전환을 관리하는 페이지 컨트롤러를 전달
                                  pageController: _small2BannerPageController,
                                  // 배너 자동 스크롤 기능을 전달
                                  ...
                                  // 배너 이미지들을 관리하는 Provider를 전달
                                  bannerImagesProvider:
                                  aaxHomeSmall2BannerImagesProvider,
                                  // 배너를 탭했을 때 실행할 함수를 전달
                                  onPageTap: (context, index) =>
                                  // 소배너 클릭 시 호출할 함수 aaxOnSmallBannerTap 실행
                                  aaxOnSmallBannerTap(
                                      context, // 현재 화면의 컨텍스트를 전달함
                                      index, // 클릭된 배너의 인덱스를 전달함
                                      // aaxHomeSmall2BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                      ref.watch(aaxHomeSmall2BannerImagesProvider).value ?? [],
                                      ref // Provider의 참조를 전달함
                                  ),
                                  width: homeScreenSmallBannerWidth, // 원하는 너비
                                  height: homeScreenSmallBannerHeight, // 원하는 높이
                          ...
                          Container(
                            ...
                            child: buildSectionCard(
                                context, ref, "봄", aaxBuildSpringProductsSection,
                          ...
                          Container(
                            ...
                            child: buildSectionCard(
                                context, ref, "여름", aaxBuildSummerProductsSection,
                            ...
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  // 현재 페이지를 관리하는 Provider를 전달
                                  currentPageProvider:
                                  aaxHomeSmall3BannerPageProvider,
                                  // 페이지 전환을 관리하는 페이지 컨트롤러를 전달
                                  pageController: _small3BannerPageController,
                                  // 배너 자동 스크롤 기능을 전달
                                  ...
                                  // 배너 이미지들을 관리하는 Provider를 전달
                                  bannerImagesProvider:
                                  aaxHomeSmall3BannerImagesProvider,
                                  // 배너를 탭했을 때 실행할 함수를 전달
                                  onPageTap: (context, index) =>
                                  // 소배너 클릭 시 호출할 함수 aaxOnSmallBannerTap 실행
                                  aaxOnSmallBannerTap(
                                      context, // 현재 화면의 컨텍스트를 전달함
                                      index, // 클릭된 배너의 인덱스를 전달함
                                      // aaxHomeSmall3BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                      ref.watch(aaxHomeSmall3BannerImagesProvider).value ?? [],
                                      ref // Provider의 참조를 전달함
                                  ),
                                  width: homeScreenSmallBannerWidth, // 원하는 너비
                                  height: homeScreenSmallBannerHeight, // 원하는 높이
                          ...
                          Container(
                            ...
                            child: buildSectionCard(
                                context, ref, "가을", aaxBuildAutumnProductsSection,
                          ...
                          Container(
                            ...
                            child: buildSectionCard(
                                context, ref, "겨울", aaxBuildWinterProductsSection,
                          ...
                          Container(
                            ...
                            child: buildSectionCard(context, ref, "이벤트",
                                aaxBuildEventPosterImgProductsSection,
