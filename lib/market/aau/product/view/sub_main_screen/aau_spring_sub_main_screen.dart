...
// AauSpringSubMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AauSpringSubMainScreen extends ConsumerStatefulWidget {
  const AauSpringSubMainScreen({Key? key}) : super(key: key);

  @override
  _AauSpringSubMainScreenState createState() => _AauSpringSubMainScreenState();
}

// _AauSpringSubMainScreenState 클래스 시작
// _AauSpringSubMainScreenState 클래스는 AauSpringSubMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AauSpringSubMainScreenState extends ConsumerState<AauSpringSubMainScreen>
  ...
  // aauSpringSubMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aauSpringSubMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aauSpringSubMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void _updateScrollPosition() {
    ...
    // 'ref'를 사용하여 'aauSpringSubMainScrollPositionProvider'의 notifier를 읽어옴.
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (springSubMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition =
            ref.read(aauSpringSubMainScrollPositionProvider);
  ...
  Widget build(BuildContext context) {
    ...
    final small1BannerWidget = buildCommonBannerPageViewSection<AllSmallBannerImage>(
      ...
      bannerImagesProvider: aauSpringSubMainSmall1BannerImagesProvider,
      onPageTap: (context, index) =>
          onSmallBannerTap(context, index, ref.watch(aauSpringSubMainSmall1BannerImagesProvider).value ?? [], ref),
                        ...
                        child: Column(
                          children: [
                            ...
                            AauPriceAndDiscountPercentSortButtons<
                              ...
                              productListProvider:
                              aauSpringSubMainProductListProvider,
                              // 봄 제품 리스트 프로바이더 전달
                              sortButtonProvider:
                              aauSpringSubMainSortButtonProvider, // 봄 정렬 버튼 프로바이더 전달
                            ),
                            // 가격 및 할인 정렬 버튼 추가
                            SizedBox(height: interval1Y),
                            Container(
