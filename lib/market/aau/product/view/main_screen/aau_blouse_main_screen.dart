...
// AauBlouseMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AauBlouseMainScreen extends ConsumerStatefulWidget {
  const AauBlouseMainScreen({Key? key}) : super(key: key);

  @override
  _AauBlouseMainScreenState createState() => _AauBlouseMainScreenState();
}

// _AauBlouseMainScreenState 클래스 시작
// _AauBlouseMainScreenState 클래스는 AauBlouseMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AauBlouseMainScreenState extends ConsumerState<AauBlouseMainScreen>
  ...
  // aauBlouseMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aauBlouseMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aauBlouseMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void _updateScrollPosition() {
    ...
    // 'ref'를 사용하여 'aauBlouseMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (blouseMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(aauBlouseMainScrollPositionProvider);
        // aauBlouseMainScrollPositionProvider.jumpTo 메서드를 사용하여 스크롤 위치를 savedScrollPosition으로 즉시 이동함.
      ...
      // aauBlouseCurrentTabProvider를 통해 저장된 탭 인덱스를 읽어옴.
      int savedTabIndex = ref.read(aauBlouseCurrentTabProvider);
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(aauBlouseMainLargeBannerPageProvider.notifier).state = 0;
        ref.read(aauBlouseMainSmall1BannerPageProvider.notifier).state = 0;
        ref.read(aauBlouseMainScrollPositionProvider.notifier).state =
  ...
  // aauBlouseMainProductListProvider 관련 카테고리 인덱스를 받아 해당 카테고리 이름을 반환하는 함수
  ...
  Widget build(BuildContext context) {
    ...
    void onTopBarTap(int index) {
      ...
      Future.delayed(Duration.zero, () {
        ref.read(aauBlouseMainProductListProvider.notifier).reset(); // 상태 초기화
        ref.read(aauBlouseMainProductListProvider.notifier).fetchInitialProducts(
    ...
    Widget topBarList = buildTopBarList(context, onTopBarTap,
        aauBlouseCurrentTabProvider, blouseMainTopBarPointAutoScrollController);
    ...
    final small1BannerWidget = buildCommonBannerPageViewSection<AllSmallBannerImage>(
      ...
      bannerImagesProvider: aauBlouseMainSmall1BannerImagesProvider,
      onPageTap: (context, index) =>
          onSmallBannerTap(context, index, ref.watch(aauBlouseMainSmall1BannerImagesProvider).value ?? [], ref),
                      ...
                      child: Column(
                        children: [
                          ...
                          AauPriceAndDiscountPercentSortButtons<
                            ...
                            productListProvider:
                            aauBlouseMainProductListProvider,
                            // 블라우스 제품 리스트 프로바이더 전달
                            sortButtonProvider:
                            aauBlouseMainSortButtonProvider, // 블라우스 정렬 버튼 프로바이더 전달
                          ), // 가격 및 할인 정렬 버튼 추가
                          ...
                          Consumer(
                            ...
                            builder: (context, ref, child) {
                              ...
                              final currentTab =
                              ref.watch(aauBlouseCurrentTabProvider);
                              // 현재 탭: aauBlouseCurrentTabProvider를 구독하고 현재 선택된 탭 정보를 가져옴.
...
// _AauBlouseMainScreenState 클래스 끝
