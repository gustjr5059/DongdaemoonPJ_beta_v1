...
// AalBestSubMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AalBestSubMainScreen extends ConsumerStatefulWidget {
  const AalBestSubMainScreen({Key? key}) : super(key: key);

  @override
  _AalBestSubMainScreenState createState() => _AalBestSubMainScreenState();
}

// _AalBestSubMainScreenState 클래스 시작
// _AalBestSubMainScreenState 클래스는 AalBestSubMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AalBestSubMainScreenState extends ConsumerState<AalBestSubMainScreen>
  ...
  // aalBestSubMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aalBestSubMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aalBestSubMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void _updateScrollPosition() {
    ...
    double currentScrollPosition =
        bestSubMainScreenPointScrollController.offset;

    // 'ref'를 사용하여 'aalBestSubMainScrollPositionProvider'의 notifier를 읽어옴.
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (bestSubMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition =
            ref.read(aalBestSubMainScrollPositionProvider);
      ...
      // -> 최고 섹션 더보기 화면 초기화 시, 하단 탭 바 내 모든 버튼 비활성화
  ...
  Widget build(BuildContext context) {
    ...
    //
    // // 대배너 부분 관련 수치
    // final double bestSubMainScreenLargeBannerWidth =
    //     screenSize.width * (393 / referenceWidth); // 대배너 이미지 너비
    // final double bestSubMainScreenLargeBannerHeight =
    //     screenSize.height * (378 / referenceHeight); // 대배너 이미지 높이
    // final double bestSubMainLargeBannerViewHeight =
    //     screenSize.height * (378 / referenceHeight); // 대배너 화면 세로 비율
    //
    // // 소배너 부분 관련 수치
    // final double bestSubMainScreenSmallBannerWidth =
    //     screenSize.width * (361 / referenceWidth); // 소배너 이미지 너비
    // final double bestSubMainScreenSmallBannerHeight =
    //     screenSize.height * (90 / referenceHeight); // 소배너 이미지 높이
    // final double bestSubMainScreenSmallBannerViewHeight =
    ...
    // 대배너 부분 관련 수치
    final double bestSubMainScreenLargeBannerWidth = screenSize.width * (393 / referenceWidth); // 대배너 이미지 너비
    final double bestSubMainScreenLargeBannerHeight = screenSize.width * (378 / referenceWidth); // 대배너 이미지 높이
    final double bestSubMainLargeBannerViewHeight = screenSize.width * (378 / referenceWidth); // 대배너 화면 세로 비율

    // 소배너 부분 관련 수치
    final double bestSubMainScreenSmallBannerWidth = screenSize.width * (361 / referenceWidth); // 소배너 이미지 너비
    final double bestSubMainScreenSmallBannerHeight = screenSize.width * (90 / referenceWidth); // 소배너 이미지 높이
    final double bestSubMainScreenSmallBannerViewHeight = screenSize.width * (90 / referenceWidth); // 소배너 화면 세로 비율
    ...
    return Scaffold(
                          ...
                          children: [
                            ...
                            CommonCardView(
                              content: Container(
                                ...
                                child: SizedBox(
                                  // 배너 섹션의 높이를 설정함
                                  height: bestSubMainLargeBannerViewHeight,
                                  ...
                                  child: buildCommonBannerPageViewSection<
                                    ...
                                    width: bestSubMainScreenLargeBannerWidth,
                                    // 큰 배너 섹션의 너비를 설정함
                                    height: bestSubMainScreenLargeBannerHeight,
                                    // 큰 배너 섹션의 높이를 설정함
                            ...
                            CommonCardView(
                              content: Container(
                                ...
                                child: SizedBox(
                                  // 작은 배너 섹션의 높이를 설정함
                                  height:
                                      bestSubMainScreenSmallBannerViewHeight,
                                  ...
                                  child: buildCommonBannerPageViewSection<
                                    ...
                                    // 상태 관리를 위한 참조를 전달함
                                    currentPageProvider:
                                        aalBestSubMainSmall1BannerPageProvider,
                                    ...
                                    bannerImagesProvider:
                                        aalBestSubMainSmall1BannerImagesProvider,
                                    ...
                                    onPageTap: (context, index) =>
                                        ...
                                        aalOnSmallBannerTap(
                                            context, // 현재 화면의 컨텍스트를 전달함
                                            index, // 클릭된 배너의 인덱스를 전달함
                                            // aalBestSubMainSmall1BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                            ref
                                                    .watch(
                                                        aalBestSubMainSmall1BannerImagesProvider)
                                                    .value ??
                                    ...
                                    width: bestSubMainScreenSmallBannerWidth,
                                    // 작은 배너 섹션의 너비를 설정함
                                    height: bestSubMainScreenSmallBannerHeight,
                                    // 작은 배너 섹션의 높이를 설정함
                            ...
                            AalPriceAndDiscountPercentSortButtons<
                              ...
                              productListProvider:
                                  aalBestSubMainProductListProvider,
                              // 스테디 셀러 제품 리스트 프로바이더 전달
                              sortButtonProvider:
                                  aalBestSubMainSortButtonProvider, // 스테디 셀러 정렬 버튼 프로바이더 전달
                            ),
                            // 가격 및 할인 정렬 버튼 추가
                            SizedBox(height: interval1Y),
      ...
      bottomNavigationBar: buildCommonBottomNavigationBar(
          ...
          scrollController: bestSubMainScreenPointScrollController),
