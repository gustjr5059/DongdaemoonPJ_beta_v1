...
// AalNewSubMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AalNewSubMainScreen extends ConsumerStatefulWidget {
  const AalNewSubMainScreen({Key? key}) : super(key: key);

  @override
  _AalNewSubMainScreenState createState() => _AalNewSubMainScreenState();
}

// _AalNewSubMainScreenState 클래스 시작
// _AalNewSubMainScreenState 클래스는 AalNewSubMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AalNewSubMainScreenState extends ConsumerState<AalNewSubMainScreen>
  ...
  // aalNewSubMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(aalNewSubMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // aalNewSubMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void _updateScrollPosition() {
    ...
    // 'ref'를 사용하여 'aalNewSubMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (newSubMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(aalNewSubMainScrollPositionProvider);
                        ...
                        child: Column(
                          children: [
                              ...
                              content: Container(
                                ...
                                child: SizedBox(
                                  ...
                                  child: buildCommonBannerPageViewSection<
                                    ...
                                    // 상태 관리를 위한 참조 전달함
                                    currentPageProvider:
                                        aalNewSubMainLargeBannerPageProvider,
                                    ...
                                    aalAllLargeBannerImagesProvider,
                                    // 배너 이미지의 상태 제공자를 전달함
                              ...
                              content: Container(
                                ...
                                child: SizedBox(
                                  ...
                                  child: buildCommonBannerPageViewSection<
                                    ...
                                    // 상태 관리를 위한 참조 전달함
                                    currentPageProvider:
                                        aalNewSubMainSmall1BannerPageProvider,
                                    ...
                                    bannerImagesProvider:
                                        aalNewSubMainSmall1BannerImagesProvider,
                                    ...
                                    onPageTap: (context, index) =>
                                        ...
                                        aalOnSmallBannerTap(
                                            context, // 현재 화면의 컨텍스트를 전달함
                                            index, // 클릭된 배너의 인덱스를 전달함
                                            // aalNewSubMainSmall1BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                            ref
                                                    .watch(
                                                        aalNewSubMainSmall1BannerImagesProvider)
                                                    .value ??
                            ...
                            AalPriceAndDiscountPercentSortButtons<
                                AalSectionMoreProductListNotifier>(
                              productListProvider:
                                  aalNewSubMainProductListProvider,
                              // 신상 제품 리스트 프로바이더 전달
                              sortButtonProvider:
                                  aalNewSubMainSortButtonProvider, // 신상 정렬 버튼 프로바이더 전달
                            ),
                            // 가격 및 할인 정렬 버튼 추가
                            SizedBox(height: interval1Y),
                            // AalGeneralProductList 위젯을 생성, AalSectionMoreProductListNotifier를 사용
                            AalGeneralProductList<AalSectionMoreProductListNotifier>(
