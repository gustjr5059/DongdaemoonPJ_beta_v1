...
// AbbSpringSubMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AbbSpringSubMainScreen extends ConsumerStatefulWidget {
  const AbbSpringSubMainScreen({Key? key}) : super(key: key);

  @override
  _AbbSpringSubMainScreenState createState() => _AbbSpringSubMainScreenState();
}

// _AbbSpringSubMainScreenState 클래스 시작
// _AbbSpringSubMainScreenState 클래스는 AbbSpringSubMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AbbSpringSubMainScreenState extends ConsumerState<AbbSpringSubMainScreen>
  ...
  // abbSpringSubMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(abbSpringSubMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // abbSpringSubMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  late ScrollController springSubMainScreenPointScrollController; // 스크롤 컨트롤러 선언
  ...
  void _updateScrollPosition() {
    // 'springSubMainScreenPointScrollController'에서 현재의 스크롤 위치(offset)를 가져와서 'currentScrollPosition' 변수에 저장함.
    double currentScrollPosition =
        springSubMainScreenPointScrollController.offset;

    // 'ref'를 사용하여 'abbSpringSubMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    springSubMainScreenPointScrollController = ScrollController();
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (springSubMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition =
            ref.read(abbSpringSubMainScrollPositionProvider);
        // springSubMainScreenPointScrollController.jumpTo 메서드를 사용하여 스크롤 위치를 savedScrollPosition으로 즉시 이동함.
        // 이는 스크롤 애니메이션이나 다른 복잡한 동작 없이 바로 지정된 위치로 점프함.
        springSubMainScreenPointScrollController.jumpTo(savedScrollPosition);
      }

      // tabIndexProvider의 상태를 하단 탭 바 내 버튼과 매칭이 되면 안되므로 0~3이 아닌 -1로 매핑
      // -> 봄 섹션 더보기 화면 초기화 시, 하단 탭 바 내 모든 버튼 비활성화
    ...
    // 사용자가 스크롤할 때마다 현재의 스크롤 위치를 springSubMainScreenPointScrollController에 저장하는 코드
    // 상단 탭바 버튼 클릭 시, 해당 섹션으로 화면 이동하는 위치를 저장하는거에 해당 부분도 추가하여
    // 사용자가 앱을 종료하거나 다른 화면으로 이동한 후 돌아왔을 때 마지막으로 본 위치로 자동으로 스크롤되도록 함.
    springSubMainScreenPointScrollController.addListener(_updateScrollPosition);

    // 큰 배너에 대한 PageController 및 AutoScroll 초기화
    // 'abbSpringSubMainLargeBannerPageProvider'에서 초기 페이지 인덱스를 읽어옴
    _largeBannerPageController = PageController(
        initialPage: ref.read(abbSpringSubMainLargeBannerPageProvider));

    // 큰 배너를 자동으로 스크롤하는 기능 초기화
    _largeBannerAutoScroll = BannerAutoScrollClass(
      pageController: _largeBannerPageController,
      currentPageProvider: abbSpringSubMainLargeBannerPageProvider,
      itemCount: bannerImageCount1, // 총 배너 이미지 개수 전달
    );

    // 작은 배너1에 대한 PageController 및 AutoScroll 초기화
    // 'abbSpringSubMainSmall1BannerPageProvider'에서 초기 페이지 인덱스를 읽어옴
    _small1BannerPageController = PageController(
        initialPage: ref.read(abbSpringSubMainSmall1BannerPageProvider));

    // 작은 배너1을 자동으로 스크롤하는 기능 초기화
    _small1BannerAutoScroll = BannerAutoScrollClass(
      pageController: _small1BannerPageController,
      currentPageProvider: abbSpringSubMainSmall1BannerPageProvider,
  ...
  void dispose() {
    ...
    // 'springSubMainScreenPointScrollController'의 리스너 목록에서 '_updateScrollPosition' 함수를 제거함.
    ...
    springSubMainScreenPointScrollController.dispose(); // ScrollController 해제
  ...
  Widget build(BuildContext context) {
    ...
    final double springSubMainScreenLargeBannerHeight = screenSize.width * (378 / referenceWidth); // 대배너 이미지 높이
    final double springSubMainLargeBannerViewHeight = screenSize.width * (378 / referenceWidth); // 대배너 화면 세로 비율

    // 소배너 부분 관련 수치
    final double springSubMainScreenSmallBannerWidth = screenSize.width * (361 / referenceWidth); // 소배너 이미지 너비
    final double springSubMainScreenSmallBannerHeight = screenSize.width * (90 / referenceWidth); // 소배너 이미지 높이
    final double springSubMainScreenSmallBannerViewHeight = screenSize.width * (90 / referenceWidth); // 소배너 화면 세로 비율
    ...
    final largeBannerWidget = buildCommonBannerPageViewSection<AllLargeBannerImage>(
      ...
      height: springSubMainScreenLargeBannerHeight,
      borderRadius: 0,
    );
    ...
    final small1BannerWidget = buildCommonBannerPageViewSection<AllSmallBannerImage>(
      ...
      bannerImagesProvider: abbSpringSubMainSmall1BannerImagesProvider,
      onPageTap: (context, index) =>
          onSmallBannerTap(context, index, ref.watch(abbSpringSubMainSmall1BannerImagesProvider).value ?? [], ref),
      width: springSubMainScreenSmallBannerWidth,
      height: springSubMainScreenSmallBannerHeight,
      borderRadius: 5,
    );
    ...
    return Scaffold(
      body: Stack(
        children: [
          CustomScrollView(
            controller: springSubMainScreenPointScrollController, // 스크롤 컨트롤러 연결
            slivers: <Widget>[
                        ...
                        child: Column(
                          children: [
                            ...
                            if (largeBannerWidget is! SizedBox) ...[
                              CommonCardView(
                                content: Container(
                                  ...
                                  child: SizedBox(
                                    height: springSubMainLargeBannerViewHeight,
                            ...
                            AbbPriceAndDiscountPercentSortButtons<
                              ...
                              productListProvider:
                              abbSpringSubMainProductListProvider,
                              // 봄 제품 리스트 프로바이더 전달
                              sortButtonProvider:
                              abbSpringSubMainSortButtonProvider, // 봄 정렬 버튼 프로바이더 전달
                            ),
                            // 가격 및 할인 정렬 버튼 추가
                            SizedBox(height: interval1Y),
                            Container(
          ...
          // buildTopButton 함수는 주어진 context와 springSubMainScreenPointScrollController를 사용하여
          // 화면 상단으로 스크롤하기 위한 버튼 생성 위젯이며, common_body_parts_layout.dart 내에 있는 곳에서 재사용하여 구현한 부분
          buildTopButton(context, springSubMainScreenPointScrollController),
        ],
      ),
