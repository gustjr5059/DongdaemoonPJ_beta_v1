...
import '../../../common/layout/abb_common_body_parts_layout.dart'; // 공통 UI 컴포넌트 파일

// 제품 상태 관리를 위해 사용되는 상태 제공자 파일을 임포트합니다.
// 이 파일은 제품 관련 데이터의 상태를 관리하고, 필요에 따라 상태를 업데이트하는 로직을 포함합니다.
import '../../../common/provider/abb_common_all_provider.dart';
import '../../../home/provider/abb_home_all_providers.dart';
import '../../layout/abb_product_body_parts_layout.dart';
import '../../provider/abb_product_state_provider.dart';
...
import '../../provider/abb_product_all_providers.dart';
...
// AbbNeatMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AbbNeatMainScreen extends ConsumerStatefulWidget {
  const AbbNeatMainScreen({Key? key}) : super(key: key);

  @override
  _AbbNeatMainScreenState createState() => _AbbNeatMainScreenState();
}

// _AbbNeatMainScreenState 클래스 시작
// _AbbNeatMainScreenState 클래스는 AbbNeatMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AbbNeatMainScreenState extends ConsumerState<AbbNeatMainScreen>
  ...
  // abbNeatMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(abbNeatMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // abbNeatMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  late ScrollController neatMainScreenPointScrollController; // 스크롤 컨트롤러 선언
  ...
  // ScrollController가 여러 ScrollView에 attach 되어서 ScrollController가 동시에 여러 ScrollView에서 사용될 때 발생한 문제를 해결한 방법
  // => late로 변수 선언 / 해당 변수를 초기화(initState()) / 해당 변수를 해제 (dispose())
  late ScrollController neatMainTopBarPointAutoScrollController;

  NetworkChecker? _networkChecker; // NetworkChecker 인스턴스 저장

  // => neatMainScreenPointScrollController는 전체 화면의 스크롤을 제어함 vs neatMainTopBarPointAutoScrollController는 상단 탭 바의 스크롤을 제어함.
  // => 그러므로, 서로 다른 UI 요소 제어, 다른 동작 방식, _onScroll 함수 내 다르게 사용하므로 두 컨트롤러를 병합하면 복잡성 증가하고, 동작이 충돌할 수 있어 독립적으로 제작!!
  // => neatMainTopBarPointAutoScrollController는 전체 화면의 UI를 담당하는게 아니므로 scaffold의 body 내 컨트롤러에 연결이 안되어도 addListener()에 _onScroll()로 연결해놓은거라 해당 기능 사용이 가능!!
  ...
  void _updateScrollPosition() {
    // 'neatMainScreenPointScrollController'에서 현재의 스크롤 위치(offset)를 가져와서 'currentScrollPosition' 변수에 저장함.
    double currentScrollPosition = neatMainScreenPointScrollController.offset;

    // 'ref'를 사용하여 'abbNeatMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    neatMainScreenPointScrollController = ScrollController();
    neatMainTopBarPointAutoScrollController = ScrollController();
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (neatMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(abbNeatMainScrollPositionProvider);
        // neatMainScreenPointScrollController.jumpTo 메서드를 사용하여 스크롤 위치를 savedScrollPosition으로 즉시 이동함.
        // 이는 스크롤 애니메이션이나 다른 복잡한 동작 없이 바로 지정된 위치로 점프함.
        neatMainScreenPointScrollController.jumpTo(savedScrollPosition);
      }

      // 저장된 탭 인덱스를 불러와 상단 탭 바의 위치를 복원.
      // abbNeatCurrentTabProvider를 통해 저장된 탭 인덱스를 읽어옴.
      int savedTabIndex = ref.read(abbNeatCurrentTabProvider);

      // 저장된 탭 인덱스가 6 이상인 경우 (탭이 끝부분에 위치한 경우),
      // 상단 탭 바를 스크롤 가능한 최대 범위까지 이동시킴.
      if (savedTabIndex >= 6) {
        double offset =
            neatMainTopBarPointAutoScrollController.position.maxScrollExtent;
        neatMainTopBarPointAutoScrollController.jumpTo(offset);
      }
      // 저장된 탭 인덱스가 1 이하인 경우 (탭이 처음 부분에 위치한 경우),
      // 상단 탭 바를 맨 앞으로 이동시킴.
      else if (savedTabIndex <= 1) {
        neatMainTopBarPointAutoScrollController.jumpTo(0.0);
      }

      // tabIndexProvider의 상태를 하단 탭 바 내 버튼과 매칭이 되면 안되므로 0~3이 아닌 -1로 매핑
      // -> 니트 메인 화면 초기화 시, 하단 탭 바 내 모든 버튼 비활성화
    ...
    // 사용자가 스크롤할 때마다 현재의 스크롤 위치를 neatMainScreenPointScrollController에 저장하는 코드
    // 상단 탭바 버튼 클릭 시, 해당 섹션으로 화면 이동하는 위치를 저장하는거에 해당 부분도 추가하여
    // 사용자가 앱을 종료하거나 다른 화면으로 이동한 후 돌아왔을 때 마지막으로 본 위치로 자동으로 스크롤되도록 함.
    neatMainScreenPointScrollController.addListener(_updateScrollPosition);

    // 큰 배너에 대한 PageController 및 AutoScroll 초기화
    // 'abbNeatMainLargeBannerPageProvider'에서 초기 페이지 인덱스를 읽어옴
    _largeBannerPageController =
        PageController(initialPage: ref.read(abbNeatMainLargeBannerPageProvider));

    // 큰 배너를 자동으로 스크롤하는 기능 초기화
    _largeBannerAutoScroll = BannerAutoScrollClass(
      pageController: _largeBannerPageController,
      currentPageProvider: abbNeatMainLargeBannerPageProvider,
      itemCount: bannerImageCount1, // 총 배너 이미지 개수 전달
    );

    // 작은 배너1에 대한 PageController 및 AutoScroll 초기화
    // 'abbNeatMainSmall1BannerPageProvider'에서 초기 페이지 인덱스를 읽어옴
    _small1BannerPageController =
        PageController(initialPage: ref.read(abbNeatMainSmall1BannerPageProvider));

    // 작은 배너1을 자동으로 스크롤하는 기능 초기화
    _small1BannerAutoScroll = BannerAutoScrollClass(
      pageController: _small1BannerPageController,
      currentPageProvider: abbNeatMainSmall1BannerPageProvider,
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        // 니트 메인 화면에서 로그아웃 이벤트를 실시간으로 감지하고 처리하는 로직 (여기에도 니트 메인 화면 내 프로바이더 중 초기화해야하는 것을 로직 구현)
        ref.read(abbNeatMainLargeBannerPageProvider.notifier).state = 0;
        ref.read(abbNeatMainSmall1BannerPageProvider.notifier).state = 0;
        ref.read(abbNeatMainScrollPositionProvider.notifier).state =
            0.0; // 로그아웃 시 abbNeatMainScrollPositionProvider가 초기화되므로, 재로그인 시 초기 스크롤 위치에서 시작됨. 하지만 상품 데이터는 유지됨.
  ...
  void dispose() {
    ...
    // 'neatMainScreenPointScrollController'의 리스너 목록에서 '_updateScrollPosition' 함수를 제거함.
    // 이는 '_updateScrollPosition' 함수가 더 이상 스크롤 이벤트에 반응하지 않도록 설정함.
    neatMainScreenPointScrollController.removeListener(_updateScrollPosition);

    neatMainScreenPointScrollController.dispose(); // ScrollController 해제

    neatMainTopBarPointAutoScrollController.dispose(); // ScrollController 해제
  ...
  // abbNeatMainProductListProvider 관련 카테고리 인덱스를 받아 해당 카테고리 이름을 반환하는 함수
  ...
  Widget build(BuildContext context) {
    ...
    void onTopBarTap(int index) {
      ref.read(abbNeatCurrentTabProvider.notifier).state = index; // 현재 탭 인덱스를 업데이트
      ...
      Future.delayed(Duration.zero, () {
        ref.read(abbNeatMainProductListProvider.notifier).reset(); // 상태 초기화
        ref.read(abbNeatMainProductListProvider.notifier).fetchInitialProducts(
    ...
    Widget topBarList = buildTopBarList(context, onTopBarTap,
        abbNeatCurrentTabProvider, neatMainTopBarPointAutoScrollController);
    ...
    // 대배너 부분 관련 수치
    final double neatMainScreenLargeBannerWidth = screenSize.width * (393 / referenceWidth); // 대배너 이미지 너비
    final double neatMainScreenLargeBannerHeight = screenSize.width * (378 / referenceWidth); // 대배너 이미지 높이
    final double neatMainLargeBannerViewHeight = screenSize.width * (378 / referenceWidth);; // 대배너 화면 세로 비율

    // 소배너 부분 관련 수치
    final double neatMainScreenSmallBannerWidth = screenSize.width * (361 / referenceWidth); // 소배너 이미지 너비
    final double neatMainScreenSmallBannerHeight = screenSize.width * (90 / referenceWidth); // 소배너 이미지 높이
    final double neatMainScreenSmallBannerViewHeight = screenSize.width * (90 / referenceWidth); // 소배너 화면 세로 비율
    ...
    return Scaffold(
      body: Stack(
        children: [
          CustomScrollView(
            controller: neatMainScreenPointScrollController, // 스크롤 컨트롤러 연결
            slivers: <Widget>[
                    ...
                    return Padding(
                      ...
                      child: Column(
                        children: [
                          ...
                          CommonCardView(
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                height: neatMainLargeBannerViewHeight,
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  currentPageProvider:
                                  abbNeatMainLargeBannerPageProvider, // 큰 배너 페이지의 상태 제공자를 전달함
                                  ...
                                  abbAllLargeBannerImagesProvider, // 배너 이미지의 상태 제공자를 전달함
                                  // 배너를 탭했을 때 실행할 함수를 전달
                                  onPageTap: (context, index) =>
                                  // 대배너 클릭 시 호출할 함수 abbOnLargeBannerTap 실행
                                  abbOnLargeBannerTap(
                                      context, // 현재 화면의 컨텍스트를 전달함
                                      index, // 클릭된 배너의 인덱스를 전달함
                                      // abbAllLargeBannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                      ref.watch(abbAllLargeBannerImagesProvider).value ?? [],
                                      ref // Provider의 참조를 전달함
                                  ),
                                  width: neatMainScreenLargeBannerWidth, // 큰 배너 섹션의 너비를 설정함
                                  height: neatMainScreenLargeBannerHeight, // 큰 배너 섹션의 높이를 설정함
                          ...
                          CommonCardView(
                            content: Container(
                              ...
                              child: SizedBox(
                                ...
                                height: neatMainScreenSmallBannerViewHeight,
                                ...
                                child: buildCommonBannerPageViewSection<
                                  ...
                                  ref: ref, // 상태 관리를 위한 참조 전달함
                                  currentPageProvider:
                                  abbNeatMainSmall1BannerPageProvider, // 작은 배너 페이지의 상태 제공자를 전달함
                                  ...
                                  bannerImagesProvider:
                                  abbNeatMainSmall1BannerImagesProvider, // 작은 배너 이미지의 상태 제공자를 전달함
                                  // 배너를 탭했을 때 실행할 함수를 전달
                                  onPageTap: (context, index) =>
                                  // 소배너 클릭 시 호출할 함수 abbOnSmallBannerTap 실행
                                  abbOnSmallBannerTap(
                                      context, // 현재 화면의 컨텍스트를 전달함
                                      index, // 클릭된 배너의 인덱스를 전달함
                                      // abbNeatMainSmall1BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                                      ref.watch(abbNeatMainSmall1BannerImagesProvider).value ?? [],
                                      ref // Provider의 참조를 전달함
                                  ),
                                  width: neatMainScreenSmallBannerWidth, // 작은 배너 섹션의 너비를 설정함
                                  height: neatMainScreenSmallBannerHeight, // 작은 배너 섹션의 높이를 설정함
                          ...
                          AbbPriceAndDiscountPercentSortButtons<
                              AbbProductMainListNotifier>(
                            productListProvider: abbNeatMainProductListProvider,
                            // 니트 제품 리스트 정렬 프로바이더 전달
                            sortButtonProvider:
                            abbNeatMainSortButtonProvider, // 니트 정렬 버튼 프로바이더 전달
                          ), // 가격 및 할인 정렬 버튼 추가
                          ...
                          Consumer(
                            ...
                            builder: (context, ref, child) {
                              ...
                              final currentTab =
                              ref.watch(abbNeatCurrentTabProvider);
                              // 현재 탭: abbNeatMainProductListProvider를 구독하고 현재 선택된 탭 정보를 가져옴.
                              final productListProvider =
                                  abbNeatMainProductListProvider;
                              // 제품 리스트 제공자: abbNeatMainProductListProvider를 productListProvider 변수에 할당.
                              return AbbGeneralProductList<
                                  AbbProductMainListNotifier>(
                                // AbbGeneralProductList 반환: AbbGeneralProductList 위젯을 반환하여 화면에 제품 목록을 표시.
                                scrollController:
                                neatMainScreenPointScrollController,
                                // 스크롤 컨트롤러: neatMainScreenPointScrollController를 GeneralProductList의 scrollController로 전달.
          ...
          // buildTopButton 함수는 주어진 context와 neatMainScreenPointScrollController를 사용하여
          // 화면 상단으로 스크롤하기 위한 버튼 생성 위젯이며, common_body_parts_layout.dart 내에 있는 곳에서 재사용하여 구현한 부분
          buildTopButton(context, neatMainScreenPointScrollController),
        ],
      ),
      // 하단 탭 바 - 1번 케이스인 '홈','장바구니', '발주내역', '마이페이지' 버튼이 UI로 구현됨.
      bottomNavigationBar: buildCommonBottomNavigationBar(
          ref.watch(tabIndexProvider), ref, context, 5, 1, scrollController: neatMainScreenPointScrollController),
...
// _AbbCardiganMainScreenState 클래스 끝
