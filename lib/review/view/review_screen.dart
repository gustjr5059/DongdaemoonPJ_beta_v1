...
// ReviewMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class ReviewMainScreen extends ConsumerStatefulWidget {
  const ReviewMainScreen({Key? key}) : super(key: key);

  @override
  _ReviewMainScreenState createState() => _ReviewMainScreenState();
}
...
class _ReviewMainScreenState extends ConsumerState<ReviewMainScreen>
  ...
  // ref.read(reviewScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  void initState() {
    ...
    // // 사용자가 스크롤할 때마다 현재의 스크롤 위치를 reviewScreenPointScrollController에 저장하는 코드
    // // 상단 탭바 버튼 클릭 시, 해당 섹션으로 화면 이동하는 위치를 저장하는거에 해당 부분도 추가하여
    // // 사용자가 앱을 종료하거나 다른 화면으로 이동한 후 돌아왔을 때 마지막으로 본 위치로 자동으로 스크롤되도록 함.
    // reviewScreenPointScrollController.addListener(_updateScrollPosition);
  ...
  void dispose() {
    ...
    reviewScreenPointScrollController.dispose(); // ScrollController 해제
