...
// '커플룩' 카테고리의 레이아웃을 구성하는 위젯, Riverpod의 ConsumerWidget을 상속받음
class CoupleLayout extends ConsumerWidget {
  const CoupleLayout({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      appBar: buildCommonAppBar('커플룩', context), // '커플룩' 제목의 AppBar, common_parts.dart에서 재사용
      body: Column(
        children: [
          Container(
          ...
          Expanded(
            // '커플 옷 내용'을 표시하는 부분. 실제 애플리케이션에서는 여기에 컨텐츠를 동적으로 로드하여 표시
            child: Center(child: Text('커플 옷 내용')),
          ),
        ],
      ),
