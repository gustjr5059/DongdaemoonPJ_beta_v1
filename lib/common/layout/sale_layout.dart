...
// 'SALE' 카테고리의 레이아웃을 구성하는 위젯, Riverpod의 ConsumerWidget을 상속받음
class SaleLayout extends ConsumerWidget {
  const SaleLayout({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      appBar: buildCommonAppBar('SALE', context), // 'SALE' 제목의 AppBar, common_parts.dart에서 재사용
      body: Column(
        children: [
          Container(
          ...
          Expanded(
            // '세일 옷 내용'을 표시하는 부분. 실제 애플리케이션에서는 여기에 컨텐츠를 동적으로 로드하여 표시
            child: Center(child: Text('세일 옷 내용')),
          ),
        ],
      ),
