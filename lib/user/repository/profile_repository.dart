...
class ProfileRepository {
  ...
  Future<Map<String, dynamic>?> getUserInfoByEmail(String email) async { // 이메일을 통해 사용자 정보를 가져오는 비동기 함수를 정의.
    try {
      print('Fetching user info for email: $email'); // 이메일을 통해 사용자 정보를 가져오기 시작함을 알림.
      ...
      print('Number of documents found: ${querySnapshot.docs.length}'); // 검색된 문서의 수를 출력.

      if (querySnapshot.docs.isNotEmpty) { // 쿼리 결과에 문서가 하나 이상 있는지 확인.
        print('User document found for email: $email'); // 해당 이메일에 대한 사용자 문서를 찾았음을 알림.
        return querySnapshot.docs.first.data() as Map<String, dynamic>?; // 첫 번째 문서의 데이터를 Map<String, dynamic> 형으로 반환.
      } else {
        print('No user document found for email: $email'); // 해당 이메일에 대한 사용자 문서를 찾지 못했음을 알림.
        return null; // 문서가 없으면 null을 반환.
      }
    } catch (e) { // 오류가 발생하면 catch 블록이 실행.
      print('Error fetching user data for email $email: $e'); // 오류 메시지를 콘솔에 출력.
