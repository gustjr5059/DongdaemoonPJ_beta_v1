...
class ProfileRepository {
  ...
  // 이메일을 통해 사용자 정보를 가져오는 비동기 함수 정의
  Future<Map<String, dynamic>?> getUserInfoByEmail(String email) async {
    try {
      // 이메일을 통해 사용자 정보를 가져오기 시작함을 알림
      print('이메일로 사용자 정보 가져오는 중: $email');
      ...
      // 검색된 문서의 수를 출력
      print('검색된 문서 수: ${querySnapshot.docs.length}');

      // 쿼리 결과에 문서가 하나 이상 있는지 확인
      if (querySnapshot.docs.isNotEmpty) {
        // 해당 이메일에 대한 사용자 문서를 찾았음을 알림
        print('해당 이메일의 사용자 문서를 찾음: $email');
        // 첫 번째 문서의 데이터를 Map<String, dynamic> 형으로 반환
        return querySnapshot.docs.first.data() as Map<String, dynamic>?;
      } else {
        // 해당 이메일에 대한 사용자 문서를 찾지 못했음을 알림
        print('해당 이메일의 사용자 문서를 찾지 못함: $email');
        return null; // 문서가 없으면 null을 반환
      }
    } catch (e) {
      // 오류 발생 시 오류 메시지를 출력
      print('이메일 $email의 사용자 데이터를 가져오는 중 오류 발생: $e');
