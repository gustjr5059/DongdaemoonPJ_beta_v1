...
class _SignUpScreenState extends ConsumerState<SignUpScreen>
  ...
  final TextEditingController _phoneController = TextEditingController(); // 휴대폰 번호 입력을 위한 컨트롤러
  final TextEditingController _passwordController = TextEditingController(); // 비밀번호 입력을 위한 컨트롤러
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        ...
                        _buildEditableRow(context, '아이디(이메일)', _emailController,
                            _emailFocusNode, "이메일 형식으로 입력해 주세요."),
                        SizedBox(height: interval1Y),
                        _buildEditableRow(
                          context,
                          '비밀번호',
                          ...
                          _passwordFocusNode,
                          "비밀번호를 입력해 주세요.",
                          isPassword: true, // 비밀번호 입력임을 표시
                        ),
                        SizedBox(height: interval1Y),
                        _buildEditableRow(context, '이름', _nameController,
                            _nameFocusNode, "'성'을 붙여서 이름을 기입해 주세요."),
                        SizedBox(height: interval1Y),
                        _buildEditableRow(context, '휴대폰 번호', _phoneController,
                            _phoneNumberFocusNode, "'-'를 붙여서 연락처를 기입해 주세요."),
  ...
  Future<void> _signUp(SignUpInfoRepository signUpInfoRepository) async {
    ...
    try {
      ...
      showCustomSnackBar(
        context,
        '회원가입이 완료되었습니다!',
        duration: Duration(milliseconds: 1500), // 1.5초로 설정
      );
        ...
        Row(
          children: [
            Transform.scale(
              ...
              child: Checkbox(
                value: value,
                activeColor: SOFTGREEN60_COLOR, // 활성화된 색상
                onChanged: onChanged,
              ),
            ),
            Text(
  ...
  Widget _buildEditableRow(
      ...
      FocusNode focusNode,
      String hintText, {
        bool isPassword = false, // === 변경/추가된 부분 === (비밀번호 여부)
      }) {
    // MediaQuery로 기기의 화면 크기를 동적으로 가져옴
    final Size screenSize = MediaQuery.of(context).size;
                  ...
                  child: Container(
                    ...
                    child: GestureDetector(
                      ...
                      child: TextField(
                        ...
                        maxLines: isPassword ? 1 : null, // 비밀번호 필드는 한 줄만 허용
                        // 최대 줄 수 설정
                        onChanged: (value) {
