...
class _EasyLoginAosScreenState extends ConsumerState<EasyLoginAosScreen>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (mounted && !isNavigatedToSignUp) {
        ...
        _listenGoogleLoginState(
            context, ref.watch(googleSignInNotifierProvider));
        // Naver 로그인 상태를 확인하고 처리
  ...
  void _listenGoogleLoginState(BuildContext context, GoogleSignInState state) {
    ...
    if (state.isLoginSuccess) {
      // 상태 초기화 후 화면 이동
      ref.read(googleSignInNotifierProvider.notifier).state =
          GoogleSignInState();
  ...
  Widget build(BuildContext context) {
    ...
    // ----- Positioned, Align에 의해 화면 스크롤 시, 배경 제외한 빈 화면이 나오는 이슈 해결한 로직 시작 부분
    // ---  갤럭시 Z플립 화면 분할 케이스(화면 세로 길이가 줄어드는 형태) 고려한 사이즈 시작 부분
    // 비율을 기반으로 동적으로 크기와 위치 설정
            ...
            child: LayoutBuilder(
              builder: (context, constraints) {
                return SingleChildScrollView(
                  physics: BouncingScrollPhysics(), // AOS에서 자연스러운 스크롤
