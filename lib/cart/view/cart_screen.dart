...
class _CartMainScreenState extends ConsumerState<CartMainScreen> with WidgetsBindingObserver {
  ...
  // ScrollController가 여러 ScrollView에 attach 되어서 ScrollController가 동시에 여러 ScrollView에서 사용될 때 발생한 문제를 해결한 방법
  // => late로 변수 선언 / 해당 변수를 초기화(initState()) / 해당 변수를 해제 (dispose())
  late ScrollController cartTopBarPointAutoScrollController;
  ...
  void _onScroll() {
    ...
    ref.read(cartCurrentTabProvider.notifier).state = currentIndex;
...
// '가을'이나 '겨울' 탭이 활성화될 때 자동 스크롤
  ...
  void initState() {
    ...
    cartTopBarPointAutoScrollController = ScrollController();
    ...
    _largeBannerPageController = PageController(initialPage: ref.read(cartLargeBannerPageProvider));
  ...
  void dispose() {
    ...
    cartTopBarPointAutoScrollController.dispose(); // ScrollController 해제
  ...
  Widget build(BuildContext context) {
    ...
    void onTopBarTap(int index) {
      ...
      ref.read(cartScrollPositionProvider.notifier).state = scrollToPosition;
    }
                            ...
                            CommonCardView(
                              ...
                              content: SizedBox(
                                ...
                                child: buildCommonBannerPageViewSection<AllLargeBannerImage>(
                                  context: context, // 현재 빌드 컨텍스트를 전달
                                  ref: ref, // ref 전달 (Provider나 상태 관리에 사용)
                                  currentPageProvider: cartLargeBannerPageProvider, // 현재 페이지를 관리하는 Provider
                                  pageController: _largeBannerPageController, // 페이지 컨트롤러를 전달
                                  bannerAutoScroll: _largeBannerAutoScroll, // 배너 자동 스크롤 기능 전달
                                  bannerLinks: largeBannerLinks, // 배너 링크들을 전달
                                  bannerImagesProvider: allLargeBannerImagesProvider, // 배너 이미지들을 관리하는 Provider
                                  onPageTap: _onLargeBannerTap, // 배너 탭(클릭) 시 실행할 함수 전달
                                ),
                              ),
