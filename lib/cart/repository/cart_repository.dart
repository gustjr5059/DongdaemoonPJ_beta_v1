...
class CartItemRepository {
  ...
  CartItemRepository({required this.firestore, required this.storage}); // 생성자에서 firestore와 storage를 초기화
  ...
  Future<void> addToCartItem(ProductContent product, String? selectedColorText, String? selectedColorUrl, String? selectedSize, int quantity) async {
    ...
    if (product.thumbnail != null) { // 썸네일 이미지가 있을 경우
      final thumbnailUrl = await uploadImage(product.thumbnail!, '$storagePath/thumbnails'); // 썸네일 이미지를 업로드하고 URL을 가져옴
      data['thumbnails'] = thumbnailUrl; // 데이터를 업데이트
    }

    // 선택한 색상 이미지 저장
    if (selectedColorUrl != null) { // 선택한 색상 이미지가 있을 경우
      final colorImageUrl = await uploadImage(selectedColorUrl, '$storagePath/selected_color'); // 색상 이미지를 업로드하고 URL을 가져옴
  ...
  Future<List<Map<String, dynamic>>> getCartItems() async {
    final querySnapshot = await firestore.collection('cart_item').get(); // 'cart_item' 컬렉션의 모든 문서를 가져옴
    return querySnapshot.docs.map((doc) => doc.data()).toList(); // 각 문서의 데이터를 List로 변환하여 반환
  }
}
