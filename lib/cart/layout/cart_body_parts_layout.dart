...
class CartItemsList extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    //     screenSize.height * (17 / referenceHeight); // 할인 퍼센트 글꼴 크기 설정함
    // final double cartlistDiscountPriceFontSize =
    //     screenSize.height * (17 / referenceHeight); // 할인 가격 글꼴 크기 설정함
    // final double cartlistSelectedColorTextFontSize =
    //     screenSize.height * (16 / referenceHeight); // 선택된 색상 텍스트 글꼴 크기 설정함
    ...
    //     screenSize.width * (0 / referenceHeight); // 삭제 버튼 세로 위치 설정함
    // // 수량 업데이트 부분 설정
    ...
    // final double cartlist4X = screenSize.width * (20 / referenceWidth);
    ...
    final double cartlistCardViewWidth =
        screenSize.width * (393 / referenceWidth); // 화면 가로 비율에 맞게 설정함
    final double cartlistCardViewHeight = 320; // 화면 세로 비율에 맞게 설정함
    final double cartlistCardViewPaddingX =
        screenSize.width * (15 / referenceWidth); // 가로 패딩을 화면 비율에 맞게 설정함
    // 썸네일 이미지 크기 설정
    ...
    final double cartlistDeleteBtn1Y = 0; // 삭제 버튼 세로 위치 설정함
    // 수량 업데이트 부분 설정
    final double cartlistUpdateItemQuantityBtnSize = 20; // 수량 업데이트 버튼 크기 설정함
    final double cartlistUpdateItemQuantityBtnFontSize = 16; // 수량 업데이트 버튼 글꼴 크기 설정함
    final double cartlistUpdateItemQuantityWidth = 80; // 수량 업데이트 버튼 너비 설정함

    // 텍스트 데이터 간 너비 및 높이 설정
    final double cartlist1X = 12; // 텍스트 간 가로 여백 설정함
    final double cartlist2X = 19; // 텍스트 간 가로 여백 설정함
    final double cartlist3X = 30; // 텍스트 간 가로 여백 설정함
    final double cartlist4X = 20; // 텍스트 간 가로 여백 설정함
    final double cartlist1Y = 6; // 텍스트 간 세로 여백 설정함
    final double interval1X = 80;
    final double interval2X = 16;

    // 직접입력 버튼 수치
    final double directInsertBtnFontSize = 13;
    ...
    return cartItems.isEmpty
      ...
      children: cartItems.map((cartItem) {
        ...
        final int originalPrice =
            cartItem['original_price']?.round() ?? 0;
        // 상품의 할인 가격을 정수형으로 변환, 값이 없을 경우 빈 값으로 설정
        final int discountPrice =
            cartItem['discount_price']?.round() ?? 0;
        ...
        final product = ProductContent(
          ...
          maxStockQuantity: cartItem['max_stock_quantity'] ??
              10001, // max_stock_quantity가 없을 경우 기본값 10001 사용
        );
          ...
          child: Container(
            ...
            child: CommonCardView(
                ...
                children: [
                        ...
                        child: ElevatedButton(
                          ...
                          onPressed: () async {
                            await showSubmitAlertDialog(
                              ...
                              content:
                              '상품을 삭제하시겠습니까?', // 대화상자의 내용을 경고 메시지로 설정함
                                  ...
                                  try {
                                    ...
                                    if (itemId != null) {
                                      ...
                                      showCustomSnackBar(context,
                                          '상품이 장바구니 목록에서 삭제되었습니다.');
                                    } else {
                                      // 유효하지 않은 상품 ID 메시지를 표시함
                                      showCustomSnackBar(
                                          context, '삭제하는 중 오류가 발생했습니다.');
                                    }
                                  } catch (e) {
                          ...
                          style: ElevatedButton.styleFrom(
                            padding: EdgeInsets.symmetric(
                                horizontal: cartlistDeleteBtn1X,
                                vertical: cartlistDeleteBtn1Y),
                            // 버튼의 크기를 패딩으로 설정함
                  ...
                  Padding(
                    padding: EdgeInsets.only(
                        left: cartlistDataPaddingX,
                        bottom: cartlistDataPaddingY),
                    child: Text(
                              ...
                              children: [
                                ...
                                cartItem['selected_color_image'] !=
                                  ...
                                  'selected_color_image'] ??
                                      '',
                                  height:
                                  cartlistSelctedColorImageDataHeight,
                                  // width:
                                  //     cartlistSelctedColorImageDataWidth,
                                  fit: BoxFit.cover,
                                  // 이미지 로드 실패 시 아이콘 표시
                      ...
                      children: [
                        ...
                        IconButton(
                          icon: Icon(Icons.remove,
                              size: cartlistUpdateItemQuantityBtnSize),
                          // '-' 아이콘 설정
                          onPressed: () {
                        ...
                        Container(
                          width:
                          cartlistUpdateItemQuantityWidth, // 수량 표시 넓이
                          alignment: Alignment.center, // 텍스트를 중앙에 정렬함
                          child: Text(
                            numberFormat
                                .format(cartItem['selected_count'] ?? ''),
                            // 선택된 수량을 형식에 맞게 표시함
                            style: TextStyle(
                              fontSize:
                        ...
                        IconButton(
                          ...
                          // '+' 아이콘 설정
                          onPressed: () {
                            if (cartItem['selected_count'] <
                                product.maxStockQuantity - 1) {
                              // maxStockQuantity 확인
                              ref
                                  .read(cartItemsProvider.notifier)
                                  .updateItemQuantity(
                              ...
                              showCustomSnackBar(context,
                                  '최대 수량을 초과했습니다. 최대 수량: ${numberFormat.format(product.maxStockQuantity - 1)}개'); // 알림 메시지 표시
                            }
                          },
                        ),
                        ...
                        Container(
                          ...
                          child: ElevatedButton(
                            onPressed: () async {
                              ...
                              TextEditingController(); // 텍스트 입력 컨트롤러 초기화
                              ...
                              await showSubmitAlertDialog(
                                ...
                                contentWidget: Material(
                                  ...
                                  child: TextField(
                                    ...
                                    onChanged: (value) {
                                      input =
                                          value; // 입력된 값을 input 변수에 저장
                                    },
                                    decoration: InputDecoration(
                                      focusedBorder: UnderlineInputBorder(
                                        borderSide: BorderSide(
                                            color:
                                            BLACK_COLOR), // 포커스 시 테두리 색상 설정
                                      ),
                                    ),
                                  ),
                                ),
                                    ...
                                    if (input.isNotEmpty) {
                                      ...
                                      if (enteredQuantity > 0 &&
                                        ...
                                        ref
                                            ...
                                            .updateItemQuantity(
                                            cartItem['id'],
                                            enteredQuantity); // 수량 업데이트
                                      } else if (enteredQuantity < 1) {
                                        showCustomSnackBar(context,
                                            '1개 이상의 수량을 입력해주세요.'); // 최소 수량 알림
                                      } else {
                                        showCustomSnackBar(context,
                                            '최대 수량을 초과했습니다. 최대 수량: ${numberFormat.format(product.maxStockQuantity - 1)}개'); // 최대 수량 초과 알림
                                      }
                                      Navigator.of(context)
                                          .pop(); // 대화창 닫기
                                    }
                            ...
                            style: ElevatedButton.styleFrom(
                              ...
                              backgroundColor: Theme.of(context)
                                  .scaffoldBackgroundColor, // 버튼 배경색 설정
                              side: BorderSide(
                                  color: GRAY65_COLOR), // 버튼 테두리 색상 설정
                            ),
                            child: Text(
                              '직접입력', // 버튼 텍스트
                              style: TextStyle(
              ...
              margin: EdgeInsets.only(
                  left: cartlistCardViewPaddingX,
                  right: cartlistCardViewPaddingX),
            ),
          ),
        );
      }).toList(),
