...
// AdminReviewMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AdminReviewMainScreen extends ConsumerStatefulWidget {
  const AdminReviewMainScreen({Key? key}) : super(key: key);

  @override
  _AdminReviewMainScreenState createState() => _AdminReviewMainScreenState();
}

// _AdminReviewMainScreenState 클래스 시작
// _AdminReviewMainScreenState 클래스는 AdminReviewMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AdminReviewMainScreenState extends ConsumerState<AdminReviewMainScreen>
    with WidgetsBindingObserver {
  // 사용자 인증 상태 변경을 감지하는 스트림 구독 객체임.
  // 이를 통해 사용자 로그인 또는 로그아웃 상태 변경을 실시간으로 감지하고 처리할 수 있음.
  StreamSubscription<User?>? authStateChangesSubscription;

  // adminReviewScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(adminReviewScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // adminReviewScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  late ScrollController adminReviewScreenPointScrollController; // 스크롤 컨트롤러 선언
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (adminReviewScreenPointScrollController.hasClients) {
  ...
  void dispose() {
    ...
    adminReviewScreenPointScrollController.dispose(); // ScrollController 해제
  ...
  Widget build(BuildContext context) {
    ...
    final double reviewAppBarTitleWidth =
        screenSize.width * (240 / referenceWidth);
    final double reviewAppBarTitleHeight = 22;
    final double reviewAppBarTitleX = screenSize.width * (5 / referenceWidth);
    final double reviewAppBarTitleY = 11;
    ...
    return Scaffold(
      body: Stack(
        children: [
          CustomScrollView(
            controller: adminReviewScreenPointScrollController,
            slivers: <Widget>[
              SliverAppBar(
          ...
          buildTopButton(context, adminReviewScreenPointScrollController),
        ],
      ),
