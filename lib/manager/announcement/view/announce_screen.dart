...
// ManagerAnnounceMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class ManagerAnnounceMainScreen extends ConsumerStatefulWidget {
  const ManagerAnnounceMainScreen({Key? key}) : super(key: key);

  @override
  _ManagerAnnounceMainScreenState createState() => _ManagerAnnounceMainScreenState();
}

// _ManagerAnnounceMainScreenState 클래스 시작
// _ManagerAnnounceMainScreenState 클래스는 AnnounceMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _ManagerAnnounceMainScreenState extends ConsumerState<ManagerAnnounceMainScreen>
  ...
  // managerAnnounceScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  late ScrollController managerAnnounceScreenPointScrollController; // 스크롤 컨트롤러 선언
  ...
  void initState() {
    ...
    managerAnnounceScreenPointScrollController = ScrollController();
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (managerAnnounceScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(managerAnnounceScrollPositionProvider);
        // managerAnnounceScreenPointScrollController.jumpTo 메서드를 사용하여 스크롤 위치를 savedScrollPosition으로 즉시 이동함.
        // 이는 스크롤 애니메이션이나 다른 복잡한 동작 없이 바로 지정된 위치로 점프함.
        managerAnnounceScreenPointScrollController.jumpTo(savedScrollPosition);
      }
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(managerAnnounceScrollPositionProvider.notifier).state = 0;
      }
    });
  ...
  void dispose() {
    ...
    managerAnnounceScreenPointScrollController.dispose(); // ScrollController 해제
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Stack(
        children: [
          CustomScrollView(
            controller: managerAnnounceScreenPointScrollController,
            slivers: <Widget>[
              SliverAppBar(
          ...
          buildTopButton(context, managerAnnounceScreenPointScrollController),
        ],
      ),
...
// _ManagerAnnounceScreenState 클래스 끝
