...
import '../provider/orderlist_state_provider.dart'; // 공통 UI 컴포넌트 파일
...
// AdminOrderListDetailScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class AdminOrderListDetailScreen extends ConsumerStatefulWidget {
  final String orderNumber; // orderNumber 매개변수
  final String userEmail; // 선택된 발주자 이메일을 전달받는 필드

  // 생성자에서 orderNumber를 필수 매개변수로 받도록 설정
  const AdminOrderListDetailScreen({
    Key? key,
    required this.orderNumber, // orderNumber를 required로 설정
    required this.userEmail, // userEmail을 required로 설정
  }) : super(key: key);

  @override
  _AdminOrderListDetailScreenState createState() =>
      _AdminOrderListDetailScreenState();
}

// _AdminOrderListDetailScreenState 클래스 시작
// _AdminOrderListDetailScreenState 클래스는 AdminOrderListDetailScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _AdminOrderListDetailScreenState
    extends ConsumerState<AdminOrderListDetailScreen>
...
// 가져온 이메일을 매개변수로 활용하여 Firestore 쿼리 등에 사용할 수 있음
  ...
  StreamSubscription<firebase_auth.User?>? authStateChangesSubscription;
  ...
  // adminOrderListDetailScrollPositionProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  late ScrollController
      adminOrderListDetailScreenPointScrollController; // 스크롤 컨트롤러 선언
  ...
  void initState() {
    ...
    adminOrderListDetailScreenPointScrollController = ScrollController();
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (adminOrderListDetailScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition =
            ref.read(adminOrderListDetailScrollPositionProvider);
        // adminOrderListDetailScreenPointScrollController.jumpTo 메서드를 사용하여 스크롤 위치를 savedScrollPosition으로 즉시 이동함.
        // 이는 스크롤 애니메이션이나 다른 복잡한 동작 없이 바로 지정된 위치로 점프함.
        adminOrderListDetailScreenPointScrollController
            .jumpTo(savedScrollPosition);
      }

      // tabIndexProvider의 상태를 하단 탭 바 내 버튼과 매칭이 되면 안되므로 0~3이 아닌 -1로 매핑
      // -> 발주내역 상세 화면 초기화 시, 하단 탭 바 내 모든 버튼 비활성화
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(adminOrderListDetailScrollPositionProvider.notifier).state =
  ...
  Widget build(BuildContext context) {
    ...
    final double orderlistDtAppBarTitleWidth =
        screenSize.width * (240 / referenceWidth);
    final double orderlistDtAppBarTitleHeight = 22;
    final double orderlistDtAppBarTitleX =
        screenSize.width * (5 / referenceWidth);
    final double orderlistDtAppBarTitleY = 11;

    // body 부분 데이터 내용의 전체 패딩 수치
    final double orderlistDtPadding1Y = 10;

    // 이전화면으로 이동 아이콘 관련 수치 동적 적용
    final double orderlistDtChevronIconWidth =
        screenSize.width * (24 / referenceWidth);
    final double orderlistDtChevronIconHeight = 24;
    final double orderlistDtChevronIconX =
        screenSize.width * (10 / referenceWidth);
    final double orderlistDtChevronIconY = 8;
    ...
    final orderlistDetailItem = ref.watch(adminOrderlistDetailItemProvider(
        Tuple2(widget.userEmail, widget.orderNumber)));
    ...
    return Scaffold(
      body: Stack(
        children: [
          CustomScrollView(
            controller: adminOrderListDetailScreenPointScrollController,
            // 스크롤 컨트롤러 연결
            slivers: <Widget>[
                        ...
                        : SliverList(
                            delegate: SliverChildBuilderDelegate(
                              (BuildContext context, int index) {
                                return Column(
                                  children: [
                                    AdminOrderListDetailItemWidget(
          ...
          // buildTopButton 함수는 주어진 context와 adminOrderListDetailScreenPointScrollController를 사용하여
          // 화면 상단으로 스크롤하기 위한 버튼 생성 위젯이며, common_body_parts_layout.dart 내에 있는 곳에서 재사용하여 구현한 부분
          buildTopButton(
              context, adminOrderListDetailScreenPointScrollController),
        ],
      ),
      bottomNavigationBar: buildCommonBottomNavigationBar(
          ref.watch(tabIndexProvider), ref, context, 5, 1,
          scrollController: adminOrderListDetailScreenPointScrollController),
    );
    // ------ 화면구성 끝
  }
...
// _AdminOrderListDetailScreenState 클래스 끝
