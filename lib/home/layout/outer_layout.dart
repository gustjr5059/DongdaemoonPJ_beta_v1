...
// '아우터' 카테고리의 레이아웃을 나타내는 위젯, Riverpod의 ConsumerWidget을 상속받음
class OuterLayout extends ConsumerWidget {
  const OuterLayout({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: buildCommonAppBar('아우터', context), // '아우터' 제목의 AppBar, 공통 UI 컴포넌트 재사용
      body: Column(
        children: [
          Container(
          ...
          Expanded(
            // '아우터 옷 내용'을 표시하는 부분. 실제 애플리케이션에서는 여기에 컨텐츠를 동적으로 로드하여 표시
            child: Center(child: Text('아우터 옷 내용')),
          ),
        ],
      ),
