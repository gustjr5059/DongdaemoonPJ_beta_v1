...
import '../../common/provider/common_state_provider.dart'; // 앱 전반의 상태 관리를 위한 provider 파일
import '../../common/view/common_parts.dart'; // 앱에서 재사용 가능한 공통 UI 컴포넌트 파일

// '원피스' 카테고리의 레이아웃을 나타내는 위젯, Riverpod의 ConsumerWidget을 상속받음
class OnepieceLayout extends ConsumerWidget {
  const OnepieceLayout({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: buildCommonAppBar('원피스', context), // '원피스' 제목의 AppBar, 공통 UI 컴포넌트 재사용
      body: Column(
        children: [
          Container(
          ...
          Expanded(
            // '원피스 옷 내용'을 표시하는 부분. 실제 애플리케이션에서는 여기에 컨텐츠를 동적으로 로드하여 표시
            child: Center(child: Text('원피스 옷 내용')),
          ),
        ],
      ),
