...
class _JeanMainScreenState extends ConsumerState<JeanMainScreen> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    pageController = PageController(initialPage: ref.read(jeanMainBannerPageProvider));
      ...
      return asyncBannerImages.when(
        ...
        data: (List<String> imageUrls) {
          ...
          return buildBannerPageView(
            ...
            currentPageProvider: jeanMainBannerPageProvider, // 현재 페이지 인덱스를 관리하기 위한 provider(detailBannerPageProvider와 분리하여 디테일 화면의 페이지 뷰의 페이지 인덱스와 따로 관리)
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            ...
            buildHorizontalDocumentsList(ref, docIds1, '청바지', context),// 'alpha', 'apple', 'cat' 관련 데이터를 가로로 한줄 표시되도록 정렬하여 구현
            SizedBox(height: 20), // 간격을 추가
            // 텍스트 위에 회색선을 추가
            Divider(
              color: Colors.grey, // 선의 색상을 회색으로 지정
            ...
            SizedBox(height: 20), // 간격을 추가
            // Firestore 문서 데이터를 가로로 배열하여 표시하는 부분
            buildHorizontalDocumentsList(ref, docIds2, '청바지', context),// 'flutter', 'github', 'samsung' 관련 데이터를 가로로 한줄 표시되도록 정렬하여 구현
            SizedBox(height: 20), // 간격을 추가
            // 텍스트 위에 회색선을 추가
            Divider(
              color: Colors.grey, // 선의 색상을 회색으로 지정
