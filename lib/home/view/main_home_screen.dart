...
// MainHomeScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
...
// _MainHomeScreenState 클래스 시작
// _MainHomeScreenState 클래스는 MainHomeScreen 위젯의 상태를 관리함.
...
class _MainHomeScreenState extends ConsumerState<MainHomeScreen>
  ...
  GlobalKey _getSectionKey(int index) {
    switch (index) {
      ...
      default:
        return sectionEventKey; // 기본값으로 '이벤트' 섹션의 GlobalKey를 반환함.
    }
  }
  ...
  int _determineCurrentTabIndex(BuildContext context) {
    ...
    checkVisibility(1, sectionEventKey); // '이벤트' 섹션의 가시성 확인
  ...
  Widget build(BuildContext context) {
    ...
    final largeBannerWidget = buildCommonBannerPageViewSection<AllLargeBannerImage>(
      ...
      bannerImagesProvider: homeLargeBannerImagesProvider,
      onPageTap: (context, index) =>
          onLargeBannerTap(context, index, ref.watch(homeLargeBannerImagesProvider).value ?? [], ref),
    ...
    final small1BannerWidget = buildCommonBannerPageViewSection<AllSmallBannerImage>(
      ...
      onPageTap: (context, index) =>
          onSmallBannerTap(context, index, ref.watch(homeSmall1BannerImagesProvider).value ?? [], ref),
                ...
                flexibleSpace: FlexibleSpaceBar(
                  ...
                  background: Container(
                    ...
                    child: buildCommonAppBar(
                      ...
                      // 여기서 이벤트 이미지 클릭 시 onEventImageTap(1) 호출
                      onEventImageTap: () => onEventImageTap(1),
                    ),
                  ),
                ),
                leading: null,
