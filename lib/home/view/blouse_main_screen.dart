...
class _BlouseMainScreenState extends ConsumerState<BlouseMainScreen> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    pageController = PageController(initialPage: ref.read(blouseMainBannerPageProvider));

    // 배너의 자동 스크롤 기능을 초기화함.
    bannerAutoScrollClass = BannerAutoScrollClass(
      pageController: pageController,
      currentPageProvider: blouseMainBannerPageProvider,
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(blouseMainBannerPageProvider.notifier).state = 0;
      }
    });
  ...
  Widget build(BuildContext context) {
      ...
      return asyncBannerImages.when(
        ...
        data: (List<String> imageUrls) {
          ...
          return buildBannerPageView(
            ...
            currentPageProvider: blouseMainBannerPageProvider, // 현재 페이지 인덱스를 관리하기 위한 provider(detailBannerPageProvider와 분리하여 디테일 화면의 페이지 뷰의 페이지 인덱스와 따로 관리)
    ...
    return Scaffold(
      appBar: buildCommonAppBar('블라우스 메인', context), // 공통으로 사용되는 AppBar를 가져옴.
      body: SingleChildScrollView(
        child: Column(
          children: [
            ...
            buildCommonMidScrollCategoryButtons(context, onMidCategoryTap, "블라우스"),
            SizedBox(height: 20), // 간격을 추가
            // 이벤트 상품 섹션 제목을 표시
            Text('🛍️ 이벤트 상품',
                style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            // Firestore 문서 데이터를 가로로 배열하여 표시하는 부분
            buildHorizontalDocumentsList(ref, docIds1, '블라우스', context),// 'alpha', 'apple', 'cat' 관련 데이터를 가로로 한줄 표시되도록 정렬하여 구현
            buildHorizontalDocumentsList(ref, docIds2, '블라우스', context),// 'flutter', 'github', 'samsung' 관련 데이터를 가로로 한줄 표시되도록 정렬하여 구현
          ],
        ),
      ),
