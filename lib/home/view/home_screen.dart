...
class _HomeMainScreenState extends ConsumerState<HomeMainScreen> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    pageController = PageController(initialPage: ref.read(homeMainBannerPageProvider));
    ...
    bannerAutoScrollClass = BannerAutoScrollClass(
      ...
      currentPageProvider: homeMainBannerPageProvider,
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(homeMainBannerPageProvider.notifier).state = 0;
      }
    });
      ...
      return asyncBannerImages.when(
        ...
        data: (List<String> imageUrls) {
          ...
          return buildBannerPageView(
            ...
            currentPageProvider: homeMainBannerPageProvider, // 현재 페이지 인덱스를 관리하기 위한 provider(detailBannerPageProvider와 분리하여 디테일 화면의 페이지 뷰의 페이지 인덱스와 따로 관리)
