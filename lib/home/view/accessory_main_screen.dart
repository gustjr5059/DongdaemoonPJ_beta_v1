...
class _AccessoryMainScreenState extends ConsumerState<AccessoryMainScreen> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    pageController = PageController(initialPage: ref.read(accessoryMainBannerPageProvider));

    // 배너의 자동 스크롤 기능을 초기화함.
    bannerAutoScrollClass = BannerAutoScrollClass(
      pageController: pageController,
      currentPageProvider: accessoryMainBannerPageProvider,
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(accessoryMainBannerPageProvider.notifier).state = 0;
      }
    });
      ...
      return asyncBannerImages.when(
        ...
        data: (List<String> imageUrls) {
          ...
          return buildBannerPageView(
            ...
            currentPageProvider: accessoryMainBannerPageProvider, // 현재 페이지 인덱스를 관리하기 위한 provider(detailBannerPageProvider와 분리하여 디테일 화면의 페이지 뷰의 페이지 인덱스와 따로 관리)
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            ...
            buildHorizontalDocumentsList(ref, docIds1, '악세서리', context),// 'alpha', 'apple', 'cat' 관련 데이터를 가로로 한줄 표시되도록 정렬하여 구현
            buildHorizontalDocumentsList(ref, docIds2, '악세서리', context),// 'flutter', 'github', 'samsung' 관련 데이터를 가로로 한줄 표시되도록 정렬하여 구현
          ],
        ),
      ),
