...
class PantsDetailProductScreen extends ConsumerStatefulWidget {
  ...
  const PantsDetailProductScreen({Key? key, required this.fullPath}) : super(key: key);
  @override
  _PantsDetailProductScreenState createState() => _PantsDetailProductScreenState();
}

// _PantsDetailProductScreenState 클래스 시작
// _PantsDetailProductScreenState 클래스는 PantsDetailProductScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _PantsDetailProductScreenState extends ConsumerState<PantsDetailProductScreen> with WidgetsBindingObserver {
  ...
  late ScrollController pantsDetailProductScreenPointScrollController; // 스크롤 컨트롤러 선언

  // ------ 스크롤 위치를 업데이트하기 위한 '_updateScrollPosition' 함수 관련 구현 내용 시작
  // 사용자가 앱을 종료하거나 다른 화면으로 이동한 후 돌아왔을 때 마지막으로 본 위치로 자동으로 스크롤되도록 함.
  void _updateScrollPosition() {
    // 'pantsDetailProductScreenPointScrollController'에서 현재의 스크롤 위치(offset)를 가져와서 'currentScrollPosition' 변수에 저장함.
    double currentScrollPosition = pantsDetailProductScreenPointScrollController.offset;

    // 'ref'를 사용하여 'pantsDetailScrollPositionProvider'의 notifier를 읽어옴.
    // 읽어온 notifier의 'state' 값을 'currentScrollPosition'으로 설정함.
    // 이렇게 하면 앱의 다른 부분에서 해당 스크롤 위치 정보를 참조할 수 있게 됨.
    ref.read(pantsDetailScrollPositionProvider.notifier).state = currentScrollPosition;
  }
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (pantsDetailProductScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(pantsDetailScrollPositionProvider);
        ...
        pantsDetailProductScreenPointScrollController.jumpTo(savedScrollPosition);
      }
    });
    // 사용자가 스크롤할 때마다 현재의 스크롤 위치를 pantsDetailProductScreenPointScrollController에 저장하는 코드
    // 상단 탭바 버튼 클릭 시, 해당 섹션으로 화면 이동하는 위치를 저장하는거에 해당 부분도 추가하여
    // 사용자가 앱을 종료하거나 다른 화면으로 이동한 후 돌아왔을 때 마지막으로 본 위치로 자동으로 스크롤되도록 함.
    pantsDetailProductScreenPointScrollController.addListener(_updateScrollPosition);
  ...
  void dispose() {
    ...
    // 'pantsDetailProductScreenPointScrollController'의 리스너 목록에서 '_updateScrollPosition' 함수를 제거함.
    // 이는 '_updateScrollPosition' 함수가 더 이상 스크롤 이벤트에 반응하지 않도록 설정함.
    pantsDetailProductScreenPointScrollController.removeListener(_updateScrollPosition);
    pantsDetailProductScreenPointScrollController.dispose(); // ScrollController 해제
  ...
  Widget build(BuildContext context) {
    ...
    final productContent = ref.watch(pantsProdDetailFirestoreDataProvider(widget.fullPath));
    ...
    return Scaffold(
      body: Stack(
        children: [
          CustomScrollView(
            controller: pantsDetailProductScreenPointScrollController, // 스크롤 컨트롤러 연결
            slivers: <Widget>[
          ...
          buildTopButton(context, pantsDetailProductScreenPointScrollController),
        ],
      ),
    );
    // ------ 화면구성 끝
