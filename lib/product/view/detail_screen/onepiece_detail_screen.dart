...
// OnepieceDetailProductScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class OnepieceDetailProductScreen extends ConsumerStatefulWidget {
  final String fullPath;
  const OnepieceDetailProductScreen({Key? key, required this.fullPath}) : super(key: key);
  @override
  _OnepieceDetailProductScreenState createState() => _OnepieceDetailProductScreenState();
}

// _OnepieceDetailProductScreenState 클래스 시작
// _OnepieceDetailProductScreenState 클래스는 OnepieceDetailProductScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _OnepieceDetailProductScreenState extends ConsumerState<OnepieceDetailProductScreen> with WidgetsBindingObserver {

  // 사용자 인증 상태 변경을 감지하는 스트림 구독 객체임.
  // 이를 통해 사용자 로그인 또는 로그아웃 상태 변경을 실시간으로 감지하고 처리할 수 있음.
  StreamSubscription<User?>? authStateChangesSubscription;

  // onepieceDetailScrollPositionProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(onepieceDetailScrollPositionProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // onepieceDetailScrollPositionProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  late ScrollController onepieceDetailProductScreenPointScrollController; // 스크롤 컨트롤러 선언

  // ------ 스크롤 위치를 업데이트하기 위한 '_updateScrollPosition' 함수 관련 구현 내용 시작
  // 사용자가 앱을 종료하거나 다른 화면으로 이동한 후 돌아왔을 때 마지막으로 본 위치로 자동으로 스크롤되도록 함.
  void _updateScrollPosition() {
    // 'onepieceDetailProductScreenPointScrollController'에서 현재의 스크롤 위치(offset)를 가져와서 'currentScrollPosition' 변수에 저장함.
    double currentScrollPosition = onepieceDetailProductScreenPointScrollController.offset;

    // 'ref'를 사용하여 'onepieceDetailScrollPositionProvider'의 notifier를 읽어옴.
    // 읽어온 notifier의 'state' 값을 'currentScrollPosition'으로 설정함.
    // 이렇게 하면 앱의 다른 부분에서 해당 스크롤 위치 정보를 참조할 수 있게 됨.
    ref.read(onepieceDetailScrollPositionProvider.notifier).state = currentScrollPosition;
  }
  ...
  void initState() {
    ...
    onepieceDetailProductScreenPointScrollController = ScrollController();
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (onepieceDetailProductScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(onepieceDetailScrollPositionProvider);
        // onepieceDetailProductScreenPointScrollController.jumpTo 메서드를 사용하여 스크롤 위치를 savedScrollPosition으로 즉시 이동함.
        // 이는 스크롤 애니메이션이나 다른 복잡한 동작 없이 바로 지정된 위치로 점프함.
        onepieceDetailProductScreenPointScrollController.jumpTo(savedScrollPosition);
      }
    });
    // 사용자가 스크롤할 때마다 현재의 스크롤 위치를 neatDetailProductScreenPointScrollController에 저장하는 코드
    // 상단 탭바 버튼 클릭 시, 해당 섹션으로 화면 이동하는 위치를 저장하는거에 해당 부분도 추가하여
    // 사용자가 앱을 종료하거나 다른 화면으로 이동한 후 돌아왔을 때 마지막으로 본 위치로 자동으로 스크롤되도록 함.
    onepieceDetailProductScreenPointScrollController.addListener(_updateScrollPosition);
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(onepieceDetailScrollPositionProvider.notifier).state = 0;
      }
    });
  ...
  void dispose() {
    ...
    // 'onepieceDetailProductScreenPointScrollController'의 리스너 목록에서 '_updateScrollPosition' 함수를 제거함.
    // 이는 '_updateScrollPosition' 함수가 더 이상 스크롤 이벤트에 반응하지 않도록 설정함.
    onepieceDetailProductScreenPointScrollController.removeListener(_updateScrollPosition);
    onepieceDetailProductScreenPointScrollController.dispose(); // ScrollController 해제
  ...
  Widget build(BuildContext context) {
    ...
    final productContent = ref.watch(onepieceDetailProdFirestoreDataProvider(widget.fullPath));
    ...
    return Scaffold(
      body: Stack(
        children: [
          CustomScrollView(
            controller: onepieceDetailProductScreenPointScrollController, // 스크롤 컨트롤러 연결
            slivers: <Widget>[
              ...
              SliverAppBar(
                ...
                title: buildCommonAppBar( // 공통 AppBar 빌드
                  ...
                  ref: ref, // 참조(ref) 전달
                  title: '원피스 상세', // AppBar의 제목을 '원피스 상세'로 설정
          ...
          // buildTopButton 함수는 주어진 context와 onepieceDetailProductScreenPointScrollController를 사용하여
          // 화면 상단으로 스크롤하기 위한 버튼 생성 위젯이며, common_body_parts_layout.dart 내에 있는 곳에서 재사용하여 구현한 부분
          buildTopButton(context, onepieceDetailProductScreenPointScrollController),
        ],
      ),
    );
    // ------ 화면구성 끝
...
// _OnepieceDetailProductScreenState 클래스 끝
