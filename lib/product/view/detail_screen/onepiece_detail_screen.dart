...
class OnepieceDetailProductScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final productContent = ref.watch(onepieceProdDetailFirestoreDataProvider(fullPath));
    // Firestore 데이터 제공자를 통해 특정 문서 ID(docId)의 상품 데이터를 구독.
    return Scaffold(
      // Scaffold를 사용하여 기본적인 머티리얼 디자인 레이아웃을 제공.
      appBar: buildCommonAppBar(
          context: context, title: '원피스 상세', pageBackButton: true),
      // 앱바를 구성하며, '원피스 상세'라는 제목과 페이지 뒤로 가기 버튼을 포함.
