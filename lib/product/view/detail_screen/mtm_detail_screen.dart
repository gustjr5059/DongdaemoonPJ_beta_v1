...
class _MtmDetailProductScreenState extends ConsumerState<MtmDetailProductScreen>
  ...
  void initState() {
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(mtmDetailScrollPositionProvider.notifier).state = 0; // mtmDetailScrollPositionProvider의 상태를 0으로 설정
  ...
  void _onScroll() {
    ...
    if (mtmDetailProductScreenPointScrollController.position.pixels ==
        mtmDetailProductScreenPointScrollController.position
            .maxScrollExtent) {
  ...
  Widget build(BuildContext context) {
    ...
    final productContent =
        ref.watch(mtmDetailProdFirestoreDataProvider(widget.fullPath));
    final productReviews = ref.watch(productReviewProvider(widget.fullPath)); // 리뷰 데이터를 가져옴

    print("MtmDetailProductScreen: Loading screen for product path: ${widget.fullPath}"); // 디버깅 메시지 추가
                      ...
                      return Column(
                        children: [
                          productContent.when(
                              ...
                              return Column(
                                children: [
                                  ...
                                  productReviews.when(
                                    ...
                                    error: (error, _) {
                                      print("MtmDetailProductScreen: Error loading reviews: $error"); // 오류 메시지 추가
                            ...
                            error: (error, _) {
                              print("MtmDetailProductScreen: Error loading product data: $error"); // 오류 메시지 추가
      ...
      bottomNavigationBar: productContent.when(
        data: (product) {
          return buildCommonBottomNavigationBar(
              ref.watch(tabIndexProvider), ref, context, 5, 2, product: product, scrollController: mtmDetailProductScreenPointScrollController);
        },
