...
class CoatDetailProductScreen extends ConsumerStatefulWidget {
  ...
  @override
  _CoatDetailProductScreenState createState() =>
      _CoatDetailProductScreenState();
}

// _CoatDetailProductScreenState 클래스 시작
// _CoatDetailProductScreenState 클래스는 CoatDetailProductScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _CoatDetailProductScreenState
    extends ConsumerState<CoatDetailProductScreen> with WidgetsBindingObserver {
  ...
  void _onScroll() {
    ...
    if (coatDetailProductScreenPointScrollController.position.pixels ==
        coatDetailProductScreenPointScrollController.position
            .maxScrollExtent) {
  ...
  Widget build(BuildContext context) {
    ...
    print("CoatDetailProductScreen: Loading screen for product path: ${widget.fullPath}"); // 디버깅 메시지 추가
                      ...
                      return Column(
                        children: [
                          productContent.when(
                              ...
                              return Column(
                                children: [
                                  ...
                                  productReviews.when(
                                    ...
                                    error: (error, _) {
                                      print("CoatDetailProductScreen: Error loading reviews: $error"); // 오류 메시지 추가
                            ...
                            error: (error, _) {
                              print("CoatDetailProductScreen: Error loading product data: $error"); // 오류 메시지 추가
      ...
      bottomNavigationBar: productContent.when(
        data: (product) {
          return buildCommonBottomNavigationBar(
              ref.watch(tabIndexProvider), ref, context, 5, 2, product: product, scrollController: coatDetailProductScreenPointScrollController);
        },
