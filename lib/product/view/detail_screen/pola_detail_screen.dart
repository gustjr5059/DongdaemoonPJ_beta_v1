...
class _PolaDetailProductScreenState
  ...
  void initState() {
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(polaDetailScrollPositionProvider.notifier).state = 0; // polaDetailScrollPositionProvider의 상태를 0으로 설정
  ...
  void _onScroll() {
    ...
    if (polaDetailProductScreenPointScrollController.position.pixels ==
        polaDetailProductScreenPointScrollController.position
            .maxScrollExtent) {
  ...
  Widget build(BuildContext context) {
    ...
    print("PolaDetailProductScreen: 제품 경로에 대한 화면 로드 중: ${widget.fullPath}"); // 디버깅 메시지 추가
                  ...
                  delegate: SliverChildBuilderDelegate(
                      ...
                      return Column(
                        children: [
                          productContent.when(
                            data: (product) {
                              print("PolaDetailProductScreen: 제품 데이터 로드 완료"); // 디버깅 메시지 추가
                              return Column(
                                children: [
                                  ...
                                  productReviews.when(
                                    data: (reviewsContent) {
                                      print("PolaDetailProductScreen: ${reviewsContent.length} reviews가 로드됨"); // 디버깅 메시지 추가
