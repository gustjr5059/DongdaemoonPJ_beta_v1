...
class PolaDetailProductScreen extends ConsumerWidget {
  final String docId; // 문서 ID를 저장할 변수 선언
  const PolaDetailProductScreen({Key? key, required this.docId}) : super(key: key); // 생성자 수정
...
//           polaDetailBannerPageProvider, // 현재 페이지의 인덱스를 관리하는 StateProvider(polaDetailBannerPageProvider와 분리하여 홈 화면의 배너 페이지 뷰의 페이지 인덱스와 따로 관리)
...
// Firestore 데이터 제공자를 통해 특정 문서 ID(docId)의 상품 데이터를 구독.
    return Scaffold(
      ...
      appBar: buildCommonAppBar(
          context: context, title: '폴라티 상세', pageBackButton: true),
