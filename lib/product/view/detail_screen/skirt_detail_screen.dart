...
class SkirtDetailProductScreen extends ConsumerStatefulWidget {
  ...
  const SkirtDetailProductScreen({
...
class _SkirtDetailProductScreenState
  ...
  // skirtDetailScrollPositionProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(skirtDetailScrollPositionProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // skirtDetailScrollPositionProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void initState() {
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(skirtDetailScrollPositionProvider.notifier).state = 0; // skirtDetailScrollPositionProvider의 상태를 0으로 설정
  ...
  void _onScroll() {
    ...
    if (skirtDetailProductScreenPointScrollController.position.pixels ==
        skirtDetailProductScreenPointScrollController.position
            .maxScrollExtent) {
  ...
  Widget build(BuildContext context) {
    ...
    print("SkirtDetailProductScreen: Loading screen for product path: ${widget.fullPath}"); // 디버깅 메시지 추가
                      ...
                      return Column(
                        children: [
                          productContent.when(
                              ...
                              return Column(
                                children: [
                                  ...
                                  productReviews.when(
                                    ...
                                    error: (error, _) {
                                      print("SkirtDetailProductScreen: Error loading reviews: $error"); // 오류 메시지 추가
                            ...
                            error: (error, _) {
                              print("SkirtDetailProductScreen: Error loading product data: $error"); // 오류 메시지 추가
      ...
      bottomNavigationBar: productContent.when(
        data: (product) {
          return buildCommonBottomNavigationBar(
              ref.watch(tabIndexProvider), ref, context, 5, 2, product: product, scrollController: skirtDetailProductScreenPointScrollController);
        },
...
// _SkirtDetailProductScreenState 클래스 끝
