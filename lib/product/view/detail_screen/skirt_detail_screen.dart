...
class _SkirtDetailProductScreenState extends ConsumerState<SkirtDetailProductScreen> with WidgetsBindingObserver {
  ...
  void _updateScrollPosition() {
    // 'skirtDetailProductScreenPointScrollController'에서 현재의 스크롤 위치(offset)를 가져와서 'currentScrollPosition' 변수에 저장함.
    double currentScrollPosition = skirtDetailProductScreenPointScrollController.offset;

    // 'ref'를 사용하여 'skirtDetailScrollPositionProvider'의 notifier를 읽어옴.
    // 읽어온 notifier의 'state' 값을 'currentScrollPosition'으로 설정함.
    // 이렇게 하면 앱의 다른 부분에서 해당 스크롤 위치 정보를 참조할 수 있게 됨.
    ref.read(skirtDetailScrollPositionProvider.notifier).state = currentScrollPosition;
  }
  ...
  void initState() {
    ...
    // 사용자가 스크롤할 때마다 현재의 스크롤 위치를 skirtDetailProductScreenPointScrollController에 저장하는 코드
    // 상단 탭바 버튼 클릭 시, 해당 섹션으로 화면 이동하는 위치를 저장하는거에 해당 부분도 추가하여
    // 사용자가 앱을 종료하거나 다른 화면으로 이동한 후 돌아왔을 때 마지막으로 본 위치로 자동으로 스크롤되도록 함.
    skirtDetailProductScreenPointScrollController.addListener(_updateScrollPosition);
  ...
  void dispose() {
    ...
    // 'skirtDetailProductScreenPointScrollController'의 리스너 목록에서 '_updateScrollPosition' 함수를 제거함.
    // 이는 '_updateScrollPosition' 함수가 더 이상 스크롤 이벤트에 반응하지 않도록 설정함.
    skirtDetailProductScreenPointScrollController.removeListener(_updateScrollPosition);
  ...
  Widget build(BuildContext context) {
    ...
    final productContent = ref.watch(skirtProdDetailFirestoreDataProvider(widget.fullPath));
