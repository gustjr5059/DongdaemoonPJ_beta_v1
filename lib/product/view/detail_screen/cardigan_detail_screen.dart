...
class CardiganDetailProductScreen extends ConsumerStatefulWidget {
  ...
  _CardiganDetailProductScreenState createState() =>
      _CardiganDetailProductScreenState();
}

// _CardiganDetailProductScreenState 클래스 시작
// _CardiganDetailProductScreenState 클래스는 CardiganDetailProductScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _CardiganDetailProductScreenState
    extends ConsumerState<CardiganDetailProductScreen>
    with WidgetsBindingObserver {
  // 사용자 인증 상태 변경을 감지하는 스트림 구독 객체임.
  // 이를 통해 사용자 로그인 또는 로그아웃 상태 변경을 실시간으로 감지하고 처리할 수 있음.
  StreamSubscription<User?>? authStateChangesSubscription;

  // cardiganDetailScrollPositionProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(cardiganDetailScrollPositionProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // cardiganDetailScrollPositionProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (cardiganDetailProductScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition =
            ref.read(cardiganDetailScrollPositionProvider);
        // cardiganDetailProductScreenPointScrollController.jumpTo 메서드를 사용하여 스크롤 위치를 savedScrollPosition으로 즉시 이동함.
  ...
  void _onScroll() {
    ...
    if (cardiganDetailProductScreenPointScrollController.position.pixels ==
        cardiganDetailProductScreenPointScrollController.position
            .maxScrollExtent) {
  ...
  Widget build(BuildContext context) {
    ...
    print("CardiganDetailProductScreen: Loading screen for product path: ${widget.fullPath}"); // 디버깅 메시지 추가
                      ...
                      return Column(
                        children: [
                          productContent.when(
                              ...
                              return Column(
                                children: [
                                  ...
                                  productReviews.when(
                                    ...
                                    error: (error, _) {
                                      print("CardiganDetailProductScreen: Error loading reviews: $error"); // 오류 메시지 추가
                            ...
                            error: (error, _) {
                              print("CardiganDetailProductScreen: Error loading product data: $error"); // 오류 메시지 추가
      ...
      bottomNavigationBar: productContent.when(
        data: (product) {
          return buildCommonBottomNavigationBar(
              ref.watch(tabIndexProvider), ref, context, 5, 2, product: product, scrollController: cardiganDetailProductScreenPointScrollController);
        },
...
// _CardiganDetailProductScreenState 클래스 끝
