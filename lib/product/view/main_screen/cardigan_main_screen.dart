...
class _CardiganMainScreenState extends ConsumerState<CardiganMainScreen> with WidgetsBindingObserver {
  ...
  // cardiganMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(cardiganMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // cardiganMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void _updateScrollPosition() {
    ...
    double currentScrollPosition = cardiganMainScreenPointScrollController.offset;

    // 'ref'를 사용하여 'cardiganMainScrollPositionProvider'의 notifier를 읽어옴.
    // 읽어온 notifier의 'state' 값을 'currentScrollPosition'으로 설정함.
    // 이렇게 하면 앱의 다른 부분에서 해당 스크롤 위치 정보를 참조할 수 있게 됨.
    ref.read(cardiganMainScrollPositionProvider.notifier).state = currentScrollPosition;
  }
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (cardiganMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(cardiganMainScrollPositionProvider);
    ...
    // 'cardiganMainLargeBannerPageProvider'에서 초기 페이지 인덱스를 읽어옴
    _largeBannerPageController = PageController(initialPage: ref.read(cardiganMainLargeBannerPageProvider));

    // 큰 배너를 자동으로 스크롤하는 기능 초기화
    _largeBannerAutoScroll = BannerAutoScrollClass(
      pageController: _largeBannerPageController,
      currentPageProvider: cardiganMainLargeBannerPageProvider,
    ...
    _small1BannerPageController = PageController(initialPage: ref.read(cardiganMainSmall1BannerPageProvider));

    // 작은 배너1을 자동으로 스크롤하는 기능 초기화
    _small1BannerAutoScroll = BannerAutoScrollClass(
      pageController: _small1BannerPageController,
      currentPageProvider: cardiganMainSmall1BannerPageProvider,
      itemCount: bannerImageCount, // 총 배너 이미지 개수 전달
    );
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(cardiganMainLargeBannerPageProvider.notifier).state = 0;
        ref.read(cardiganMainSmall1BannerPageProvider.notifier).state = 0;
        ref.read(cardiganMainScrollPositionProvider.notifier).state = 0.0; // 로그아웃 시 cardiganMainScrollPositionProvider가 초기화되므로, 재로그인 시 초기 스크롤 위치에서 시작됨. 하지만 상품 데이터는 유지됨.
  ...
  void dispose() {
    ...
    cardiganMainScreenPointScrollController.removeListener(_updateScrollPosition);

    cardiganMainScreenPointScrollController.dispose(); // ScrollController 해제

    cardiganMainTopBarPointAutoScrollController.dispose(); // ScrollController 해제
  ...
  // cardiganMainProductListProvider 관련 카테고리 인덱스를 받아 해당 카테고리 이름을 반환하는 함수
  ...
  Widget build(BuildContext context) {
    ...
    void onTopBarTap(int index) {
      ref.read(cardiganCurrentTabProvider.notifier).state = index; // 현재 탭 인덱스를 업데이트
      ...
      Future.delayed(Duration.zero,() {
        ref.read(cardiganMainProductListProvider.notifier).reset(); // 상태 초기화
        ref.read(cardiganMainProductListProvider.notifier).fetchInitialProducts(_getCategory(index)); // 선택한 탭에 해당하는 초기 제품 가져오기 호출
      });
    }
    // 상단 탭 바를 구성하는 리스트 뷰를 가져오는 위젯
    // (common_parts.dart의 buildTopBarList 재사용 후 topBarList 위젯으로 재정의)
    Widget topBarList = buildTopBarList(context, onTopBarTap, cardiganCurrentTabProvider, cardiganMainTopBarPointAutoScrollController);
                          ...
                          children: [
                            ...
                            CommonCardView(
                              content: SizedBox(
                                ...
                                height: 30,
                                // 카드뷰 내용으로 buildCommonBannerPageViewSection 재사용하여 구현함.
                                child: buildCommonBannerPageViewSection<CardiganMainSmall1BannerImage>(
                                  context: context,
                                  ref: ref,
                                  currentPageProvider: cardiganMainSmall1BannerPageProvider,
                                  pageController: _small1BannerPageController,
                                  ...
                                  bannerImagesProvider: cardiganMainSmall1BannerImagesProvider,
                            ...
                            PriceAndDiscountPercentSortButtons(
                              productListProvider: cardiganMainProductListProvider, // 가디건 제품 리스트 정렬 프로바이더 전달
                              sortButtonProvider: cardiganMainSortButtonProvider, // 가디건 정렬 버튼 프로바이더 전달
                            ), // 가격 및 할인 정렬 버튼 추가
                            SizedBox(height: 3), // 3의 높이를 가진 간격 추가
                            Consumer(
                              ...
                              builder: (context, ref, child) {
                                ...
                                final currentTab = ref.watch(cardiganCurrentTabProvider);
                                // 현재 탭: cardiganCurrentTabProvider를 구독하고 현재 선택된 탭 정보를 가져옴.
                                final productListProvider = cardiganMainProductListProvider;
                                ...
                                return GeneralProductList(
                                  ...
                                  scrollController: cardiganMainScreenPointScrollController,
