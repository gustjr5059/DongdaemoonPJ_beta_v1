...
class _JeanMainScreenState extends ConsumerState<JeanMainScreen>
  ...
  // jeanMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(jeanMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // jeanMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void _updateScrollPosition() {
    ...
    // 'ref'를 사용하여 'jeanMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (jeanMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(jeanMainScrollPositionProvider);
        ...
        int savedTabIndex = ref.read(jeanCurrentTabProvider);
        if (savedTabIndex >= 6) {
          double offset =
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(jeanMainLargeBannerPageProvider.notifier).state = 0;
        ref.read(jeanMainSmall1BannerPageProvider.notifier).state = 0;
        ref.read(jeanMainScrollPositionProvider.notifier).state =
            0.0; // 로그아웃 시 jeanMainScrollPositionProvider가 초기화되므로, 재로그인 시 초기 스크롤 위치에서 시작됨. 하지만 상품 데이터는 유지됨.
  ...
  // jeanMainProductListProvider 관련 카테고리 인덱스를 받아 해당 카테고리 이름을 반환하는 함수
  ...
  Widget build(BuildContext context) {
    ...
    void onTopBarTap(int index) {
      ref.read(jeanCurrentTabProvider.notifier).state = index; // 현재 탭 인덱스를 업데이트
      ...
      Future.delayed(Duration.zero, () {
        ref.read(jeanMainProductListProvider.notifier).reset(); // 상태 초기화
        ref.read(jeanMainProductListProvider.notifier).fetchInitialProducts(
    ...
    Widget topBarList = buildTopBarList(context, onTopBarTap,
        jeanCurrentTabProvider, jeanMainTopBarPointAutoScrollController);
    ...
    final small1BannerWidget = buildCommonBannerPageViewSection<AllSmallBannerImage>(
      ...
      bannerImagesProvider: jeanMainSmall1BannerImagesProvider,
      onPageTap: (context, index) =>
          onSmallBannerTap(context, index, ref.watch(jeanMainSmall1BannerImagesProvider).value ?? [], ref),
                      ...
                      return Padding(
                        ...
                        child: Column(
                          children: [
                              ...
                              content: Container(
                                ...
                                child: SizedBox(
                                  ...
                                  child: buildCommonBannerPageViewSection<
                                    ...
                                    currentPageProvider:
                                    jeanMainLargeBannerPageProvider, // 큰 배너 페이지의 상태 제공자를 전달함
                            ...
                            SizedBox(height: interval3Y), // 높이를 10으로 설정한 간격을 추가함
                            // CommonCardView(
                            //   content: Container(
                            //     // 모서리에 반경을 주기 위한 BoxDecoration 추가함
                            //     decoration: BoxDecoration(
                            ...
                            //         ref: ref, // 상태 관리를 위한 참조 전달함
                            //         currentPageProvider:
                            //         jeanMainSmall1BannerPageProvider, // 작은 배너 페이지의 상태 제공자를 전달함
                            ...
                            //         bannerImagesProvider:
                            //         jeanMainSmall1BannerImagesProvider, // 작은 배너 이미지의 상태 제공자를 전달함
                            //         // 배너를 탭했을 때 실행할 함수를 전달
                            //         onPageTap: (context, index) =>
                            ...
                            //             context, // 현재 화면의 컨텍스트를 전달함
                            //             index, // 클릭된 배너의 인덱스를 전달함
                            //             // jeanMainSmall1BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                            //             ref.watch(jeanMainSmall1BannerImagesProvider).value ?? [],
                            //             ref // Provider의 참조를 전달함
                            //         ),
                            ...
                            PriceAndDiscountPercentSortButtons<
                              ...
                              productListProvider: jeanMainProductListProvider,
                              // 청바지 제품 리스트 정렬 프로바이더 전달
                              sortButtonProvider:
                                  jeanMainSortButtonProvider, // 청바지 정렬 버튼 프로바이더 전달
                            ), // 가격 및 할인 정렬 버튼 추가
                            SizedBox(height: interval1Y), // 3의 높이를 가진 간격 추가
                            Consumer(
                              ...
                              builder: (context, ref, child) {
                                ...
                                final currentTab =
                                    ref.watch(jeanCurrentTabProvider);
                                // 현재 탭: jeanCurrentTabProvider를 구독하고 현재 선택된 탭 정보를 가져옴.
                                final productListProvider =
                                    jeanMainProductListProvider;
                                // 제품 리스트 제공자: jeanMainProductListProvider를 productListProvider 변수에 할당.
