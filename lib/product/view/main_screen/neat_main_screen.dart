...
class _NeatMainScreenState extends ConsumerState<NeatMainScreen> with WidgetsBindingObserver {
  ...
  void _updateScrollPosition() {
    ...
    ref.read(neatMainScrollPositionProvider.notifier).state = currentScrollPosition;
  }
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (neatMainScreenPointScrollController.hasClients) {
        ...
        if (savedTabIndex >= 6) {
          double offset = neatMainTopBarPointAutoScrollController.position.maxScrollExtent;
    ...
    _largeBannerPageController = PageController(initialPage: ref.read(neatMainLargeBannerPageProvider));
    ...
    _small1BannerPageController = PageController(initialPage: ref.read(neatMainSmall1BannerPageProvider));
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(neatMainScrollPositionProvider.notifier).state = 0.0; // 로그아웃 시 neatMainScrollPositionProvider가 초기화되므로, 재로그인 시 초기 스크롤 위치에서 시작됨. 하지만 상품 데이터는 유지됨.
        ref.read(neatCurrentTabProvider.notifier).state = 0; // 니트 메인 화면 상단 탭 바 버튼 위치 인덱스를 초기화
        ref.read(neatMainProductListProvider.notifier).reset(); // 탭 관련 상품 데이터를 초기화함.
  ...
  Widget build(BuildContext context) {
    ...
    void onTopBarTap(int index) {
      ...
      Future.delayed(Duration.zero,() {
        ...
        ref.read(neatMainProductListProvider.notifier).fetchInitialProducts(_getCategory(index)); // 선택한 탭에 해당하는 초기 제품 가져오기 호출
      });
    }
    // 상단 탭 바를 구성하는 리스트 뷰를 가져오는 위젯
    // (common_parts.dart의 buildTopBarList 재사용 후 topBarList 위젯으로 재정의)
    Widget topBarList = buildTopBarList(context, onTopBarTap, neatCurrentTabProvider, neatMainTopBarPointAutoScrollController);
                          ...
                          children: [
                            ...
                            CommonCardView(
                              content: SizedBox(
                                ...
                                height: 30,
                                // 카드뷰 내용으로 buildCommonBannerPageViewSection 재사용하여 구현함.
                                child: buildCommonBannerPageViewSection<NeatMainSmall1BannerImage>(
                                  context: context,
                                  ref: ref,
                                  currentPageProvider: neatMainSmall1BannerPageProvider,
                                  pageController: _small1BannerPageController,
                            ...
                            PriceAndDiscountPercentSortButtons<ProductMainListNotifier>(
                              productListProvider: neatMainProductListProvider, // 니트 제품 리스트 정렬 프로바이더 전달
                              sortButtonProvider: neatMainSortButtonProvider, // 니트 정렬 버튼 프로바이더 전달
                            ), // 가격 및 할인 정렬 버튼 추가
                            SizedBox(height: 3), // 3의 높이를 가진 간격 추가
                            Consumer(
                              ...
                              builder: (context, ref, child) {
                                ...
                                final currentTab = ref.watch(neatCurrentTabProvider);
                                // 현재 탭: neatCurrentTabProvider를 구독하고 현재 선택된 탭 정보를 가져옴.
                                final productListProvider = neatMainProductListProvider;
                                ...
                                return GeneralProductList<ProductMainListNotifier>(
                                  ...
                                  scrollController: neatMainScreenPointScrollController,
