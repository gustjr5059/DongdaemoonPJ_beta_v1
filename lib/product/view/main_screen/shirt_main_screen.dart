...
class _ShirtMainScreenState extends ConsumerState<ShirtMainScreen> with WidgetsBindingObserver {
  ...
  void _updateScrollPosition() {
    ...
    ref.read(shirtMainScrollPositionProvider.notifier).state = currentScrollPosition;
  }
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (shirtMainScreenPointScrollController.hasClients) {
        ...
        if (savedTabIndex >= 6) {
          double offset = shirtMainTopBarPointAutoScrollController.position.maxScrollExtent;
    ...
    _largeBannerPageController = PageController(initialPage: ref.read(shirtMainLargeBannerPageProvider));
    ...
    _small1BannerPageController = PageController(initialPage: ref.read(shirtMainSmall1BannerPageProvider));
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(shirtMainProductListProvider.notifier).reset(); // 탭 관련 상품 데이터를 초기화함.
  ...
  Widget build(BuildContext context) {
    ...
    void onTopBarTap(int index) {
      ref.read(shirtCurrentTabProvider.notifier).state = index; // 현재 탭 인덱스를 업데이트
      ...
      Future.delayed(Duration.zero,() {
        ...
        ref.read(shirtMainProductListProvider.notifier).fetchInitialProducts(_getCategory(index)); // 선택한 탭에 해당하는 초기 제품 가져오기 호출
      });
    }
    // 상단 탭 바를 구성하는 리스트 뷰를 가져오는 위젯
    // (common_parts.dart의 buildTopBarList 재사용 후 topBarList 위젯으로 재정의)
    Widget topBarList = buildTopBarList(context, onTopBarTap, shirtCurrentTabProvider, shirtMainTopBarPointAutoScrollController);
                          ...
                          children: [
                            ...
                            CommonCardView(
                              content: SizedBox(
                                ...
                                height: 30,
                                // 카드뷰 내용으로 buildCommonBannerPageViewSection 재사용하여 구현함.
                                child: buildCommonBannerPageViewSection<ShirtMainSmall1BannerImage>(
                                  context: context,
                                  ref: ref,
                                  currentPageProvider: shirtMainSmall1BannerPageProvider,
                                  pageController: _small1BannerPageController,
                            ...
                            PriceAndDiscountPercentSortButtons<ProductMainListNotifier>(
                              productListProvider: shirtMainProductListProvider, // 티셔츠 제품 리스트 프로바이더 전달
                              sortButtonProvider: shirtMainSortButtonProvider, // 티셔츠 정렬 버튼 프로바이더 전달
                            ), // 가격 및 할인 정렬 버튼 추가
                            SizedBox(height: 3), // 3의 높이를 가진 간격 추가
                            Consumer(
                              ...
                              builder: (context, ref, child) {
                                ...
                                final currentTab = ref.watch(shirtCurrentTabProvider);
                                // 현재 탭: shirtCurrentTabProvider를 구독하고 현재 선택된 탭 정보를 가져옴.
                                final productListProvider = shirtMainProductListProvider;
                                ...
                                return GeneralProductList<ProductMainListNotifier>(
                                  ...
                                  scrollController: shirtMainScreenPointScrollController,
