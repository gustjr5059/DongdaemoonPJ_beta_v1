...
// 각 배너 클릭 시 연결될 웹사이트 주소를 리스트로 관리합니다.
  ...
  void initState() {
    ...
    _pageController = PageController(initialPage: ref.read(skirtMainBannerPageProvider));

    // 배너의 자동 스크롤 기능을 초기화함.
    _bannerAutoScroll = BannerAutoScrollClass(
      pageController: _pageController,
      currentPageProvider: skirtMainBannerPageProvider,
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(skirtMainBannerPageProvider.notifier).state = 0;
      }
    });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: buildCommonAppBar(context: context, title: '스커트 메인', pageBackButton: true), // 공통으로 사용되는 AppBar를 가져옴.
        ...
        child: Column(
          children: [
            ...
            SizedBox(
              ...
              child: buildLargeBannerPageViewSection(context, ref, skirtMainBannerPageProvider, _pageController, _bannerAutoScroll, bannerLinks), // 배너 페이지뷰 위젯 재사용하여 구현
            ),
            SizedBox(height: 120), // 높이 20으로 간격 설정
            ...
            buildHorizontalDocumentsList(ref, docIds1, '스커트', context),// 'alpha', 'apple', 'cat' 관련 데이터를 가로로 한줄 표시되도록 정렬하여 구현
            SizedBox(height: 20), // 간격을 추가
            // 텍스트 위에 회색선을 추가
            Divider(
              color: Colors.grey, // 선의 색상을 회색으로 지정
            ...
            SizedBox(height: 20), // 간격을 추가
            // Firestore 문서 데이터를 가로로 배열하여 표시하는 부분
            buildHorizontalDocumentsList(ref, docIds2, '스커트', context),// 'flutter', 'github', 'samsung' 관련 데이터를 가로로 한줄 표시되도록 정렬하여 구현
            SizedBox(height: 20), // 간격을 추가
            // 텍스트 위에 회색선을 추가
            Divider(
              color: Colors.grey, // 선의 색상을 회색으로 지정
