...
// SkirtMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class SkirtMainScreen extends ConsumerStatefulWidget {
  const SkirtMainScreen({Key? key}) : super(key: key);

  @override
  _SkirtMainScreenState createState() => _SkirtMainScreenState();
}

// _SkirtMainScreenState 클래스 시작
// _SkirtMainScreenState 클래스는 SkirtMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _SkirtMainScreenState extends ConsumerState<SkirtMainScreen>
  ...
  // skirtMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(skirtMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // skirtMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void _updateScrollPosition() {
    ...
    // 'ref'를 사용하여 'skirtMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (skirtMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(skirtMainScrollPositionProvider);
        ...
        int savedTabIndex = ref.read(skirtCurrentTabProvider);
        if (savedTabIndex >= 6) {
          double offset =
    ...
    // 'skirtMainLargeBannerPageProvider'에서 초기 페이지 인덱스를 읽어옴
    _largeBannerPageController =
        PageController(initialPage: ref.read(skirtMainLargeBannerPageProvider));

    // 큰 배너를 자동으로 스크롤하는 기능 초기화
    _largeBannerAutoScroll = BannerAutoScrollClass(
      pageController: _largeBannerPageController,
      currentPageProvider: skirtMainLargeBannerPageProvider,
      itemCount: bannerImageCount1, // 총 배너 이미지 개수 전달
    );

    // 작은 배너1에 대한 PageController 및 AutoScroll 초기화
    // 'skirtMainSmall1BannerPageProvider'에서 초기 페이지 인덱스를 읽어옴
    _small1BannerPageController = PageController(
        initialPage: ref.read(skirtMainSmall1BannerPageProvider));
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(skirtMainLargeBannerPageProvider.notifier).state = 0;
        ref.read(skirtMainSmall1BannerPageProvider.notifier).state = 0;
        ref.read(skirtMainScrollPositionProvider.notifier).state =
            0.0; // 로그아웃 시 skirtMainScrollPositionProvider가 초기화되므로, 재로그인 시 초기 스크롤 위치에서 시작됨. 하지만 상품 데이터는 유지됨.
  ...
  // skirtMainProductListProvider 관련 카테고리 인덱스를 받아 해당 카테고리 이름을 반환하는 함수
  ...
  Widget build(BuildContext context) {
    ...
    void onTopBarTap(int index) {
      ...
      Future.delayed(Duration.zero, () {
        ref.read(skirtMainProductListProvider.notifier).reset(); // 상태 초기화
        ref.read(skirtMainProductListProvider.notifier).fetchInitialProducts(
    ...
    Widget topBarList = buildTopBarList(context, onTopBarTap,
        skirtCurrentTabProvider, skirtMainTopBarPointAutoScrollController);
                          ...
                          children: [
                              ...
                              content: Container(
                                ...
                                child: SizedBox(
                                  ...
                                  child: buildCommonBannerPageViewSection<
                                    ...
                                    currentPageProvider:
                                    skirtMainLargeBannerPageProvider, // 큰 배너 페이지의 상태 제공자를 전달함
                            ...
                            Consumer(
                              ...
                              builder: (context, ref, child) {
                                ...
                                final currentTab =
                                    ref.watch(skirtCurrentTabProvider);
                                // 현재 탭: skirtCurrentTabProvider를 구독하고 현재 선택된 탭 정보를 가져옴.
                                final productListProvider =
                                    skirtMainProductListProvider;
