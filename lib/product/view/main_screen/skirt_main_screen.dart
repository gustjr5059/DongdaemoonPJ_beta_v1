...
class _SkirtMainScreenState extends ConsumerState<SkirtMainScreen> with WidgetsBindingObserver {
  ...
  void _updateScrollPosition() {
    ...
    ref.read(skirtMainScrollPositionProvider.notifier).state = currentScrollPosition;
  }
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (skirtMainScreenPointScrollController.hasClients) {
        ...
        if (savedTabIndex >= 6) {
          double offset = skirtMainTopBarPointAutoScrollController.position.maxScrollExtent;
    ...
    _largeBannerPageController = PageController(initialPage: ref.read(skirtMainLargeBannerPageProvider));
    ...
    _small1BannerPageController = PageController(initialPage: ref.read(skirtMainSmall1BannerPageProvider));
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(skirtMainScrollPositionProvider.notifier).state = 0.0; // 로그아웃 시 skirtMainScrollPositionProvider가 초기화되므로, 재로그인 시 초기 스크롤 위치에서 시작됨. 하지만 상품 데이터는 유지됨.
        ref.read(skirtCurrentTabProvider.notifier).state = 0; // 스커트 메인 화면 상단 탭 바 버튼 위치 인덱스를 초기화
        ref.read(skirtMainProductListProvider.notifier).reset(); // 탭 관련 상품 데이터를 초기화함.
  ...
  Widget build(BuildContext context) {
    ...
    void onTopBarTap(int index) {
      ref.read(skirtCurrentTabProvider.notifier).state = index; // 현재 탭 인덱스를 업데이트
      ...
      Future.delayed(Duration.zero,() {
        ...
        ref.read(skirtMainProductListProvider.notifier).fetchInitialProducts(_getCategory(index)); // 선택한 탭에 해당하는 초기 제품 가져오기 호출
      });
    }
    // 상단 탭 바를 구성하는 리스트 뷰를 가져오는 위젯
    // (common_parts.dart의 buildTopBarList 재사용 후 topBarList 위젯으로 재정의)
    Widget topBarList = buildTopBarList(context, onTopBarTap, skirtCurrentTabProvider, skirtMainTopBarPointAutoScrollController);
                          ...
                          children: [
                            ...
                            CommonCardView(
                              content: SizedBox(
                                ...
                                height: 30,
                                // 카드뷰 내용으로 buildCommonBannerPageViewSection 재사용하여 구현함.
                                child: buildCommonBannerPageViewSection<SkirtMainSmall1BannerImage>(
                                  context: context,
                                  ref: ref,
                                  currentPageProvider: skirtMainSmall1BannerPageProvider,
                                  pageController: _small1BannerPageController,
                            ...
                            Consumer(
                              ...
                              builder: (context, ref, child) {
                                ...
                                final currentTab = ref.watch(skirtCurrentTabProvider);
