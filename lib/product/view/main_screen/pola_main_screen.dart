...
// PolaMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class PolaMainScreen extends ConsumerStatefulWidget {
  const PolaMainScreen({Key? key}) : super(key: key);

  @override
  _PolaMainScreenState createState() => _PolaMainScreenState();
}

// _PolaMainScreenState 클래스 시작
// _PolaMainScreenState 클래스는 PolaMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _PolaMainScreenState extends ConsumerState<PolaMainScreen>
  ...
  // polaMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(polaMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  void _updateScrollPosition() {
    ...
    // 'ref'를 사용하여 'polaMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (polaMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(polaMainScrollPositionProvider);
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(polaMainScrollPositionProvider.notifier).state =
            0.0; // 로그아웃 시 polaMainScrollPositionProvider가 초기화되므로, 재로그인 시 초기 스크롤 위치에서 시작됨. 하지만 상품 데이터는 유지됨.
  ...
  // polaMainProductListProvider 관련 카테고리 인덱스를 받아 해당 카테고리 이름을 반환하는 함수
  ...
  Widget build(BuildContext context) {
    ...
    void onTopBarTap(int index) {
      ref.read(polaCurrentTabProvider.notifier).state = index; // 현재 탭 인덱스를 업데이트
      ...
      Future.delayed(Duration.zero, () {
        ref.read(polaMainProductListProvider.notifier).reset(); // 상태 초기화
        ref.read(polaMainProductListProvider.notifier).fetchInitialProducts(
    ...
    Widget topBarList = buildTopBarList(context, onTopBarTap,
        polaCurrentTabProvider, polaMainTopBarPointAutoScrollController);
    ...
    final small1BannerWidget = buildCommonBannerPageViewSection<AllSmallBannerImage>(
      ...
      bannerImagesProvider: polaMainSmall1BannerImagesProvider,
      onPageTap: (context, index) =>
          onSmallBannerTap(context, index, ref.watch(polaMainSmall1BannerImagesProvider).value ?? [], ref),
                      ...
                      return Padding(
                        ...
                        child: Column(
                          children: [
                              ...
                              content: Container(
                                ...
                                child: SizedBox(
                                  ...
                                  child: buildCommonBannerPageViewSection<
                                    ...
                                    currentPageProvider:
                                    polaMainLargeBannerPageProvider, // 큰 배너 페이지의 상태 제공자를 전달함
                            ...
                            //         ref: ref, // 상태 관리를 위한 참조 전달함
                            //         currentPageProvider:
                            //         polaMainSmall1BannerPageProvider, // 작은 배너 페이지의 상태 제공자를 전달함
                            ...
                            //         bannerImagesProvider:
                            //         polaMainSmall1BannerImagesProvider, // 작은 배너 이미지의 상태 제공자를 전달함
                            //         // 배너를 탭했을 때 실행할 함수를 전달
                            //         onPageTap: (context, index) =>
                            ...
                            //             context, // 현재 화면의 컨텍스트를 전달함
                            //             index, // 클릭된 배너의 인덱스를 전달함
                            //             // polaMainSmall1BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                            //             ref.watch(polaMainSmall1BannerImagesProvider).value ?? [],
                            //             ref // Provider의 참조를 전달함
                            //         ),
                            ...
                            PriceAndDiscountPercentSortButtons<
                              ...
                              // 폴라티 제품 리스트 프로바이더 전달
                              sortButtonProvider:
                                  polaMainSortButtonProvider, // 폴라티 정렬 버튼 프로바이더 전달
                            ), // 가격 및 할인 정렬 버튼 추가
                            SizedBox(height: interval1Y), // 3의 높이를 가진 간격 추가
                            Consumer(
                              ...
                              builder: (context, ref, child) {
                                ...
                                final currentTab =
                                    ref.watch(polaCurrentTabProvider);
                                // 현재 탭: polaCurrentTabProvider를 구독하고 현재 선택된 탭 정보를 가져옴.
                                final productListProvider =
                                    polaMainProductListProvider;
                                // 제품 리스트 제공자: polaMainProductListProvider를 productListProvider 변수에 할당.
