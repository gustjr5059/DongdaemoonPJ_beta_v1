...
class _WinterSubMainScreenState extends ConsumerState<WinterSubMainScreen> with WidgetsBindingObserver {
  ...
  late ScrollController winterSubMainScreenPointScrollController = ref.read(winterSubMainScrollControllerProvider); // 스크롤 컨트롤러 선언
  ...
  late ScrollController winterSubMainTopBarPointAutoScrollController = ScrollController();

  // => winterSubMainScreenPointScrollController는 전체 화면의 스크롤을 제어함 vs winterSubMainTopBarPointAutoScrollController는 상단 탭 바의 스크롤을 제어함.
  // => 그러므로, 서로 다른 UI 요소 제어, 다른 동작 방식, _onScroll 함수 내 다르게 사용하므로 두 컨트롤러를 병합하면 복잡성 증가하고, 동작이 충돌할 수 있어 독립적으로 제작!!
  // => winterSubMainTopBarPointAutoScrollController는 전체 화면의 UI를 담당하는게 아니므로 scaffold의 body 내 컨트롤러에 연결이 안되어도 addListener()에 _onScroll()로 연결해놓은거라 해당 기능 사용이 가능!!
  ...
  Widget build(BuildContext context) {
    ...
    Widget topBarList = buildTopBarList(context, onTopBarTap, blouseCurrentTabProvider, winterSubMainTopBarPointAutoScrollController);
