...
class PaedingSubMainScreen extends ConsumerStatefulWidget {
  const PaedingSubMainScreen({Key? key}) : super(key: key);
  @override
  _PaedingSubMainScreenState createState() => _PaedingSubMainScreenState();
}

// _PaedingMainScreenState 클래스 시작
// _PaedingMainScreenState 클래스는 BottomMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _PaedingSubMainScreenState extends ConsumerState<PaedingSubMainScreen> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(paedingMainBannerPageProvider.notifier).state = 0;
      }
    });
        ...
        child: Column(
          children: [
            ...
            SizedBox(
              ...
              child: buildLargeBannerPageViewSection(context, ref, paedingMainBannerPageProvider, _pageController, _bannerAutoScroll, bannerLinks), // 배너 페이지뷰 위젯 재사용하여 구현
            ),
