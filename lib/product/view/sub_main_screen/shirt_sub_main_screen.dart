...
class ShirtSubMainScreen extends ConsumerStatefulWidget {
  const ShirtSubMainScreen({Key? key}) : super(key: key);
  @override
  _ShirtSubMainScreenState createState() => _ShirtSubMainScreenState();
}

// _ShirtMainScreenState 클래스 시작
// _ShirtMainScreenState 클래스는 ShirtMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _ShirtSubMainScreenState extends ConsumerState<ShirtSubMainScreen> with WidgetsBindingObserver {
...
// 각 배너 클릭 시 연결될 웹사이트 주소를 리스트로 관리합니다.
  ...
  void initState() {
    ...
    _pageController = PageController(initialPage: ref.read(shirtMainBannerPageProvider));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: buildCommonAppBar(context: context, title: '티셔츠 메인', pageBackButton: true), // 공통으로 사용되는 AppBar를 가져옴.
        ...
        child: Column(
          children: [
            ...
            SizedBox(
              ...
              child: buildLargeBannerPageViewSection(context, ref, shirtMainBannerPageProvider, _pageController, _bannerAutoScroll, bannerLinks), // 배너 페이지뷰 위젯 재사용하여 구현
            ),
            SizedBox(height: 110), // 높이 20으로 간격 설정
