...
class PantsSubMainScreen extends ConsumerStatefulWidget {
  const PantsSubMainScreen({Key? key}) : super(key: key);
  @override
  _PantsSubMainScreenState createState() => _PantsSubMainScreenState();
}

// _PantsMainScreenState 클래스 시작
// _PantsMainScreenState 클래스는 PantsMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _PantsSubMainScreenState extends ConsumerState<PantsSubMainScreen> with WidgetsBindingObserver {
...
// 각 배너 클릭 시 연결될 웹사이트 주소를 리스트로 관리합니다.
  ...
  void initState() {
    ...
    _pageController = PageController(initialPage: ref.read(pantsMainBannerPageProvider));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: buildCommonAppBar(context: context, title: '팬츠 메인', pageBackButton: true), // 공통으로 사용되는 AppBar를 가져옴.
        ...
        child: Column(
          children: [
            ...
            SizedBox(
              ...
              child: buildLargeBannerPageViewSection(context, ref, pantsMainBannerPageProvider, _pageController, _bannerAutoScroll, bannerLinks), // 배너 페이지뷰 위젯 재사용하여 구현
            ),
            SizedBox(height: 90), // 높이 20으로 간격 설정
