...
// NewSubMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class NewSubMainScreen extends ConsumerStatefulWidget {
  const NewSubMainScreen({Key? key}) : super(key: key);

  @override
  _NewSubMainScreenState createState() => _NewSubMainScreenState();
}

// _NewSubMainScreenState 클래스 시작
// _NewSubMainScreenState 클래스는 NewSubMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _NewSubMainScreenState extends ConsumerState<NewSubMainScreen>
  ...
  // newSubMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(newSubMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // newSubMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void _updateScrollPosition() {
    ...
    // 'ref'를 사용하여 'newSubMainScrollPositionProvider'의 notifier를 읽어옴.
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (newSubMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(newSubMainScrollPositionProvider);
  ...
  Widget build(BuildContext context) {
    ...
    //
    // // 대배너 부분 관련 수치
    // final double autumnSubMainScreenLargeBannerWidth = screenSize.width * (393 / referenceWidth); // 대배너 이미지 너비
    // final double autumnSubMainScreenLargeBannerHeight = screenSize.height * (378 / referenceHeight); // 대배너 이미지 높이
    // final double autumnSubMainLargeBannerViewHeight =
    //     screenSize.height * (378 / referenceHeight); // 대배너 화면 세로 비율
    //
    // // 소배너 부분 관련 수치
    // final double autumnSubMainScreenSmallBannerWidth = screenSize.width * (361 / referenceWidth); // 소배너 이미지 너비
    // final double autumnSubMainScreenSmallBannerHeight = screenSize.height * (90 / referenceHeight); // 소배너 이미지 높이
    // final double autumnSubMainScreenSmallBannerViewHeight =
    ...
    final small1BannerWidget = buildCommonBannerPageViewSection<AllSmallBannerImage>(
      ...
      bannerImagesProvider: newSubMainSmall1BannerImagesProvider,
      onPageTap: (context, index) =>
          onSmallBannerTap(context, index, ref.watch(newSubMainSmall1BannerImagesProvider).value ?? [], ref),
                      ...
                      return Padding(
                        ...
                        child: Column(
                          children: [
                              ...
                              content: Container(
                                ...
                                child: SizedBox(
                                  ...
                                  child: buildCommonBannerPageViewSection<
                                    ...
                                    currentPageProvider:
                                    newSubMainLargeBannerPageProvider, // 큰 배너 페이지의 상태 제공자를 전달함
                                    ...
                                    width: newSubMainScreenLargeBannerWidth, // 배너 섹션의 너비를 설정함
                                    height: newSubMainScreenLargeBannerHeight, // 배너 섹션의 높이를 설정함
                            ...
                            //         ref: ref, // 상태 관리를 위한 참조 전달함
                            //         currentPageProvider:
                            //         newSubMainSmall1BannerPageProvider, // 작은 배너 페이지의 상태 제공자를 전달함
                            ...
                            //         bannerImagesProvider:
                            //         newSubMainSmall1BannerImagesProvider, // 작은 배너 이미지의 상태 제공자를 전달함
                            //         // 배너를 탭했을 때 실행할 함수를 전달
                            //         onPageTap: (context, index) =>
                            ...
                            //             context, // 현재 화면의 컨텍스트를 전달함
                            //             index, // 클릭된 배너의 인덱스를 전달함
                            //             // newSubMainSmall1BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                            //             ref.watch(newSubMainSmall1BannerImagesProvider).value ?? [],
                            //             ref // Provider의 참조를 전달함
                            //         ),
                            //         width: newSubMainScreenSmallBannerWidth, // 작은 배너 섹션의 너비를 설정함
                            //         height: newSubMainScreenSmallBannerHeight, // 작은 배너 섹션의 높이를 설정함
                            ...
                            PriceAndDiscountPercentSortButtons<
                              ...
                              productListProvider:
                                  newSubMainProductListProvider,
                              // 신상 제품 리스트 프로바이더 전달
                              sortButtonProvider:
                                  newSubMainSortButtonProvider, // 신상 정렬 버튼 프로바이더 전달
                            ),
                            // 가격 및 할인 정렬 버튼 추가
                            SizedBox(height: interval1Y),
