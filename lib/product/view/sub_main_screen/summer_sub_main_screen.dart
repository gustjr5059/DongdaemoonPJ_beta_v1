...
// SummerSubMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class SummerSubMainScreen extends ConsumerStatefulWidget {
  const SummerSubMainScreen({Key? key}) : super(key: key);

  @override
  _SummerSubMainScreenState createState() => _SummerSubMainScreenState();
}

// _SummerSubMainScreenState 클래스 시작
// _SummerSubMainScreenState 클래스는 SummerSubMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _SummerSubMainScreenState extends ConsumerState<SummerSubMainScreen>
  ...
  // summerSubMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(summerSubMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // summerSubMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (summerSubMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition =
            ref.read(summerSubMainScrollPositionProvider);
    ...
    _largeBannerPageController = PageController(
        initialPage: ref.read(summerSubMainLargeBannerPageProvider));

    // 큰 배너를 자동으로 스크롤하는 기능 초기화
    _largeBannerAutoScroll = BannerAutoScrollClass(
      pageController: _largeBannerPageController,
      currentPageProvider: summerSubMainLargeBannerPageProvider,
    ...
    _small1BannerPageController = PageController(
        initialPage: ref.read(summerSubMainSmall1BannerPageProvider));

    // 작은 배너1을 자동으로 스크롤하는 기능 초기화
    _small1BannerAutoScroll = BannerAutoScrollClass(
      pageController: _small1BannerPageController,
      currentPageProvider: summerSubMainSmall1BannerPageProvider,
  ...
  Widget build(BuildContext context) {
    ...
    final small1BannerWidget = buildCommonBannerPageViewSection<AllSmallBannerImage>(
      ...
      bannerImagesProvider: summerSubMainSmall1BannerImagesProvider,
      onPageTap: (context, index) =>
          onSmallBannerTap(context, index, ref.watch(summerSubMainSmall1BannerImagesProvider).value ?? [], ref),
                      ...
                      return Padding(
                        ...
                        child: Column(
                          children: [
                              ...
                              content: Container(
                                ...
                                child: SizedBox(
                                  ...
                                  child: buildCommonBannerPageViewSection<
                                    ...
                                    currentPageProvider:
                                    summerSubMainLargeBannerPageProvider, // 큰 배너 페이지의 상태 제공자를 전달함
                                    ...
                                    width: summerSubMainScreenLargeBannerWidth, // 배너 섹션의 너비를 설정함
                                    height: summerSubMainScreenLargeBannerHeight, // 배너 섹션의 높이를 설정함
                            ...
                            //         ref: ref, // 상태 관리를 위한 참조 전달함
                            //         currentPageProvider:
                            //         summerSubMainSmall1BannerPageProvider, // 작은 배너 페이지의 상태 제공자를 전달함
                            ...
                            //         bannerImagesProvider:
                            //         summerSubMainSmall1BannerImagesProvider, // 작은 배너 이미지의 상태 제공자를 전달함
                            //         // 배너를 탭했을 때 실행할 함수를 전달
                            //         onPageTap: (context, index) =>
                            ...
                            //             context, // 현재 화면의 컨텍스트를 전달함
                            //             index, // 클릭된 배너의 인덱스를 전달함
                            //             // summerSubMainSmall1BannerImagesProvider에서 대배너 이미지 리스트를 가져옴. 값이 없으면 빈 리스트를 사용함
                            //             ref.watch(summerSubMainSmall1BannerImagesProvider).value ?? [],
                            //             ref // Provider의 참조를 전달함
                            //         ),
                            //         width: summerSubMainScreenSmallBannerWidth, // 작은 배너 섹션의 너비를 설정함
                            //         height: summerSubMainScreenSmallBannerHeight, // 작은 배너 섹션의 높이를 설정함
                            ...
                            PriceAndDiscountPercentSortButtons<
                              ...
                              productListProvider:
                                  summerSubMainProductListProvider,
                              // 여름 제품 리스트 프로바이더 전달
                              sortButtonProvider:
                                  summerSubMainSortButtonProvider, // 여름 정렬 버튼 프로바이더 전달
                            ),
                            // 가격 및 할인 정렬 버튼 추가
                            SizedBox(height: interval1Y),
...
// _SummerSubMainScreenState 클래스 끝
