...
class _SummerSubMainScreenState extends ConsumerState<SummerSubMainScreen> with WidgetsBindingObserver {
  ...
  // ScrollController가 여러 ScrollView에 attach 되어서 ScrollController가 동시에 여러 ScrollView에서 사용될 때 발생한 문제를 해결한 방법
  // => late로 변수 선언 / 해당 변수를 초기화(initState()) / 해당 변수를 해제 (dispose())
  late ScrollController summerSubMainTopBarPointAutoScrollController;

  // => summerSubMainScreenPointScrollController는 전체 화면의 스크롤을 제어함 vs summerSubMainTopBarPointAutoScrollController는 상단 탭 바의 스크롤을 제어함.
  // => 그러므로, 서로 다른 UI 요소 제어, 다른 동작 방식, _onScroll 함수 내 다르게 사용하므로 두 컨트롤러를 병합하면 복잡성 증가하고, 동작이 충돌할 수 있어 독립적으로 제작!!
  // => summerSubMainTopBarPointAutoScrollController는 전체 화면의 UI를 담당하는게 아니므로 scaffold의 body 내 컨트롤러에 연결이 안되어도 addListener()에 _onScroll()로 연결해놓은거라 해당 기능 사용이 가능!!
  ...
  void initState() {
    ...
    summerSubMainTopBarPointAutoScrollController = ScrollController();
  ...
  void dispose() {
    ...
    summerSubMainTopBarPointAutoScrollController.dispose(); // ScrollController 해제
  ...
  Widget build(BuildContext context) {
    ...
    Widget topBarList = buildTopBarList(context, onTopBarTap, blouseCurrentTabProvider, summerSubMainTopBarPointAutoScrollController);
