...
class _AutumnSubMainScreenState extends ConsumerState<AutumnSubMainScreen> with WidgetsBindingObserver {
...
// 각 배너 클릭 시 연결될 웹사이트 주소를 리스트로 관리합니다.
  ...
  late ScrollController scrollController = ref.read(autumnSubMainScrollControllerProvider); // 스크롤 컨트롤러 선언
  ...
  void initState() {
    ...
    _pageController = PageController(initialPage: ref.read(neatMainBannerPageProvider));

    // 배너의 자동 스크롤 기능을 초기화함.
    _bannerAutoScroll = BannerAutoScrollClass(
      pageController: _pageController,
      currentPageProvider: neatMainBannerPageProvider,
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(neatMainBannerPageProvider.notifier).state = 0;
      }
    });
  ...
  Widget build(BuildContext context) {
    ...
    Widget topBarList = buildTopBarList(context, onTopBarTap, currentTabProvider, topBarAutoScrollController);
    ...
    return Scaffold(
      appBar: buildCommonAppBar(context: context, title: '니트 메인', pageBackButton: true), // 공통으로 사용되는 AppBar를 가져옴.
        ...
        child: Column(
          children: [
            ...
            SizedBox(
              ...
              child: buildLargeBannerPageViewSection(context, ref, neatMainBannerPageProvider, _pageController, _bannerAutoScroll, bannerLinks), // 배너 페이지뷰 위젯 재사용하여 구현
            ),
            SizedBox(height: 60), // 높이 20으로 간격 설정
