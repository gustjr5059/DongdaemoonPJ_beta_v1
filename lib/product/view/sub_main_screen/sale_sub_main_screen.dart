...
class _SaleSubMainScreenState extends ConsumerState<SaleSubMainScreen> with WidgetsBindingObserver {
  ...
  // ScrollController가 여러 ScrollView에 attach 되어서 ScrollController가 동시에 여러 ScrollView에서 사용될 때 발생한 문제를 해결한 방법
  // => late로 변수 선언 / 해당 변수를 초기화(initState()) / 해당 변수를 해제 (dispose())
  late ScrollController saleSubMainTopBarPointAutoScrollController;
  ...
  void initState() {
    ...
    saleSubMainTopBarPointAutoScrollController = ScrollController();
  ...
  void dispose() {
    ...
    saleSubMainTopBarPointAutoScrollController.dispose(); // ScrollController 해제
