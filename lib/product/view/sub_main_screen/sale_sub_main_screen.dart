...
// SaleSubMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class SaleSubMainScreen extends ConsumerStatefulWidget {
  const SaleSubMainScreen({Key? key}) : super(key: key);

  @override
  _SaleSubMainScreenState createState() => _SaleSubMainScreenState();
}

// _SaleSubMainScreenState 클래스 시작
// _SaleSubMainScreenState 클래스는 SaleSubMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _SaleSubMainScreenState extends ConsumerState<SaleSubMainScreen>
  ...
  // saleSubMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(saleSubMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // saleSubMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (saleSubMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition =
            ref.read(saleSubMainScrollPositionProvider);
    ...
    _largeBannerPageController = PageController(
        initialPage: ref.read(saleSubMainLargeBannerPageProvider));

    // 큰 배너를 자동으로 스크롤하는 기능 초기화
    _largeBannerAutoScroll = BannerAutoScrollClass(
      pageController: _largeBannerPageController,
      currentPageProvider: saleSubMainLargeBannerPageProvider,
  ...
  Widget build(BuildContext context) {
    ...
    final small1BannerWidget = buildCommonBannerPageViewSection<AllSmallBannerImage>(
      ...
      bannerImagesProvider: saleSubMainSmall1BannerImagesProvider,
      onPageTap: (context, index) =>
          onSmallBannerTap(context, index, ref.watch(saleSubMainSmall1BannerImagesProvider).value ?? [], ref),
    ...
    return Scaffold(
      ...
      bottomNavigationBar: buildCommonBottomNavigationBar(
          ref.watch(tabIndexProvider), ref, context, 5, 1, scrollController: saleSubMainScreenPointScrollController),
