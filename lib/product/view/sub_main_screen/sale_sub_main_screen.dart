...
// 각 배너 클릭 시 연결될 웹사이트 주소를 리스트로 관리합니다.
  ...
  late ScrollController scrollController = ref.read(saleSubMainScrollControllerProvider); // 스크롤 컨트롤러 선언
  ...
  void initState() {
    ...
    _pageController = PageController(initialPage: ref.read(coatMainBannerPageProvider));

    // 배너의 자동 스크롤 기능을 초기화함.
    _bannerAutoScroll = BannerAutoScrollClass(
      pageController: _pageController,
      currentPageProvider: coatMainBannerPageProvider,
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(coatMainBannerPageProvider.notifier).state = 0;
      }
    });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: buildCommonAppBar(context: context, title: '코트 메인', pageBackButton: true), // 공통으로 사용되는 AppBar를 가져옴.
        ...
        child: Column(
          children: [
            ...
            // 페이지 뷰 섹션을 표시
              ...
              child: buildLargeBannerPageViewSection(context, ref, coatMainBannerPageProvider, _pageController, _bannerAutoScroll, bannerLinks), // 배너 페이지뷰 위젯 재사용하여 구현
            ),
            SizedBox(height: 30), // 높이 20으로 간격 설정
