...
// SpringSubMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class SpringSubMainScreen extends ConsumerStatefulWidget {
  const SpringSubMainScreen({Key? key}) : super(key: key);

  @override
  _SpringSubMainScreenState createState() => _SpringSubMainScreenState();
}

// _SpringSubMainScreenState 클래스 시작
// _SpringSubMainScreenState 클래스는 SpringSubMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _SpringSubMainScreenState extends ConsumerState<SpringSubMainScreen>
  ...
  // springSubMainScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(springSubMainScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // springSubMainScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (springSubMainScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition =
            ref.read(springSubMainScrollPositionProvider);
    ...
    _largeBannerPageController = PageController(
        initialPage: ref.read(springSubMainLargeBannerPageProvider));
    ...
    _small1BannerPageController = PageController(
        initialPage: ref.read(springSubMainSmall1BannerPageProvider));
  ...
  Widget build(BuildContext context) {
    ...
    final small1BannerWidget = buildCommonBannerPageViewSection<AllSmallBannerImage>(
      ...
      bannerImagesProvider: springSubMainSmall1BannerImagesProvider,
      onPageTap: (context, index) =>
          onSmallBannerTap(context, index, ref.watch(springSubMainSmall1BannerImagesProvider).value ?? [], ref),
