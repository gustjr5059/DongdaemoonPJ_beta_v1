            ...
            children: announceItems.map((announceItem) {
              ...
              return GestureDetector(
                ...
                onTap: () {
                  if (documentId.isNotEmpty) {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => AnnounceDetailScreen(
                            documentId: documentId), // 공지사항 상세 화면으로 이동함
                      ),
                    );
                  }
                },
                ...
                child: Padding(
                  ...
                  child: Column(
                    crossAxisAlignment:
                        CrossAxisAlignment.start, // 컬럼 내에서 왼쪽 정렬을 설정함
                    children: [
...
class AnnounceDetailBodyPartsLayout extends ConsumerWidget {
  ...
  AnnounceDetailBodyPartsLayout(
      {required this.documentId}); // documentId를 받아오는 생성자임
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // 공지사항 상세 데이터를 가져옴
    final announceDetailItem =
        ref.watch(announceDetailItemProvider(documentId));
    ...
    final title = (announceDetailItem['title'] as String?) ??
        ''; // 공지사항 제목을 가져오며 없을 경우 ''로 설정함
    final timestamp =
    ...
    final contentsTextUrl = (announceDetailItem['contents_text'] as String?) ??
        ''; // 텍스트 콘텐츠 URL을 가져옴
    final contentsWebLink =
        (announceDetailItem['contents_web_link'] as String?) ??
            ''; // 웹 링크 URL을 가져옴
    final contentsImageUrl =
        (announceDetailItem['contents_image'] as String?) ?? ''; // 이미지 URL을 가져옴
    final contentsWebLinkText =
        (announceDetailItem['contents_web_link_text'] as String?) ??
            ''; // 웹 링크에 대한 텍스트를 가져옴
      ...
      child: Column(
        ...
        children: [
          ...
          if (contentsWebLink.isNotEmpty)
            GestureDetector(
              // 터치 이벤트 처리를 위한 GestureDetector 사용
              onTap: () async {
                // 웹 링크를 열기 위한 launchURL 함수를 호출함
                try {
                  final bool launched = await launchUrl(
                      Uri.parse(contentsWebLink),
          ...
          contentsImageUrl != null && contentsImageUrl != ''
              ? Image.network(
                  ...
                  width: MediaQuery.of(context)
                      .size
                      .width, // 화면 너비에 맞게 이미지 너비를 설정함
                  errorBuilder: (context, error, stackTrace) =>
                      Container(), // 이미지 로드 실패 시 빈 컨테이너를 표시
                )
