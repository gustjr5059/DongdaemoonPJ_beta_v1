...
class _OrderMainScreenState extends ConsumerState<OrderMainScreen> with WidgetsBindingObserver {
  ...
  // ScrollController가 여러 ScrollView에 attach 되어서 ScrollController가 동시에 여러 ScrollView에서 사용될 때 발생한 문제를 해결한 방법
  // => late로 변수 선언 / 해당 변수를 초기화(initState()) / 해당 변수를 해제 (dispose())
  late ScrollController orderTopBarPointAutoScrollController;

  // => orderScreenPointScrollController는 전체 화면의 스크롤을 제어함 vs orderTopBarPointAutoScrollController는 상단 탭 바의 스크롤을 제어함.
  // => 그러므로, 서로 다른 UI 요소 제어, 다른 동작 방식, _onScroll 함수 내 다르게 사용하므로 두 컨트롤러를 병합하면 복잡성 증가하고, 동작이 충돌할 수 있어 독립적으로 제작!!
  // => orderTopBarPointAutoScrollController는 전체 화면의 UI를 담당하는게 아니므로 scaffold의 body 내 컨트롤러에 연결이 안되어도 addListener()에 _onScroll()로 연결해놓은거라 해당 기능 사용이 가능!!

  // ------ 스크롤 이벤트가 발생할 때마다 호출되는 함수인 _onScroll() 내용 시작
  void _onScroll() {
    // 현재 스크롤 위치를 orderScreenPointScrollController의 offset 값으로부터 가져옴.
    double currentScroll = orderScreenPointScrollController.offset;
...
// '가을'이나 '겨울' 탭이 활성화될 때 자동 스크롤
    if (currentIndex >= 6) {
      ...
      double offset = orderTopBarPointAutoScrollController.position.maxScrollExtent;

      // 스크롤 애니메이션을 사용하여 orderTopBarPointAutoScrollController를 offset 위치로 이동
      orderTopBarPointAutoScrollController.animateTo(
      ...
      // 스크롤 애니메이션을 사용하여 orderTopBarPointAutoScrollController를 0.0 위치로 이동 (맨 처음 위치)
      orderTopBarPointAutoScrollController.animateTo(
        0.0, // 이동할 위치 (맨 처음)
        duration: Duration(milliseconds: 50), // 애니메이션 시간 (50밀리초)
        curve: Curves.easeInOut, // 애니메이션 커브 (서서히 시작하고 서서히 끝나는 곡선)
  ...
  void _updateScrollPosition() {
    // 'orderTopBarPointAutoScrollController'에서 현재의 스크롤 위치(offset)를 가져와서 'currentScrollPosition' 변수에 저장함.
    double currentScrollPosition = orderTopBarPointAutoScrollController.offset;
  ...
  void initState() {
    ...
    orderTopBarPointAutoScrollController = ScrollController();
    ...
    // orderScreenPointScrollController에 스크롤 이벤트 리스너를 추가함.
    // 이 리스너는 사용자가 스크롤할 때마다 _onScroll 함수를 호출하도록 설정됨.
    orderScreenPointScrollController.addListener(_onScroll);
  ...
  void dispose() {
    ...
    // orderScreenPointScrollController에서 _onScroll 함수를 리스너로서 제거함.
    // 이 작업은 더 이상 스크롤 이벤트에 반응하지 않도록 설정할 때 사용됨.
    orderScreenPointScrollController.removeListener(_onScroll);

    orderScreenPointScrollController.dispose(); // ScrollController 해제

    orderTopBarPointAutoScrollController.dispose(); // ScrollController 해제
  ...
  Widget build(BuildContext context) {
    ...
    void onTopBarTap(int index) {
      ...
      orderScreenPointScrollController.animateTo(
          scrollToPosition,
    ...
    Widget topBarList = buildTopBarList(context, onTopBarTap, orderCurrentTabProvider, orderTopBarPointAutoScrollController);
    ...
    return Scaffold(
      ...
      bottomNavigationBar: buildCommonBottomNavigationBar(
          ref.watch(tabIndexProvider), ref, context, 3), // 공통으로 사용되는 하단 네비게이션 바를 가져옴.
    );
    // ------ 화면구성 끝
  }
