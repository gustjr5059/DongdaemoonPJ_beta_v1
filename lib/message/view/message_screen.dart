...
// MessageMainScreen 클래스는 ConsumerWidget 상속, Riverpod를 통한 상태 관리 지원
class MessageMainScreen extends ConsumerStatefulWidget {
  const MessageMainScreen({Key? key}) : super(key: key);

  @override
  _MessageMainScreenState createState() => _MessageMainScreenState();
}

// _MessageMainScreenState 클래스 시작
// _MessageMainScreenState 클래스는 MessageMainScreen 위젯의 상태를 관리함.
// WidgetsBindingObserver 믹스인을 통해 앱 생명주기 상태 변화를 감시함.
class _MessageMainScreenState extends ConsumerState<MessageMainScreen>
    with WidgetsBindingObserver {
  // 사용자 인증 상태 변경을 감지하는 스트림 구독 객체임.
  // 이를 통해 사용자 로그인 또는 로그아웃 상태 변경을 실시간으로 감지하고 처리할 수 있음.
  StreamSubscription<User?>? authStateChangesSubscription;

  // messageScrollControllerProvider에서 ScrollController를 읽어와서 scrollController에 할당
  // ref.read(messageScrollControllerProvider)는 provider를 이용해 상태를 읽는 방식.
  ...
  // messageScrollControllerProvider : 여러 위젯에서 동일한 ScrollController를 공유하고,
  ...
  late ScrollController messageScreenPointScrollController; // 스크롤 컨트롤러 선언
  ...
  void initState() {
    ...
    messageScreenPointScrollController = ScrollController();
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (messageScreenPointScrollController.hasClients) {
        ...
        double savedScrollPosition = ref.read(messageScrollPositionProvider);
        // messageScreenPointScrollController.jumpTo 메서드를 사용하여 스크롤 위치를 savedScrollPosition으로 즉시 이동함.
        // 이는 스크롤 애니메이션이나 다른 복잡한 동작 없이 바로 지정된 위치로 점프함.
        messageScreenPointScrollController.jumpTo(savedScrollPosition);
      }
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (user == null) {
        ...
        ref.read(messageScrollPositionProvider.notifier).state = 0;
      }
    });
  ...
  void dispose() {
    ...
    messageScreenPointScrollController.dispose(); // ScrollController 해제
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Stack(
        children: [
          CustomScrollView(
            controller: messageScreenPointScrollController,
            slivers: <Widget>[
              SliverAppBar(
          ...
          buildTopButton(context, messageScreenPointScrollController),
        ],
      ),
